CREATE OR REPLACE PROCEDURE {{ envi }}_API_REF.FUSE.LH_LOADING_CYCLE_INCR_P("NUMBER_OF_DAYS" VARCHAR(16777216) DEFAULT '3')
...
* SOURCE    : {{ RO_PROD }}_WG.LOAD_HAUL.LH_LOADING_CYCLE
* TARGET    : {{ envi }}_API_REF.FUSE.LH_LOADING_CYCLE_INCR
...
sql_count_incr = `SELECT COUNT(*) AS count_check_1 
                  FROM {{ envi }}_API_REF.fuse.lh_loading_cycle_incr 
                  WHERE cycle_start_ts_local::date < DATEADD(day, -` + NUMBER_OF_DAYS + `, CURRENT_DATE);`;

sql_delete_incr = `DELETE FROM {{ envi }}_API_REF.fuse.lh_loading_cycle_incr 
                   WHERE cycle_start_ts_local::date < DATEADD(day, -` + NUMBER_OF_DAYS + `, CURRENT_DATE);`;

sql_merge = `MERGE INTO {{ envi }}_API_REF.fuse.lh_loading_cycle_incr tgt
USING (
    SELECT
        loading_cycle_id,
        site_code,
    ...
    FROM {{ RO_PROD }}_WG.load_haul.lh_loading_cycle
    WHERE cycle_start_ts_local::date >= DATEADD(day, -` + NUMBER_OF_DAYS + `, CURRENT_DATE)
) AS src
...
sql_delete = `UPDATE {{ envi }}_API_REF.fuse.lh_loading_cycle_incr tgt
              SET dw_logical_delete_flag = ''Y'', dw_modify_ts = CURRENT_TIMESTAMP(0)::TIMESTAMP_NTZ
              WHERE tgt.dw_logical_delete_flag = ''N''
              AND NOT EXISTS (SELECT 1 FROM {{ RO_PROD }}_WG.load_haul.lh_loading_cycle src
