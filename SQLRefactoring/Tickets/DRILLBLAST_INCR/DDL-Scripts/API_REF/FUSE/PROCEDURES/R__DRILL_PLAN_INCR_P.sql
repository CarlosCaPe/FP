...
-- =============================================
CREATE OR REPLACE PROCEDURE {{ envi }}_API_REF.FUSE.DRILL_PLAN_INCR_P("NUMBER_OF_DAYS" VARCHAR(16777216) DEFAULT '3', "MAX_DAYS_TO_KEEP" VARCHAR(16777216) DEFAULT '90')
...
var sql_count_incr = `SELECT COUNT(*) AS cnt FROM {{ envi }}_API_REF.fuse.drill_plan_incr 
                      WHERE DW_MODIFY_TS < DATEADD(day, -` + maxDays + `, CURRENT_TIMESTAMP())`;

// STEP 2: DELETE old records (purge) - prevents unbounded table growth
var sql_delete_incr = `DELETE FROM {{ envi }}_API_REF.fuse.drill_plan_incr 
                       WHERE DW_MODIFY_TS < DATEADD(day, -` + maxDays + `, CURRENT_TIMESTAMP())`;

// STEP 3: MERGE new/updated records from source
var sql_merge = `MERGE INTO {{ envi }}_API_REF.fuse.drill_plan_incr tgt
USING (
