...
--
-- DEV/SOS experiment variant:
-- - Uses DEV_GENERAL.TARGET.SENSOR_READING_* where available (SOS enabled there)
-- - Uses PROD_DATALAKE.FCTS.SENSOR_READING_NMO_B because NMO is missing in DEV_GENERAL.TARGET
-- - Uses DEV_TARGET.PI.* for PI mapping to match DEV
...
src_pi_af_attribute AS (
  SELECT
    a.sensor_id,
    a.attributepath
  FROM DEV_TARGET.PI.PI_PRISM_AF_ATTRIBUTE_B a
),
...
src_pi_point AS (
  SELECT
    p.sensor_id,
    p.pipointname
  FROM DEV_TARGET.PI.PI_POINT_B p
),
...
int_latest_rows AS (
  ...
  FROM IDENTIFIER(
    CASE UPPER(PARAM_SITE_CODE)
      WHEN ''SAM'' THEN ''DEV_GENERAL.TARGET.SENSOR_READING_SAM_B''
      WHEN ''MOR'' THEN ''DEV_GENERAL.TARGET.SENSOR_READING_MOR_B''
      WHEN ''CMX'' THEN ''DEV_GENERAL.TARGET.SENSOR_READING_CMX_B''
      WHEN ''SIE'' THEN ''DEV_GENERAL.TARGET.SENSOR_READING_SIE_B''
      -- NOTE: DEV_GENERAL.TARGET does not have SENSOR_READING_NMO_B; use PROD source.
      WHEN ''NMO'' THEN ''PROD_DATALAKE.FCTS.SENSOR_READING_NMO_B''
      WHEN ''BAG'' THEN ''DEV_GENERAL.TARGET.SENSOR_READING_BAG_B''
      WHEN ''CVE'' THEN ''DEV_GENERAL.TARGET.SENSOR_READING_CVE_B''
    END
  ) raw
  JOIN int_cutoff c
    ON 1 = 1
