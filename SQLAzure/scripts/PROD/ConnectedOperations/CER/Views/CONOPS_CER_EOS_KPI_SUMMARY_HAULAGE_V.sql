CREATE VIEW [CER].[CONOPS_CER_EOS_KPI_SUMMARY_HAULAGE_V] AS
...
FORECAST AS (
	...
	LEFT JOIN [cer].[CONOPS_CER_SHIFT_TARGET_V] [st] WITH (NOLOCK)
...
HAULAGE AS (
	SELECT [si].SiteFlag
		,[si].ShiftFlag
		,[si].ShiftIndex
		,(CASE WHEN [fc].Duration = 0 OR [ov].TotalMined = 0 THEN 0
			ELSE [ov].TotalMined / [fc].TotalMined END) AS Total
		,(CASE WHEN [fc].Duration = 0 OR [ef].EFH = 0THEN 0
			ELSE [ef].EFH / [fc].EFH END) AS EFH
	FROM CteShiftInfo [si]
