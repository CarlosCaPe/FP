CREATE VIEW [CER].[CONOPS_CER_EQMT_OTHER_V] AS
  
  
  
  
  
-- SELECT * FROM [cer].[CONOPS_CER_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'  ORDER BY shiftflag, siteflag, SupportEquipment  
CREATE VIEW [cer].[CONOPS_CER_EQMT_OTHER_V]  
AS  
...
SELECT [shift].shiftflag,  
   ...
   END AS EndTime,  
    ...
    ELSE concat([img].Value, RIGHT('0000000000' + [PersonnelId], 10),'.jpg') END as OperatorImageURL,  
    [shift].ShiftDuration  
...
LEFT JOIN (  
 SELECT [pax].SHIFTINDEX,  
     ...
     [op].PERSONNEL_ID AS [PersonnelId],  
 ...
 LEFT JOIN [cer].[operator_personnel_map] [op] WITH(NOLOCK)  
  ON [op].OPERATOR_ID = [w].FieldId  
) [pax]  
