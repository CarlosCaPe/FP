CREATE VIEW [CER].[CONOPS_CER_DAILY_OPERATOR_HAS_LATE_START_V] AS

  
-- SELECT * FROM [cer].[CONOPS_CER_DAILY_OPERATOR_HAS_LATE_START_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'    
CREATE VIEW [cer].[CONOPS_CER_DAILY_OPERATOR_HAS_LATE_START_V]    
AS    
...
SELECT [shift].shiftflag,    
 ...
 [shift].[shiftid],    
    ...
    CASE WHEN [PersonnelId] IS NOT NULL OR [PersonnelId] != -1    
      THEN concat([img].[value],RIGHT('0000000000' + [PersonnelId], 10),'.jpg')    
   WHEN ([PersonnelId] IS NULL OR [PersonnelId] = -1) AND ([OperatorId] IS NOT NULL OR [OperatorId] != -1)    
      THEN concat([img].[value],RIGHT('0000000000' + [OperatorId], 10),'.jpg')    
...
LEFT JOIN (    
 SELECT [LateStart].shiftindex,    
    ...
    ELSE NULL    
     ...
     CASE [LateStart].UNIT_CODE    
    WHEN 1 THEN LEFT(CAST(DATEADD(MINUTE, -15, 