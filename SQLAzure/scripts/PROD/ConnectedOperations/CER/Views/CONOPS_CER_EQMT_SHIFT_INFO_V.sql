CREATE VIEW [CER].[CONOPS_CER_EQMT_SHIFT_INFO_V] AS





--select * from [cer].[CONOPS_CER_EQMT_SHIFT_INFO_V] order by shiftflag
CREATE VIEW [cer].[CONOPS_CER_EQMT_SHIFT_INFO_V]
AS
...
SELECT sh.siteflag,
       ...
       (CONVERT(int, (DATEDIFF(DD, '7/12/2007', CAST(sh.ShiftStartDateTime AS DATE))*2) + 27412 +
                    (SELECT RIGHT(sh.[ShiftId], 1) - 1))) *10 ShiftIndex,
...
FROM (
	 SELECT a.siteflag,
            ...
            CASE WHEN b.ShiftStartDateTime IS NULL 
			  	 THEN (CASE WHEN right(a.shiftid, 1) = 1
					  		THEN concat(cast(left(a.shiftid, 6) AS date), ' 07:30:00.000')
							ELSE concat(cast(left(a.shiftid, 6) AS date), ' 19:30:00.000')
					   END)
				  ELSE b.ShiftStartDateTime
            END AS ShiftStartDateTime,
            CASE WHEN b.ShiftEndDateTime IS NULL
				 THEN (CASE WHEN right(a.shiftid, 1) = 1
							THEN concat(cast(left(a.shiftid, 6) AS date), ' 19:30:00.000')
                            ELSE concat(dateadd(DAY, 1, cast(left(a.shiftid, 6) AS date)), ' 07:30:00.000')
                       END)
     ...
     FROM
         (SELECT 'CER' AS siteflag,
          ...
          FROM [cer].[shift_info] WITH (NOLOCK)
          UNION SELECT 'CER' AS siteflag,
                       'CURR' AS shiftflag,
          ...
          UNION SELECT 'CER' AS siteflag,
                       ...
                       END AS shiftid
          FROM [cer].[shift_info] WITH (NOLOCK)) a
     LEFT JOIN
         (SELECT shiftid,
                 ShiftStartDateTime,
                 LEAD(ShiftStartDateTime) OVER ( ORDER BY shiftid) AS ShiftEndDateTime,
                 ShiftDuration
          FROM [cer].[shift_info] WITH (nolock)) b ON a.shiftid = b.shiftid) sh
