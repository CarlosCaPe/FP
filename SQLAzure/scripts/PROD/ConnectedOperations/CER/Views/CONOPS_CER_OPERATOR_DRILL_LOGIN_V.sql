CREATE VIEW [CER].[CONOPS_CER_OPERATOR_DRILL_LOGIN_V] AS



-- SELECT * FROM [CER].[CONOPS_CER_OPERATOR_DRILL_LOGIN_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [cer].[CONOPS_CER_OPERATOR_DRILL_LOGIN_V] 
AS
	...
	WITH OperatorDetail AS (
   		SELECT [ds].SHIFTINDEX,
          	   ...
          	   RIGHT('0000000000' + OPERATOR_ID, 10) AS OPERATORID,
	...
	OperatorLogin AS (
   		SELECT DISTINCT oper.shiftindex
			  ...
			  ,MIN( dateadd(second, eroot.starts + oper.logintime, CAST(eroot.shiftdate AS DATETIME))
					) OVER (PARTITION BY oper.operid, eroot.shiftindex, eroot.site_code) AS FirstLoginTime
			  ,ROW_NUMBER() OVER (PARTITION BY oper.operid, eroot.shiftindex, eroot.site_code, oper.eqmtid ORDER BY oper.logintime) AS rn
		...
		WHERE trim(oper.operid) not in ('mmsunk', '')
			AND oper.unit_code IN (14, 43)
			AND trim(eroot.site_code) = 'CER'
