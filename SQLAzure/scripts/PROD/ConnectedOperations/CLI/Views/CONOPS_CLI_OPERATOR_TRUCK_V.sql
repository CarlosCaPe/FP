CREATE VIEW [CLI].[CONOPS_CLI_OPERATOR_TRUCK_V] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_TRUCK_V] 
AS
...
LEFT JOIN [CLI].[CONOPS_CLI_TRUCK_SHIFT_OVERVIEW_V] c
...
LEFT JOIN [cli].[CONOPS_CLI_OPERATOR_TRUCK_LIST_V] [tl]
ON [shift].shiftflag = [tl].shiftflag
LEFT JOIN [cli].[CONOPS_CLI_TRUCK_POPUP] [pop] WITH (NOLOCK)
ON [pop].shiftflag = [tl].shiftflag
AND [pop].TruckID = [tl].truckid
LEFT JOIN [cli].[CONOPS_CLI_TRUCK_TPRH] [tp]
...
LEFT JOIN [CLI].[CONOPS_CLI_OPERATOR_EQMT_READY_HOURS_V] [rh]
