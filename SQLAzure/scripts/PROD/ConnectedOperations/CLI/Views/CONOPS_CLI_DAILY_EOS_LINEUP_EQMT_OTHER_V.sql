CREATE VIEW [CLI].[CONOPS_CLI_DAILY_EOS_LINEUP_EQMT_OTHER_V] AS




-- SELECT * FROM [cli].[CONOPS_CLI_DAILY_EOS_LINEUP_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'PREV'  ORDER BY shiftflag, siteflag, SupportEquipment
CREATE VIEW [cli].[CONOPS_CLI_DAILY_EOS_LINEUP_EQMT_OTHER_V]
AS
...
FROM( 
	...
	LEFT JOIN [CLI].[CONOPS_CLI_EOS_SHIFT_INFO_V] [shift]
