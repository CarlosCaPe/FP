CREATE VIEW [BAG].[CONOPS_BAG_SHOVEL_DETAIL_PER_HOUR_V] AS
...
-- SELECT * FROM [bag].[CONOPS_BAG_SHOVEL_DETAIL_PER_HOUR_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [SHOVELID]
CREATE VIEW [bag].[CONOPS_BAG_SHOVEL_DETAIL_PER_HOUR_V]
AS
...
LEFT OUTER JOIN BAG.[CONOPS_BAG_EQMT_SHOVEL_HOURLY_TOTALMATERIALMINED_V] [tmm]
	ON [tprh].shiftflag = [tmm].shiftflag AND [tprh].EQMT = [tmm].equipment AND [tprh].Hr = [tmm].TimeinHour
LEFT OUTER JOIN [bag].[CONOPS_BAG_HOURLY_TRUCK_ASSET_EFFICIENCY_V] [ae]
	ON [tprh].siteflag = [ae].siteflag 
	AND [tprh].shiftflag = [ae].shiftflag 
	AND [tprh].EQMT = [ae].Equipment 
	AND [tprh].Hr = [ae].Hr
	AND [ae].EqmtUnit = 2
LEFT OUTER JOIN BAG.[CONOPS_BAG_EQMT_SHOVEL_HOURLY_DELTAC_V] [dc]
	ON [tprh].shiftflag = [dc].shiftflag AND [tprh].EQMT = [dc].Equipment AND [tprh].Hr = [dc].deltac_ts
LEFT OUTER JOIN BAG.[CONOPS_BAG_EQMT_SHOVEL_HOURLY_PAYLOAD_V] [p]
