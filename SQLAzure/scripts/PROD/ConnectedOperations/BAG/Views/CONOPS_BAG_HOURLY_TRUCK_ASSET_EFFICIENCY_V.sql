CREATE VIEW [BAG].[CONOPS_BAG_HOURLY_TRUCK_ASSET_EFFICIENCY_V] AS


--select * from [bag].[CONOPS_BAG_HOURLY_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
CREATE VIEW [bag].[CONOPS_BAG_HOURLY_TRUCK_ASSET_EFFICIENCY_V] 
AS
...
WITH TimesDetails AS (
	SELECT SHIFTINDEX,
		   ...
		   HOS + 1 as Hr,
		   UNIT as EqmtUnit,
		   COALESCE(SUM(TotalTime), 0.0) as TotalTime,
		   COALESCE(SUM(ReadyTime), 0.0) as ReadyTime,
		   COALESCE(SUM(OperationalDownTime), 0.0) as OperationalDownTime,
		   COALESCE(SUM(ScheduledDownTime), 0.0) as ScheduledDownTime,
		   COALESCE(SUM(UnscheduledDownTime), 0.0) as UnscheduledDownTime,
		   COALESCE(SUM(DelayTime), 0.0) as DelayTime,
		   COALESCE(SUM(SpareTime), 0.0) as SpareTime,
		   COALESCE(SUM(ShiftChangeTime), 0.0) as ShiftChangeTime
	FROM(
		SELECT SHIFTINDEX,
			   SHIFTDATE,
			   EQMT,
	...
	GROUP BY SHIFTINDEX, SHIFTDATE, EQMT, HOS, UNIT
)
...
LEFT JOIN (
	SELECT  TimesDetails.shiftindex ,
			...
			, CASE WHEN TotalTime = 0 THEN 0 
				ELSE((TotalTime - ScheduledDownTime - UnscheduledDownTime) / TotalTime) * 100.00 END as Avail
			, CASE WHEN TotalTime = 0 OR ReadyTime = 0 THEN 0 
				ELSE(ReadyTime  / (TotalTime - ScheduledDownTime - UnscheduledDownTime)) * 100.00 END as UofA
			, CASE WHEN TotalTime = 0 OR ReadyTime = 0 THEN 0 
				ELSE(ReadyTime / TotalTime) * 100.00 END as AE
	FROM    TimesDetails
) [truckAE]
