CREATE VIEW [BAG].[CONOPS_BAG_EOS_TONS_MINED_V] AS

--SELECT * FROM [BAG].[CONOPS_BAG_EOS_TONS_MINED_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [bag].[CONOPS_BAG_EOS_TONS_MINED_V]
AS
...
FROM(
	SELECT
		...
		ROUND(t.TonsMovedTarget/1000.0, 1) AS TotalMaterialMovedTarget
	FROM [BAG].[CONOPS_BAG_EOS_TOTAL_MATERIAL_V] ov
	LEFT JOIN [BAG].[CONOPS_BAG_SHIFT_TARGET_V] t
