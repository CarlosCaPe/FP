...
WHERE C.ECF_CLASS_ID IN ('XAEntity.Cycle.ProductionCycle.LoaderCycle')
...
SELECT
	...
	CASE WHEN shifttype = 0
		...
		ELSE NULL
	END)) AS SHIFT_ID
	-- Replace(shifttype, '0', '2')) AS SHIFT_ID,
	-- PRIMARYMACHINE AS TRUCK_ID,
	,SM.NAME AS TRUCK_NAME
	,SMC.NAME AS TRUCK_EQUIP_CLASS
	,SO.PERSONNELID AS TRUCK_OPERATOR_ID
	,SO.NAME AS TRUCK_OPERATOR_NAME
	,PM.NAME AS SHOVEL_NAME
	,TMC.NAME AS SHOVEL_EQUIP_CLASS
	,PO.PERSONNELID AS SHOVEL_OPERATOR_ID
	,PO.NAME AS SHOVEL_OPERATOR_NAME
