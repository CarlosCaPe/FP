CREATE VIEW [BAG].[CONOPS_BAG_DAILY_HOURLY_TOTALMATERIALMINED_V] AS



   
--select * from [bag].[CONOPS_BAG_DAILY_HOURLY_TOTALMATERIALMINED_V]   
CREATE VIEW [bag].[CONOPS_BAG_DAILY_HOURLY_TOTALMATERIALMINED_V]      
AS      
...
FROM BAG.CONOPS_BAG_EOS_SHIFT_INFO_V s
LEFT JOIN GroupDumpTime f
	ON s.shiftid = f.shiftid
LEFT JOIN bag.CONOPS_BAG_SHIFT_