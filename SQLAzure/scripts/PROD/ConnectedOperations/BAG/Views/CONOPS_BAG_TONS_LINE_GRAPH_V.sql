CREATE VIEW [BAG].[CONOPS_BAG_TONS_LINE_GRAPH_V] AS


--select * from [bag].[CONOPS_BAG_TONS_LINE_GRAPH_V] where shiftflag = 'curr'
CREATE VIEW [bag].[CONOPS_BAG_TONS_LINE_GRAPH_V]
AS
...
SELECT
    shiftid,
    dumping_time,
    SUM(NrOfDumps) OVER (PARTITION BY shiftid ORDER BY dumping_time) AS CumNrOfDumps,
    SUM(TotalMaterialMoved) OVER (PARTITION BY shiftid ORDER BY dumping_time) AS CumTotalMaterialMoved,
    SUM(TotalMaterialMined) OVER (PARTITION BY shiftid ORDER BY dumping_time) AS CumTotalMaterialMined,
    SUM(ExPitTons) OVER (PARTITION BY shiftid ORDER BY dumping_time) AS CumExPitTons,
    SUM(TotalMaterialDeliveredToCrusher) OVER (PARTITION BY shiftid ORDER BY dumping_time) AS CumTotalMaterialDeliveredToCrusher,
    SUM(RehandleOre) OVER (PARTITION BY shifti