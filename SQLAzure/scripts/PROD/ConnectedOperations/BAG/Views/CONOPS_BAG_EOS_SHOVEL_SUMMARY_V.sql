...
SELECT
	...
	CASE WHEN MEASURED_PAYLOAD_SHORT_TONS >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'BAG')
...
FROM LH_LOAD
GROUP BY shift_oid, ShovelId, PushBack
),
...
GROUP BY SHIFTINDEX, SITE_CODE, EQMT, CASE WHEN LEFT(LOC,4) BETWEEN '0000' AND '9999' THEN SUBSTRING(LOC,6,2) ELSE LOC END
)
