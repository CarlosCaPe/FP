...
SELECT
	...
	REPLACE(REPLACE(REPLACE(JOB_NAME, 'job_conops_', ''), '_bag', ''), '_2', '') AS TABLE_NAME,
...
FROM dbo.DI_JOB_CONTROL_ENTRY_TS_BASE WITH(NOLOCK)
WHERE JOB_NAME IN (
'job_conops_equipment_hourly_status_BAG_2', 'job_conops_asset_efficiency_BAG', 'job_conops_drill_utilization_BAG_2',
...
SELECT
	JOB_NAME,
	REPLACE(REPLACE(JOB_NAME, 'job_conops_', ''), '_bag', '') AS TABLE_NAME,
