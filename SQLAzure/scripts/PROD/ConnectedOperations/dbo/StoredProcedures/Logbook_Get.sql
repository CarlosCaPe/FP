		...
		IF(@Page IS NULL)
			BEGIN
				...
				WHERE
					...
					A.ProcessId = ISNULL(@ProcessId, CASE WHEN A.LogbookTypeCode = 'MO' THEN 'CON'  ELSE '' END) AND -- ISNULL to support backward compatible
					A.SubProcessId = ISNULL(@SubProcessId, CASE WH