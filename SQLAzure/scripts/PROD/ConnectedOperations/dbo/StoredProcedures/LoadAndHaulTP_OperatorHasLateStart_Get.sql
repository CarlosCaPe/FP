...
* {10 Nov 2025}		{dbonardo}		{String Split Using UDT}
*******************************************************************/ 
...
BEGIN TRY
	...
	BEGIN
		...
		WHERE [ls].shiftflag = @SHIFT
			AND unit_code = 1
			AND ([t].TruckID IN (SELECT [Value] FROM @splitEqmt) OR @EQMT IS NULL)
			AND ([t].eqmttype IN (SELECT [Value] FROM @splitEType) OR @EQMTTYPE IS NULL)
			AND ([t].StatusName IN (SELECT [Value] FROM @splitEStat) OR @STATUS IS NULL)
