...
/******************************************************************    
* PROCEDURE : dbo.[UPSERT_CONOPS_DRILL_UTILIZATION]  
* PURPOSE : Upsert [UPSERT_CONOPS_DRILL_UTILIZATION]  
* NOTES     :   
* CREATED : ggosal1 
* SAMPLE    : EXEC dbo.[UPSERT_CONOPS_DRILL_UTILIZATION] 'cer'
...
* {22 JAN 2025}  {asatria}   {Add filter for TYR, source system <> RCS}  
*******************************************************************/    
CREATE  PROCEDURE [dbo].[UPSERT_CONOPS_DRILL_UTILIZATION]  
(  
@G_SITE VARCHAR(5)  
)  
AS  
...
IF @G_SITE = 'BAG'
BEGIN
	DELETE FROM bag.DRILL_UTILIZATION_STG
	WHERE SOURCE_SYSTEM = 'Modular'
END

ELSE IF @G_SITE = 'MOR'
BEGIN
	DELETE FROM mor.DRILL_UTILIZATION_STG
	WHERE SOURCE_SYSTEM = 'Modular'
END

ELSE IF @G_SITE = 'TYR'
BEGIN
	DELETE FROM tyr.DRILL_UTILIZATION_STG
	WHERE SOURCE_SYSTEM = 'RCS'
END

EXEC   
(  

'MERGE ' +@G_SITE+ '.DRILL_UTILIZATION AS T'  
+' USING ('
+' SELECT ' 
+' SITE_CODE'
+' ,SHIFTINDEX'
...
+' ,SOURCE_SYSTEM' 
+' ,UTC_CREATED_DATE'
+' FROM ' +@G_SITE+ '.DRILL_UTILIZATION_STG ) AS S   '
