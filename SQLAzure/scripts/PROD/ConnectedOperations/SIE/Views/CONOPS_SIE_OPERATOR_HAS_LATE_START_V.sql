CREATE VIEW [SIE].[CONOPS_SIE_OPERATOR_HAS_LATE_START_V] AS

  
   
-- SELECT * FROM [sie].[CONOPS_SIE_OPERATOR_HAS_LATE_START_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'    
CREATE VIEW [sie].[CONOPS_SIE_OPERATOR_HAS_LATE_START_V]     
AS    
...
SELECT DISTINCT oper.shiftindex
    ...
    FROM SIE.CONOPS_SIE_SHIFT_INFO_V (nolock) AS eroot
