CREATE VIEW [SIE].[CONOPS_SIE_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] AS
  
  
  
--select * from [sie].[CONOPS_SIE_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V]  
CREATE VIEW [sie].[CONOPS_SIE_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V]  
AS  
  
WITH TimesDetails AS (  
 SELECT SHIFTINDEX,  
     CASE WHEN [EQUIPMENTMODEL] = 'ADS PV351' OR [EQUIPMENTMODEL] = 'ADS PV271' THEN 'DR' + LEFT(EQMT, 2)  
       ELSE EQMT  
     END AS Equipment,  
 ...
 FROM(  
  ...
  FROM [sie].[DRILL_UTILIZATION] [HOS] WITH (NOLOCK)  
  ...
  WHERE [HOS].[MACHINESTATUSCODE] IS NOT NULL  
 ) [AE]  
 GROUP BY SHIFTINDEX, EQMT, [EQUIPMENTMODEL]  
)  
