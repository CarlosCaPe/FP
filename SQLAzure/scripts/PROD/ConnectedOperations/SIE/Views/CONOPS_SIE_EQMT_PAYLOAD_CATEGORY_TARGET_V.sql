CREATE VIEW [SIE].[CONOPS_SIE_EQMT_PAYLOAD_CATEGORY_TARGET_V] AS
...
-- SELECT * FROM [sie].[CONOPS_SIE_EQMT_PAYLOAD_CATEGORY_TARGET_V] WHERE shiftindex = '38916'  
CREATE VIEW [sie].[CONOPS_SIE_EQMT_PAYLOAD_CATEGORY_TARGET_V]  
AS  
...
LEFT JOIN dbo.LH_LOAD ld WITH (NOLOCK) ON t.SHIFTINDEX = ld.SHIFTINDEX AND s.FieldId = ld.excav AND ld.SITE_CODE = 'SIE'  
left join [sie].enum enum WITH (NOLOCK)on enum.id = t.FieldEqmttype),  
...
0.05 * 269 AS UnderLoadedTarget,  
FLOOR(269 - (0.05 * 269)) AS UnderLoaded,  
