CREATE VIEW [ABR].[CONOPS_ABR_TRUCK_POPUP_V] AS



-- SELECT * FROM [abr].[CONOPS_ABR_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_TRUCK_POPUP_V] 
AS
...
FROM [abr].[CONOPS_ABR_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V] WITH (NOLOCK))
...
SELECT [t].shiftflag,
       ...
       COALESCE([dct].LoadedTravelTarget, 0) AS LoadedTravelTarget,
	   COALESCE(EmptyTravel, 0) AS EmptyTravel,
       COALESCE([dct].EmptyTravelTarget, 0) AS EmptyTravelTarget,
...
LEFT JOIN [abr].[CONOPS_ABR_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK)
...
LEFT JOIN [abr].[CONOPS_ABR_TP_DELTA_C_AVG_V] [dc] WITH (NOLOCK)
ON [t].shiftindex = [dc].shiftindex AND  [t].TruckID = [dc].truck
LEFT JOIN AE ae ON ae.shiftflag = t.shiftflag AND ae.eqmt = t.TruckID
LEFT JOIN [abr].[CONOPS_ABR_DELTA_C_TARGET_V] [dct] ON [t].shiftid = [dct].shiftid
