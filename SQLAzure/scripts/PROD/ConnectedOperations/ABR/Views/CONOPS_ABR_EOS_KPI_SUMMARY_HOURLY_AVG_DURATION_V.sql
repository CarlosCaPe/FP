CREATE VIEW [ABR].[CONOPS_ABR_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS




-- SELECT * FROM [abr].[CONOPS_ABR_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [ABR].[CONOPS_ABR_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]  
AS  
...
StatusEvent AS (
	...
	FROM (
		...
		FROM [abr].[CONOPS_ABR_SHIFT_INFO_V] [si] WITH (NOLOCK)
