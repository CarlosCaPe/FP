CREATE VIEW [ABR].[CONOPS_ABR_SP_EQMT_STATUS_GANTTCHART_V] AS



--select * from [abr].[CONOPS_ABR_SP_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'curr'order by eqmt

CREATE VIEW [ABR].[CONOPS_ABR_SP_EQMT_STATUS_GANTTCHART_V]
AS
...
b.ShiftEndDateTime
FROM [abr].[CONOPS_ABR_EQMT_SHIFT_INFO_V] a
LEFT JOIN (
SELECT 
siteflag,
shiftflag,
...
FROM [abr].[CONOPS_ABR_EQMT_SHIFT_INFO_V]
...
eqmt,
tprh 
FROM [abr].[CONOPS_ABR_SHOVEL_TPRH_V] WITH (NOLOCK)
WHERE site_code = 'ELA')
