CREATE VIEW [ABR].[CONOPS_ABR_EOS_LINEUP_EQMT_OTHER_V] AS





-- SELECT * FROM [abr].[CONOPS_ABR_EOS_LINEUP_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'  ORDER BY shiftflag, siteflag, SupportEquipment
CREATE VIEW [ABR].[CONOPS_ABR_EOS_LINEUP_EQMT_OTHER_V]
AS
...
FROM ( 
	...
	LEFT JOIN [ABR].[CONOPS_ABR_SHIFT_INFO_V] [shift] WITH (NOLOCK)
