CREATE VIEW [ABR].[CONOPS_ABR_SHIFT_INFO_V] AS
...
--SELECT * FROM [abr].[CONOPS_ABR_SHIFT_INFO_V]      
CREATE VIEW [abr].[CONOPS_ABR_SHIFT_INFO_V]        
AS      
...
FROM (      
 SELECT A.SITEFLAG,      
    ...
    THEN (CASE WHEN RIGHT(A.SHIFTID, 1) = 1      
         THEN CONCAT(CAST(LEFT(A.SHIFTID, 6) AS DATE), ' 08:00:00.000')      
         ELSE CONCAT(CAST(LEFT(A.SHIFTID, 6) AS DATE), ' 20:00:00.000')      
       END)      
    ELSE B.SHIFTSTARTDATETIME      
     END AS SHIFTSTARTDATETIME,      
     CASE WHEN B.SHIFTENDDATETIME IS NULL      
    THEN (CASE WHEN RIGHT(A.SHIFTID, 1) = 1      
         THEN CONCAT(CAST(LEFT(A.SHIFTID, 6) AS DATE), ' 20:00:00.000')      
         ELSE CONCAT(DATEADD(DAY, 1, CAST(LEFT(A.SHIFTID, 6) AS DATE)), ' 08:00:00.000')      
    END)      
