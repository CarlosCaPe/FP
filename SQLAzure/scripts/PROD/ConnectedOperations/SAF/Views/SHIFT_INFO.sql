CREATE VIEW [SAF].[SHIFT_INFO] AS

--SELECT * FROM SAF.SHIFT_INFO
CREATE VIEW [SAF].[SHIFT_INFO]
AS
...
FROM [SAFOperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [SAFOperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [SAFOperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [SAFOperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'SAF' AS SiteFlag,
