CREATE VIEW [SAF].[CONOPS_SAF_DAILY_EOS_TONS_MINED_V] AS

--SELECT * FROM [SAF].[CONOPS_SAF_DAILY_EOS_TONS_MINED_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [saf].[CONOPS_SAF_DAILY_EOS_TONS_MINED_V]
AS
...
FROM(
	...
	FROM [SAF].[CONOPS_SAF_DAILY_EOS_TOTAL_MATERIAL_V] ov
	LEFT JOIN [SAF].[CONOPS_SAF_SHIFT_BASE_TARGET_V] t
