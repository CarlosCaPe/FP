CREATE VIEW [SAF].[CONOPS_SAF_OPERATOR_SHOVEL_V] AS
...
-- SELECT * FROM [SAF].[CONOPS_SAF_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [SAF].[CONOPS_SAF_OPERATOR_SHOVEL_V]  
AS  
...
SELECT   
 ...
 ,[es].Hangtime  
 ,[es].HangtimeTarget 
FROM [SAF].[CONOPS_SAF_OPERATOR_SHOVEL_LIST_V] [os]  
LEFT JOIN [SAF].[CONOPS_SAF_SHIFT_INFO_V] [shift]   
 ON [os].shiftid = [shift].shiftid  
LEFT JOIN [SAF].[CONOPS_SAF_SHOVEL_POPUP] [es] WITH (NOLOCK)
 ON [os].ShiftFlag = [es].shiftflag  
 AND [os].ShovelID = [es].shovelid  
LEFT JOIN [SAF].[CONOPS_SAF_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] [ae]  
 ON [os].shiftid = [ae].shiftid  
 AND [os].ShovelID = [ae].eqmt  
CROSS JOIN [SAF].[CONOPS_SAF_EQMT_ASSET_EFFICIENCY_TARGET_V] [dct]  
