CREATE VIEW [SAF].[CONOPS_SAF_DAILY_EOS_EQUIPMENT_READINESS_V] AS

--select * from [SAF].[CONOPS_SAF_DAILY_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [saf].[CONOPS_SAF_DAILY_EOS_EQUIPMENT_READINESS_V]
AS
...
SELECT TOP 1
	...
	ELECSHOVELAVGREADYNUMBERS AS ShovelReadinessTarget,
	8 AS DrillReadinessTarget
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			FROM SAF.EQUIPMENT_HOURLY_STATUS WITH(NOLOCK)
			WHERE
		...
		INNER JOIN SAF.CONOPS_SAF_EOS_SHIFT_INFO_V s
