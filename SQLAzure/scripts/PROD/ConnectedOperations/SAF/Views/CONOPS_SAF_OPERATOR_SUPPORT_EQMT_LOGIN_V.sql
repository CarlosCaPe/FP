CREATE VIEW [SAF].[CONOPS_SAF_OPERATOR_SUPPORT_EQMT_LOGIN_V] AS





-- SELECT * FROM [saf].[CONOPS_SAF_OPERATOR_SUPPORT_EQMT_LOGIN_V] WHERE SHIFTFLAG = 'CURR'
CREATE VIEW [saf].[CONOPS_SAF_OPERATOR_SUPPORT_EQMT_LOGIN_V]
AS
...
WITH OperatorDetail AS (
    SELECT [shiftflag]
        ...
        ,[OperatorImageURL]
    FROM [saf].[CONOPS_SAF_EQMT_OTHER_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
