CREATE VIEW [SAF].[CONOPS_SAF_EOS_KPI_SUMMARY_HAULAGE_V] AS


-- SELECT * FROM [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'  
...
FORECAST AS (
	...
	LEFT JOIN [saf].[CONOPS_SAF_SHIFT_TARGET_V] [st] WITH (NOLOCK)
...
EFHCTE AS (
	...
	FROM (
		SELECT ShiftFlag
			...
			,EFH
		FROM [saf].[CONOPS_SAF_EFH_V] WITH (NOLOCK)
	) AS efhc
	WHERE rn = 1
),
