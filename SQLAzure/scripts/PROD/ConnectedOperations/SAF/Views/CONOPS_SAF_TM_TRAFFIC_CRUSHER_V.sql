CREATE VIEW [SAF].[CONOPS_SAF_TM_TRAFFIC_CRUSHER_V] AS
  
    
    
    
    
--SELECT * FROM [saf].[CONOPS_SAF_TM_TRAFFIC_CRUSHER_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'      
CREATE VIEW [saf].[CONOPS_SAF_TM_TRAFFIC_CRUSHER_V]      
AS      
 ...
 WITH CRUSHER AS (    
  ...
  LEFT JOIN [saf].[PIT_LOC_C] loc WITH (NOLOCK)    
  ON [shift].SHIFTINDEX = [loc].SHIFTINDEX    
  INNER JOIN [saf].[PIT_AUXEQMT_C] aux WITH (NOLOCK)    
  ON loc.[FieldId] = 'CRUSHER' AND aux.[FieldId] = 'PC001'    
  ...
  LEFT JOIN [saf].ENUM e WITH (NOLOCK)    
