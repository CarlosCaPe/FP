CREATE VIEW [CHI].[CONOPS_CHI_TP_DELTA_C_V] AS
  
    
    
--select * FROm [chi].[CONOPS_CHI_TP_DELTA_C_V] WITH (NOLOCK) ORDER BY shiftflag, truck     
CREATE VIEW [chi].[CONOPS_CHI_TP_DELTA_C_V]     
AS    
...
LoadedTravel,    
EmptyTravel    
FROM [chi].[CONOPS_CHI_TP_DELTA_C_AVG_V]    
),    
...
SELECT pop.shiftflag,    
    ...
    pop.EmptyTravelTarget,    
       ...
       st.[eqmtcurrstatus]    
FROM DELTAC dc    
LEFT JOIN [chi].[CONOPS_CHI_TRUCK_POPUP] pop WITH (NOLOCK) ON dc.shiftindex = pop.shiftindex AND dc.truck = pop.TruckID    
LEFT JOIN STAT st ON st.shiftid = pop.shiftid AND dc.truck = st.eqmt AND st.num = 1    
