CREATE VIEW [CHI].[CONOPS_CHI_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS
...
-- SELECT * FROM [chi].[CONOPS_CHI_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [chi].[CONOPS_CHI_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]  
AS  
...
StatusEvent AS (
	...
	FROM (
		...
		FROM [chi].[CONOPS_CHI_SHIFT_INFO_V] [si] WITH (NOLOCK)
