CREATE VIEW [MOR].[CONOPS_MOR_TP_EQMT_STATUS_GANTTCHART_V] AS

--select * from [mor].[CONOPS_MOR_TP_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'prev'
CREATE VIEW [mor].[CONOPS_MOR_TP_EQMT_STATUS_GANTTCHART_V] 
AS
...
SELECT
	...
	x.eqmt,
	x.eqmtcurrstatus
FROM MOR.CONOPS_MOR_SHIFT_INFO_V AS si
LEFT JOIN (
	SELECT
		shiftid,
		eqmt,
...
LEFT JOIN STAT st
	...
	AND e.eqmt = st.eqmt
LEFT JOIN MOR.CONOPS_MOR_TRUCK_POPUP tp WITH(NOLOCK)
