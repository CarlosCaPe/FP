CREATE VIEW [MOR].[ASSET_EFFICIENCY_V] AS




--SELECT * FROM [mor].[ASSET_EFFICIENCY_V]
CREATE VIEW [mor].[ASSET_EFFICIENCY_V] 
AS
...
Final AS(
	...
	FROM (
		SELECT REVERSE(PARSENAME(REPLACE(REVERSE([ShiftDate]), '-', '.'), 1)) AS [Year],
			...
			START_TIME_TS,
			END_TIME_TS,
			DATEDIFF(MILLISECOND, START_TIME_TS, LAG(START_TIME_TS, 1, END_TIME_TS) OVER (Partition BY SHIFTINDEX, EQMT ORDER BY SHIFTINDEX, EQMT, START_TIME_TS DESC)) / 1000 AS DURATION,
			HOS.STATUS,
