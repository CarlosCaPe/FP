CREATE VIEW [TYR].[CONOPS_TYR_DAILY_TRUCK_POPUP_V] AS





-- SELECT * FROM [tyr].[CONOPS_TYR_DAILY_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [TYR].[CONOPS_TYR_DAILY_TRUCK_POPUP_V] 
AS
...
AE AS (
	...
	FROM [tyr].[CONOPS_TYR_DAILY_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V] WITH (NOLOCK)
),
...
SELECT DISTINCT [t].shiftflag
	...
	,[t].Location
FROM [tyr].[CONOPS_TYR_DAILY_TRUCK_DETAIL_V] [t] WITH (NOLOCK)
LEFT JOIN [tyr].[CONOPS_TYR_DAILY_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK) 
