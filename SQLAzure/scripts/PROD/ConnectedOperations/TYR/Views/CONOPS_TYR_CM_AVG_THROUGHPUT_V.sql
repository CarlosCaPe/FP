CREATE VIEW [TYR].[CONOPS_TYR_CM_AVG_THROUGHPUT_V] AS



  
-- SELECT * FROM [tyr].[CONOPS_TYR_CM_AVG_THROUGHPUT_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [TYR].[CONOPS_TYR_CM_AVG_THROUGHPUT_V]    
AS    
...
WITH ShiftAvg AS (  
 SELECT ShiftIndex  
  ...
  --,Hr  
 FROM [tyr].[CONOPS_TYR_CM_HOURLY_CRUSHER_THROUGHPUT_V] WITH (NOLOCK)  
...
FROM (   
 ...
 FROM [tyr].[CONOPS_TYR_CM_HOURLY_CRUSHER_THROUGHPUT_V] [hct] WITH (NOLOCK)  
