CREATE VIEW [TYR].[CONOPS_TYR_OEE_V] AS
 
--select * from [tyr].[CONOPS_TYR_OEE_V]
CREATE VIEW [TYR].[CONOPS_TYR_OEE_V]
AS  
...
SELECT s.siteflag
	...
	,CASE WHEN TotalCycleAVG = 0 OR OptimalPayload = 0 THEN 0
		...
		END AS OEE
FROM TYR.CONOPS_TYR_SHIFT_INFO_V s
LEFT JOIN TYR.CONOPS_TYR_ASSET_EFFICIENCY_V ae ON s.shiftflag = ae.shiftflag
