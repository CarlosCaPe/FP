CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_TRUCK_V] AS
...
-- SELECT * FROM [tyr].[CONOPS_TYR_OPERATOR_TRUCK_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_TRUCK_V] 
AS
...
FROM [TYR].[CONOPS_TYR_TRUCK_DETAIL_V] a
LEFT JOIN [TYR].[CONOPS_TYR_TRUCK_SHIFT_OVERVIEW_V] c
...
,CASE WHEN ([shift].ShiftDuration/3600.00) = 0 OR (([shift].ShiftDuration/3600.00) * ([Availability]) / 100) = 0 OR [Availability] IS NULL THEN 0
...
FROM [tyr].[CONOPS_TYR_SHIFT_INFO_V] [shift]
LEFT JOIN [tyr].[CONOPS_TYR_OPERATOR_TRUCK_LIST_V] [tl]
ON [shift].shiftflag = [tl].shiftflag
LEFT JOIN [tyr].[CONOPS_TYR_TRUCK_POPUP] [pop]
ON [pop].shiftflag = [tl].shiftflag
AND [pop].TruckID = [tl].truckid
LEFT JOIN [tyr].[CONOPS_TYR_TRUCK_TPRH] [tp]
...
LEFT JOIN [TYR].[CONOPS_TYR_OPERATOR_EQMT_READY_HOURS_V] [rh]
