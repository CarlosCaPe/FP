CREATE VIEW [TYR].[CONOPS_TYR_EOS_HAULAGE_V] AS




--SELECT * FROM [tyr].[CONOPS_TYR_EOS_HAULAGE_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [TYR].[CONOPS_TYR_EOS_HAULAGE_V]
AS
...
WITH NrOfLoad AS(
	...
	FROM [tyr].[CONOPS_TYR_SP_NROFLOAD_V]
	GROUP BY shiftflag
),
...
FROM(
	...
	FROM [tyr].[CONOPS_TYR_DELTA_C_DETAIL_V] [dc]
