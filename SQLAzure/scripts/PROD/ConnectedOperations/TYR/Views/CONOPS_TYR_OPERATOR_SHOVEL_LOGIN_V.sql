CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_SHOVEL_LOGIN_V] AS





-- SELECT * FROM [tyr].[CONOPS_TYR_OPERATOR_SHOVEL_LOGIN_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_SHOVEL_LOGIN_V] 
AS
...
WITH OperatorDetail AS (
    SELECT [shiftflag]
        ...
        ,[OperatorImageURL]
    FROM [tyr].[CONOPS_TYR_SHOVEL_INFO_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
