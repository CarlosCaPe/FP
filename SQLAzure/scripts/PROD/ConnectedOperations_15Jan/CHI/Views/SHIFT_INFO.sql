CREATE VIEW [CHI].[SHIFT_INFO] AS

--SELECT * FROM CHI.SHIFT_INFO
CREATE VIEW [CHI].[SHIFT_INFO]
AS
...
FROM [CHNOperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [CHNOperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [CHNOperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [CHNOperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'CHI' AS SiteFlag,
