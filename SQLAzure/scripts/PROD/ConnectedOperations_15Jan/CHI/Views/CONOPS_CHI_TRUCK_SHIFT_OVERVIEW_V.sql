CREATE VIEW [CHI].[CONOPS_CHI_TRUCK_SHIFT_OVERVIEW_V] AS
...
--select * from [chi].[CONOPS_CHI_TRUCK_SHIFT_OVERVIEW_V] where shiftid = '230615001'
-- SELECT shiftid, sum(TotalMaterialMined) FROM [chi].[CONOPS_CHI_TRUCK_SHIFT_OVERVIEW_V] where shiftid = '230807002' group by shiftid
CREATE VIEW [chi].[CONOPS_CHI_TRUCK_SHIFT_OVERVIEW_V]
AS
...
FROM (
     ...
     FROM (
          ...
          LEFT JOIN chi.shift_eqmt s WITH (NOLOCK)  ON s.Id = dumps.FieldTruck AND s.SHIFTID = dumps.origshiftid
