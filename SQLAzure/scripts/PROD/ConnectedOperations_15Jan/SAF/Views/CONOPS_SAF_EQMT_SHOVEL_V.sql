CREATE VIEW [SAF].[CONOPS_SAF_EQMT_SHOVEL_V] AS

  
  
  
-- SELECT * FROM [saf].[CONOPS_SAF_EQMT_SHOVEL_V] where shiftflag = 'prev'  
CREATE VIEW [saf].[CONOPS_SAF_EQMT_SHOVEL_V]  
AS  
...
SELECT  
 ...
 IdleTime,  
 IdleTimeTarget  
FROM [saf].[CONOPS_SAF_SHOVEL_POPUP] WITH (NOLOCK)),  
...
FROM [SAF].[CONOPS_SAF_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] ae  
CROSS JOIN [SAF].[CONOPS_SAF_EQMT_ASSET_EFFICIENCY_TARGET_V] t),  
...
-- OverLoaded,  
-- InvalidPayload  
--FROM [saf].[CONOPS_SAF_EQMT_PAYLOAD_CATEGORY_V]),  
...
--FROM [saf].[CONOPS_SAF_EQMT_PAYLOAD_CATEGORY_TARGET_V]),  
