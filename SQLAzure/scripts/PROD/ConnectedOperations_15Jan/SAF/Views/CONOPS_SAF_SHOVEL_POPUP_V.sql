CREATE VIEW [SAF].[CONOPS_SAF_SHOVEL_POPUP_V] AS



-- SELECT * FROM [saf].[CONOPS_SAF_SHOVEL_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [saf].[CONOPS_SAF_SHOVEL_POPUP_V]
AS
...
SELECT TOP 1
	ps.SITEFLAG,
	cast((ELECSHOVELAVAILABILITY) as decimal(5,2)) * 100AS AvailabilityTarget
...
FROM [saf].[CONOPS_SAF_OVERVIEW_V]
...
LEFT JOIN [saf].[CONOPS_SAF_SP_DELTA_C_AVG_V] [dc]
...
LEFT JOIN [SAF].[CONOPS_SAF_SHOVEL_TPRH_V] [tprh] WITH (NOLOCK)
	ON [s].shiftindex = [tprh].shiftindex
	AND [s].ShovelID = [tprh].EQMT
LEFT JOIN [SAF].[CONOPS_SAF_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] [ae] WITH (NOLOCK)
