CREATE VIEW [SAF].[CONOPS_SAF_OPERATOR_TRUCK_V] AS



-- SELECT * FROM [saf].[CONOPS_SAF_OPERATOR_TRUCK_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [saf].[CONOPS_SAF_OPERATOR_TRUCK_V] 
AS
...
LEFT JOIN [SAF].[CONOPS_SAF_TRUCK_SHIFT_OVERVIEW_V] c
...
FROM [saf].[CONOPS_SAF_SHIFT_INFO_V] [shift]
LEFT JOIN [saf].[CONOPS_SAF_OPERATOR_TRUCK_LIST_V] [tl]
ON [shift].shiftflag = [tl].shiftflag
LEFT JOIN [saf].[CONOPS_SAF_TRUCK_POPUP] [pop] WITH (NOLOCK)
ON [pop].shiftflag = [tl].shiftflag
AND [pop].TruckID = [tl].truckid
LEFT JOIN [saf].[CONOPS_SAF_TRUCK_TPRH] [tp]
...
LEFT JOIN [SAF].[CONOPS_SAF_OPERATOR_EQMT_READY_HOURS_V] [rh]
