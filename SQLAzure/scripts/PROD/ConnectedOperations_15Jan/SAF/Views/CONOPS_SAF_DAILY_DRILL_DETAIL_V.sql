CREATE VIEW [SAF].[CONOPS_SAF_DAILY_DRILL_DETAIL_V] AS
  
      
-- SELECT * FROM [saf].[CONOPS_SAF_DAILY_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]        
CREATE VIEW [saf].[CONOPS_SAF_DAILY_DRILL_DETAIL_V]        
AS        
...
OperatorDetail AS (        
 SELECT [ds].SHIFTINDEX,        
        [ds].[SITE_CODE],        
     'D' + RIGHT('000' + SUBSTRING(REPLACE(DRILL_ID, ' ',''),CHARINDEX('-',REPLACE(DRILL_ID, ' ',''))+1,LEN(REPLACE(DRILL_ID, ' ',''))), 3) AS DRILL_ID,        
     [ds].PATTERN_NO,        
     OPERATORID,        
     [w].CREW,        
...
SELECT DISTINCT ae.[shiftflag] ,        
 ...
 ae.[ShiftId],    
   ...
   ELSE concat([img].[value],        
    ...
    [es].[Duration],        
    [Holes_Drilled] ,        
       43.98 AS [Hole_Drilled_Target] ,        
       [Feet_Drilled] ,        
       2330.94 AS [Feet_Drilled_Target] ,        
       [Avail] ,        
       [DRILLAVAILABILITY] ,        
 ...
 DRILLASSETEFFICIENCY AS AssetEfficiencyTarget,      
    ...
    [XY_Drill_Score],        
    [OVERALLSCORE]        
FROM [saf].[CONOPS_SAF_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)        
LEFT JOIN [saf].[CONOPS_SAF_DAILY_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)        
