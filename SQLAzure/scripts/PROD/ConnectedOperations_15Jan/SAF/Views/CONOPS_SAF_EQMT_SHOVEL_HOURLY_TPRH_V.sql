CREATE VIEW [SAF].[CONOPS_SAF_EQMT_SHOVEL_HOURLY_TPRH_V] AS

--select * from [saf].[CONOPS_SAF_EQMT_SHOVEL_HOURLY_TPRH_V] where shiftflag = 'curr'
CREATE VIEW [saf].[CONOPS_SAF_EQMT_SHOVEL_HOURLY_TPRH_V]
AS
...
FROM(
	...
	FROM SAF.EQUIPMENT_HOURLY_STATUS (NOLOCK) AS HOS
	WHERE HOS.UNIT = '2' 
) AS sub1
...
FROM SAF.SHIFT_LOAD_DETAIL_V (NOLOCK)
