CREATE VIEW [SAF].[CONOPS_SAF_EOS_LINEUP_SHOVEL_V] AS



-- SELECT * FROM [saf].[CONOPS_SAF_EOS_LINEUP_SHOVEL_V]  WHERE [shiftflag] = 'CURR'
CREATE VIEW [saf].[CONOPS_SAF_EOS_LINEUP_SHOVEL_V] 
AS
...
FROM (
	SELECT [shift].shiftflag,
		   ...
		   [s].FieldReason
	FROM [SAF].[pit_excav_c] [s] WITH (NOLOCK)
	LEFT JOIN [SAF].[CONOPS_SAF_SHIFT_INFO_V] [shift] WITH (NOLOCK)
