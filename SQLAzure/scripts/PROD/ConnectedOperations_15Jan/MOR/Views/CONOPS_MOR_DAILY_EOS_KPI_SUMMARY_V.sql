CREATE VIEW [MOR].[CONOPS_MOR_DAILY_EOS_KPI_SUMMARY_V] AS
...
-- SELECT * FROM [mor].[CONOPS_MOR_DAILY_EOS_KPI_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'      
CREATE VIEW [mor].[CONOPS_MOR_DAILY_EOS_KPI_SUMMARY_V]      
AS      
...
FROM [mor].[CONOPS_MOR_DAILY_DELTA_J_V]  
WHERE DeltaJ <> 0  
