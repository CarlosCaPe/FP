CREATE VIEW [MOR].[CONOPS_MOR_OPERATOR_SUPPORT_EQMT_LOGIN_V] AS





-- SELECT * FROM [mor].[CONOPS_MOR_OPERATOR_SUPPORT_EQMT_LOGIN_V] WHERE SHIFTFLAG = 'CURR'
CREATE VIEW [mor].[CONOPS_MOR_OPERATOR_SUPPORT_EQMT_LOGIN_V]
AS
...
WITH OperatorDetail AS (
    SELECT [shiftflag]
        ...
        ,[OperatorImageURL]
    FROM [mor].[CONOPS_MOR_EQMT_OTHER_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
