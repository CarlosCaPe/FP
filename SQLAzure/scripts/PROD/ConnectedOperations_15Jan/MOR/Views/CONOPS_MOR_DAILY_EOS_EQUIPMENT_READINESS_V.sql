CREATE VIEW [MOR].[CONOPS_MOR_DAILY_EOS_EQUIPMENT_READINESS_V] AS

--select * from [MOR].[CONOPS_MOR_DAILY_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [mor].[CONOPS_MOR_DAILY_EOS_EQUIPMENT_READINESS_V]
AS
...
WHERE PlanName = FORMAT(GETDATE(), 'yyyy MMMM')
)
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			FROM MOR.EQUIPMENT_HOURLY_STATUS WITH(NOLOCK)
			WHERE
		...
		INNER JOIN MOR.CONOPS_MOR_EOS_SHIFT_INFO_V s
