CREATE VIEW [MOR].[CONOPS_MOR_DAILY_EOS_TONS_MINED_V] AS

--SELECT * FROM [MOR].[CONOPS_MOR_DAILY_EOS_TONS_MINED_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [mor].[CONOPS_MOR_DAILY_EOS_TONS_MINED_V]
AS
...
FROM(
	...
	FROM [MOR].[CONOPS_MOR_DAILY_EOS_TOTAL_MATERIAL_V] ov
	LEFT JOIN [MOR].[CONOPS_MOR_SHIFT_BASE_TARGET_V] t
...
SELECT
	...
	CASE
		...
		ELSE 'Exceeds Plan'
	END AS Status
FROM [MOR].[CONOPS_MOR_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V]
CROSS APPLY
(
VALUES
