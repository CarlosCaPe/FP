...
* {21 Nov 2025}		{ggosal1}		{Change SUM of Grades to AVG} 
*******************************************************************/ 
...
BEGIN TRY
	...
	BEGIN
		...
		SELECT UPPER([CrusherLoc]) AS [CrusherLoc]
			  ,ROUND(AVG([TCU_PCT]),3) AS TcuPct
			  ,ROUND(AVG([TMO_PCT]),3) AS TmoPct
			  ,ROUND(AVG([TCLAY_PCT]),3) AS TClayPct
			  ,ROUND(AVG([XCU_PCT]),3) AS XcuPct
			  ,ROUND(AVG([KAOLINITE_PCT]),3) AS KaolinitePct
			  ,ROUND(AVG([ASCU_PCT]),3) AS AscuPct
			  ,ROUND(AVG([SWELLING_CLAY_PCT]),3) AS SwellingClayPct
			  ,ROUND(AVG([SDR_P80]),3) AS SdrP80
