...
/******************************************************************  
* PROCEDURE	: dbo.DrillAndBlast_Spare_Get
* PURPOSE	: 
* NOTES		: 
* CREATED	: jrodulfa, 14 Feb 2023
* SAMPLE	: 
	1. EXEC dbo.DrillAndBlast_Spare_Get 'CURR', 'MOR', NULL, NULL, NULL
...
*******************************************************************/ 
CREATE PROCEDURE [dbo].[DrillAndBlast_Spare_Get] 
(	
	@SHIFT VARCHAR(4),
	@SITE VARCHAR(4),
	@STATUS NVARCHAR(MAX),
...
BEGIN TRY
	...
	BEGIN
		...
		FROM [BAG].[CONOPS_BAG_DB_SPARE_V] (NOLOCK)
	...
	BEGIN
		...
		SELECT TOP 5
			...
			CAST(reasonidx AS VARCHAR(10)) AS ReasonId,
			SUM(duration) As TimeInMinutes
		FROM [CER].[CONOPS_CER_DB_SPARE_V] (NOLOCK)
		WHERE shiftflag = @SHIFT
			AND (eqmt IN (SELECT [Value] FROM @splitEqmt) OR @EQMT IS NULL)
			AND (eqmtcurrstatus IN (SELECT [Value] FROM @splitEStat) OR @STATUS IS NULL)
			AND (eqmttype IN (SELECT [Value] FROM @splitEType) OR @EQMTTYPE IS NULL)
	...
	BEGIN
		...
		SELECT TOP 5
			...
			CAST(reasonidx AS VARCHAR(10)) AS ReasonId,
			SUM(duration) As TimeInMinutes
		FROM [CHI].[CONOPS_CHI_DB_SPARE_V] (NOLOCK)
		WHERE shiftflag = @SHIFT
			AND (eqmt IN (SELECT [Value] FROM @splitEqmt) OR @EQMT IS NULL)
			AND (eqmtcurrstatus IN (SELECT [Value] FROM @splitEStat) OR @STATUS IS NULL)
			AND (eqmttype IN (SELECT [Value] FROM @splitEType) OR @EQMTTYPE IS NULL)
	...
	BEGIN
		...
		SELECT TOP 5
			...
			CAST(reasonidx AS VARCHAR(10)) AS ReasonId,
			SUM(duration) As TimeInMinutes
		FROM [CLI].[CONOPS_CLI_DB_SPARE_V] (NOLOCK)
		WHERE shiftflag = @SHIFT
			AND (eqmt IN (SELECT [Value] FROM @splitEqmt) OR @EQMT IS NULL)
			AND (eqmtcurrstatus IN (SELECT [Value] FROM @splitEStat) OR @STATUS IS NULL)
			AND (eqmttype IN (SELECT [Value] FROM @splitEType) OR @EQMTTYPE IS NULL)
	...
	BEGIN
		...
		SELECT TOP 5
			...
			CAST(reasonidx AS VARCHAR(10)) AS ReasonId,
			SUM(duration) As TimeInMinutes
		FROM [MOR].[CONOPS_MOR_DB_SPARE_V] (NOLOCK)
		WHERE shiftflag = @SHIFT
			AND (eqmt IN (SELECT [Value] FROM @splitEqmt) OR @EQMT IS NULL)
