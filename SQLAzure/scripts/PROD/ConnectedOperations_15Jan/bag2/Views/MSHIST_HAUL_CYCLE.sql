...
(
    ...
    WHERE   C.ECF_CLASS_ID IN ( 'XAEntity.Cycle.ProductionCycle.TruckCycle', 'XAEntity.Cycle.ProductionCycle.LoaderCycle')
        AND 
            C.ENDTIME_UTC >=  DATEADD(DAY,-2, GETUTCDATE())
),      starttime_utc 
AS 
(
...
(
    ...
    INNER JOIN 
            [BAG_MSHIST].[dbo].[CYCLE] C WITH (NOLOCK)
        ON
            cF.[CYCLE_OID] = C.[CYCLE_OID]
