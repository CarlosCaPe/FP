CREATE VIEW [ABR].[CONOPS_ABR_EOS_SHIFT_INFO_V] AS




--select * from [abr].[CONOPS_ABR_EOS_SHIFT_INFO_V] order by shiftflag  
  
CREATE VIEW [ABR].[CONOPS_ABR_EOS_SHIFT_INFO_V]   
AS  
...
WHERE name = 'Pacific SA Standard Time'
)
...
(CASE WHEN right(a.shiftid,1) = 1 THEN concat(cast(left(a.shiftid,6) as date),' 08:00:00.000')  
ELSE concat(cast(left(a.shiftid,6) as date),' 20:00:00.000') END)  
ELSE b.ShiftStartDateTime END AS ShiftStartDateTime,  
  
  
CASE WHEN b.ShiftEndDateTime IS NULL THEN   
(CASE WHEN right(a.shiftid,1) = 1 THEN concat(cast(left(a.shiftid,6) as date),' 20:00:00.000')  
ELSE concat(dateadd(day,1,cast(left(a.shiftid,6) as date)),' 08:00:00.000') END)  
...
FROM [abr].[shift_info] WITH (NOLOCK)  
GROUP BY siteflag  
...
FROM [abr].[shift_info] WITH (nolock)) b  
