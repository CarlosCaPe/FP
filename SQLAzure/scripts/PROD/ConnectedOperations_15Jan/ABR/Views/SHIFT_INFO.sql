CREATE VIEW [ABR].[SHIFT_INFO] AS

--SELECT * FROM ABR.SHIFT_INFO
CREATE VIEW [ABR].[SHIFT_INFO]
AS
...
FROM [SCMAOperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [SCMAOperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [SCMAOperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [SCMAOperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'ABR' AS SiteFlag,
...
SELECT
	...
	,ShiftDuration
	,ShiftDate
	,((DATEDIFF(DAY, '1970-01-01', ShiftStartDateTime)) * 2 + ((shiftid - 1) % 2)) AS ShiftIndex
	,TimeZoneName
	,is_currently_dst
	,current_utc_offset
