CREATE VIEW [ABR].[CONOPS_ABR_EQMT_SHOVEL_HOURLY_TPRH_V] AS

--select * from [abr].[CONOPS_ABR_EQMT_SHOVEL_HOURLY_TPRH_V] where shiftflag = 'curr'
CREATE VIEW [ABR].[CONOPS_ABR_EQMT_SHOVEL_HOURLY_TPRH_V]
AS
...
FROM(
	...
	FROM ABR.EQUIPMENT_HOURLY_STATUS (NOLOCK) AS HOS
	WHERE HOS.UNIT = '2' 
) AS sub1
...
FROM ABR.SHIFT_LOAD_DETAIL_V (NOLOCK)
