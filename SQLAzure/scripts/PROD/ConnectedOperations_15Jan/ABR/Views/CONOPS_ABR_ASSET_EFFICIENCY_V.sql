CREATE VIEW [ABR].[CONOPS_ABR_ASSET_EFFICIENCY_V] AS



--select * from [ABR].[CONOPS_ABR_ASSET_EFFICIENCY_V]
CREATE VIEW [ABR].[CONOPS_ABR_ASSET_EFFICIENCY_V] 
AS
...
SELECT shiftflag,
	   [siteflag],
	   unittype,
	   CAST(FORMAT(ROUND(ISNULL(Ops_efficient_pct, 0), 0), '#0') AS DECIMAL) [overall_efficiency],
