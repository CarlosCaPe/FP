CREATE VIEW [ABR].[SHIFT_LOAD_DETAIL_V] AS

--SELECT * FROM ABR.SHIFT_LOAD_DETAIL_V
CREATE VIEW [ABR].[SHIFT_LOAD_DETAIL_V]
AS
...
FROM abr.shift_load sl WITH(NOLOCK)
LEFT JOIN abr.SHIFT_INFO si WITH(NOLOCK)
	ON sl.ShiftId = si.ShiftId
LEFT JOIN abr.shift_eqmt eqe WITH(NOLOCK)
	ON sl.FieldExcav = eqe.Id
LEFT JOIN abr.shift_eqmt eqt WITH(NOLOCK)
	ON sl.FieldTruck = eqt.Id
LEFT JOIN abr.SHIFT_OPER oe WITH(NOLOCK)
	ON sl.FieldEoper = oe.Id
LEFT JOIN abr.SHIFT_OPER ot WITH(NOLOCK)
	ON sl.FieldToper = ot.Id
LEFT JOIN abr.enum exeqt WITH(NOLOCK)
	ON eqe.FieldEqmttype = exeqt.Id
LEFT JOIN abr.enum teqt
	ON eqt.FieldEqmttype = teqt.Id
LEFT JOIN abr.shift_grade sg WITH(NOLOCK)
	ON sl.FieldGrade = sg.Id
LEFT JOIN abr.SHIFT_LOC slo WITH(NOLOCK)
