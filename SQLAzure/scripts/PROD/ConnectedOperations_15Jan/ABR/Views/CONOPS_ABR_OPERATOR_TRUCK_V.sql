CREATE VIEW [ABR].[CONOPS_ABR_OPERATOR_TRUCK_V] AS
...
-- SELECT * FROM [abr].[CONOPS_ABR_OPERATOR_TRUCK_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_OPERATOR_TRUCK_V] 
AS
...
LEFT JOIN [ABR].[CONOPS_ABR_TRUCK_SHIFT_OVERVIEW_V] c
...
LEFT JOIN [abr].[CONOPS_ABR_OPERATOR_TRUCK_LIST_V] [tl]
ON [shift].shiftflag = [tl].shiftflag
LEFT JOIN [abr].[CONOPS_ABR_TRUCK_POPUP] [pop]
ON [pop].shiftflag = [tl].shiftflag
AND [pop].TruckID = [tl].truckid
LEFT JOIN [abr].[CONOPS_ABR_TRUCK_TPRH] [tp]
...
LEFT JOIN [ABR].[CONOPS_ABR_OPERATOR_EQMT_READY_HOURS_V] [rh]
