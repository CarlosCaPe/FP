CREATE VIEW [ABR].[CONOPS_ABR_EQMT_ALARM_SHOVEL_DOWN_V] AS





-- SELECT * FROM [abr].[CONOPS_ABR_EQMT_ALARM_SHOVEL_DOWN_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_EQMT_ALARM_SHOVEL_DOWN_V]
AS
	...
	LEFT JOIN [abr].[asset_efficiency] [ae] (NOLOCK)
	ON [a].SHIFTID = [ae].shiftid
	LEFT JOIN [abr].[PIT_EXCAV_C] [s] (NOLOCK)
	ON [s].FieldId = [ae].eqmt and [s].SHIFTID = [ae].shiftid
	LEFT JOIN [abr].[PIT_WORKER_C] [w] (NOLOCK)
