CREATE VIEW [ABR].[CONOPS_ABR_TP_EQMT_STATUS_GANTTCHART_V] AS


--select * from [abr].[CONOPS_ABR_TP_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'prev'
CREATE VIEW [ABR].[CONOPS_ABR_TP_EQMT_STATUS_GANTTCHART_V] 
AS
...
SELECT
	...
	x.eqmt,
	x.eqmtcurrstatus
FROM ABR.CONOPS_ABR_SHIFT_INFO_V AS si
LEFT JOIN (
	SELECT
		shiftid,
		eqmt,
...
LEFT JOIN STAT st
	...
	AND e.eqmt = st.eqmt
LEFT JOIN ABR.CONOPS_ABR_TRUCK_POPUP tp WITH(NOLOCK)
