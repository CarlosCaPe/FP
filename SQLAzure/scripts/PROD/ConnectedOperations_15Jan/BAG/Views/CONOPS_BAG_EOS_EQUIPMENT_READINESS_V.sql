CREATE VIEW [BAG].[CONOPS_BAG_EOS_EQUIPMENT_READINESS_V] AS

--select * from [BAG].[CONOPS_BAG_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [bag].[CONOPS_BAG_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		INNER JOIN BAG.CONOPS_BAG_SHIFT_INFO_V s
	...
	LEFT JOIN EqReadinessTarget eq
		ON LEFT(pvt.shiftid, 4) = eq.TargetPeriod
) a
CROSS APPLY (
	VALUES
