CREATE VIEW [CLI].[SHIFT_INFO] AS

--SELECT * FROM CLI.SHIFT_INFO
CREATE VIEW [CLI].[SHIFT_INFO]
AS
...
FROM [CMXOperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [CMXOperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [CMXOperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [CMXOperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'CMX' AS SiteFlag,
