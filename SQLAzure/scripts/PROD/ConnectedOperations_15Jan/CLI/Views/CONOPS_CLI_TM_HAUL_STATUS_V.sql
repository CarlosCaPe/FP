CREATE VIEW [CLI].[CONOPS_CLI_TM_HAUL_STATUS_V] AS



-- SELECT * FROM [cli].[CONOPS_CLI_TM_HAUL_STATUS_V] WITH (NOLOCK)
CREATE VIEW [cli].[CONOPS_CLI_TM_HAUL_STATUS_V] 
AS
	...
	ON a.SHIFTINDEX = h.SHIFTINDEX AND
	   a.SITEFLAG = 'CMX' AND h.SITE_CODE = 'CLI'
	LEFT JOIN [cli].[CONOPS_CLI_DELTA_C_DETAIL_V] [dt]
