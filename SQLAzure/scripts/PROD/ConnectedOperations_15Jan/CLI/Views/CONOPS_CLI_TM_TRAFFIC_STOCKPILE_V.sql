CREATE VIEW [CLI].[CONOPS_CLI_TM_TRAFFIC_STOCKPILE_V] AS
...
--SELECT * FROM [cli].[CONOPS_CLI_TM_TRAFFIC_STOCKPILE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'      
CREATE VIEW [cli].[CONOPS_CLI_TM_TRAFFIC_STOCKPILE_V]      
AS      
 ...
 WITH STOCKPILE AS (    
  ...
  ON rp.REASON = loc.[FieldReason]    
     AND rp.SITE_CODE = 'CLI' AND loc.siteflag = 'CMX'    
     AND rp.SHIFTINDEX = loc.SHIFTINDEX    
  LEFT JOIN [cli].ENUM e WITH (NOLOCK)    
  ON rp.STATUS = e.IDX AND e.ENUMTYPEID = 1    
  LEFT JOIN [cli].ENUM enumUnit  WITH (NOLOCK)    
     ON loc.FieldUnit = enumUnit.Id    
  WHERE enumUnit.Idx IN (3, 5)--dump and stockpile locations    
 )    
 ...
 FROM STOCKPILE [c]    
 LEFT JOIN [cli].[CONOPS_CLI_TRUCK_DETAIL_V] [t] WITH (NOLOCK)    
