CREATE VIEW [CLI].[CONOPS_CLI_CM_AVG_THROUGHPUT_V] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_CM_AVG_THROUGHPUT_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [cli].[CONOPS_CLI_CM_AVG_THROUGHPUT_V]    
AS    
...
WITH ShiftAvg AS (  
 SELECT ShiftIndex  
  ...
  --,Hr  
 FROM [cli].[CONOPS_CLI_CM_HOURLY_CRUSHER_THROUGHPUT_V] WITH (NOLOCK)  
...
FROM (   
 ...
 FROM [cli].[CONOPS_CLI_CM_HOURLY_CRUSHER_THROUGHPUT_V] [hct] WITH (NOLOCK)  
