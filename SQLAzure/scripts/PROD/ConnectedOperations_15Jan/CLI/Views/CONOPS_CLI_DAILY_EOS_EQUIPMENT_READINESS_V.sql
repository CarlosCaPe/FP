CREATE VIEW [CLI].[CONOPS_CLI_DAILY_EOS_EQUIPMENT_READINESS_V] AS

--select * from [CLI].[CONOPS_CLI_DAILY_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [cli].[CONOPS_CLI_DAILY_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		INNER JOIN CLI.CONOPS_CLI_EOS_SHIFT_INFO_V s
			ON r.site_code = 'CLI'
