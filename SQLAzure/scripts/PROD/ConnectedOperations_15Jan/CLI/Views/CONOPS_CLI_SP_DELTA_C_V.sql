CREATE VIEW [CLI].[CONOPS_CLI_SP_DELTA_C_V] AS
  
  
--SELECT * from [cli].[CONOPS_CLI_SP_DELTA_C_V] where shiftflag = 'prev' order by deltac desc  
  
CREATE VIEW [cli].[CONOPS_CLI_SP_DELTA_C_V]  
AS  
...
EmptyTravel,  
LoadedTravel  
FROM [CLI].[CONOPS_CLI_SP_DELTA_C_AVG_V]  
),  
...
stat.eqmtcurrstatus  
FROM DELTAC dc  
LEFT JOIN [CLI].[CONOPS_CLI_SHOVEL_POPUP] [pop] WITH (NOLOCK)
