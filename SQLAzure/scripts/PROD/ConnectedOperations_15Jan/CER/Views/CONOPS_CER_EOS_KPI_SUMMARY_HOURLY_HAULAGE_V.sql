CREATE VIEW [CER].[CONOPS_CER_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] AS





-- SELECT * FROM [cer].[CONOPS_CER_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [cer].[CONOPS_CER_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V]  
AS  
	...
	TonnageRaw AS (
		...
		FROM (
			SELECT Siteflag,
				   ...
				   ShiftStartDateTime,
				   ShiftEndDateTime
			FROM [cer].[CONOPS_CER_SHIFT_LINE_GRAPH_V] (NOLOCK)
		) m
	),
	...
	EFHTarget AS (
		...
		FROM [cer].[CONOPS_CER_DELTA_C_TARGET_V] ps WITH (NOLOCK)
	),
