...
FROM [cer].[asset_efficiency] ae WITH (NOLOCK)
...
SELECT 
	...
	CASE WHEN [PERSONNEL_ID] IS NULL OR [PERSONNEL_ID] = '-1' THEN NULL
		ELSE concat([img].[value], RIGHT('0000000000' + [o].PERSONNEL_ID, 10),'.jpg') END as OperatorImageURL,
	[o].OperatorName,
	[o].CREW,
	[o].PATTERN_NO,
	[em].MODEL,
...
LEFT JOIN EqmtModel em
	ON em.Shiftindex = ae.ShiftIndex
	AND em.EQMTID = ae.Equipment
