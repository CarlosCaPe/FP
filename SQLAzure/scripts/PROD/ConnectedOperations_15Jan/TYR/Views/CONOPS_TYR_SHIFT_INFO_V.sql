CREATE VIEW [TYR].[CONOPS_TYR_SHIFT_INFO_V] AS



  
--SELECT * FROM [tyr].[CONOPS_TYR_SHIFT_INFO_V]      
CREATE VIEW [TYR].[CONOPS_TYR_SHIFT_INFO_V]        
AS      
...
WHERE name = 'Mountain Standard Time'  
)  
...
FROM (      
 ...
 FROM      
   (SELECT SITEFLAG,      
    ...
    FROM [tyr].[SHIFT_INFO] WITH (NOLOCK)      
    GROUP BY SITEFLAG      
    UNION      
    SELECT SITEFLAG,      
     'CURR' AS SHIFTFLAG,      
     MAX(SHIFTID) AS SHIFTID      
    FROM [tyr].[SHIFT_INFO] (NOLOCK)      
    GROUP BY SITEFLAG      
    UNION      
    SELECT SITEFLAG,      
     'NEXT' AS SHIFTFLAG,      
