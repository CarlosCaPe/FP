CREATE VIEW [TYR].[SHIFT_INFO] AS

--SELECT * FROM TYR.SHIFT_INFO
CREATE VIEW [TYR].[SHIFT_INFO]
AS
...
FROM [TYROperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [TYROperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [TYROperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [TYROperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'TYR' AS SiteFlag,
