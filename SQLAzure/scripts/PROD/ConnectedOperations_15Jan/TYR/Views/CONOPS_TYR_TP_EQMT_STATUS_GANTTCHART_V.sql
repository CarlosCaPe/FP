CREATE VIEW [TYR].[CONOPS_TYR_TP_EQMT_STATUS_GANTTCHART_V] AS

--select * from [tyr].[CONOPS_TYR_TP_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'prev'
CREATE VIEW [TYR].[CONOPS_TYR_TP_EQMT_STATUS_GANTTCHART_V]
AS
...
SELECT
	...
	x.eqmt,
	x.eqmtcurrstatus
FROM TYR.CONOPS_TYR_SHIFT_INFO_V AS si
LEFT JOIN (
	SELECT
		shiftid,
		eqmt,
...
LEFT JOIN STAT st
	...
	AND e.eqmt = st.eqmt
LEFT JOIN TYR.CONOPS_TYR_TRUCK_POPUP tp WITH(NOLOCK)
