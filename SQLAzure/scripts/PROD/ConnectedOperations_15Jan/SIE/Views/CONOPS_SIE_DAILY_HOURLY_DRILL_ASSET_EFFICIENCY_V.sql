CREATE VIEW [SIE].[CONOPS_SIE_DAILY_HOURLY_DRILL_ASSET_EFFICIENCY_V] AS
  
  
  
--select * from [sie].[CONOPS_SIE_DAILY_HOURLY_DRILL_ASSET_EFFICIENCY_V] WITH (NOLOCK)  
CREATE VIEW [sie].[CONOPS_SIE_DAILY_HOURLY_DRILL_ASSET_EFFICIENCY_V]  
AS  
  
WITH TimesDetails AS (  
 SELECT SHIFTINDEX,  
     CASE [eqmttype] WHEN 'ADS PV351' THEN 'DR' + LEFT(EQMT, 2)  
       ELSE EQMT  
     END AS Equipment,  
     eqmttype,  
     HOS + 1 as Hr,  
 ...
 GROUP BY SHIFTINDEX, EQMT, HOS, eqmttype  
),  
