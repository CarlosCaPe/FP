CREATE VIEW [SIE].[CONOPS_SIE_DAILY_EOS_EQUIPMENT_READINESS_V] AS

--select * from [SIE].[CONOPS_SIE_DAILY_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [sie].[CONOPS_SIE_DAILY_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			FROM SIE.EQUIPMENT_HOURLY_STATUS WITH(NOLOCK)
			WHERE
		...
		INNER JOIN SIE.CONOPS_SIE_EOS_SHIFT_INFO_V s
