CREATE VIEW [SIE].[SHIFT_INFO] AS

--SELECT * FROM SIE.SHIFT_INFO
CREATE VIEW [SIE].[SHIFT_INFO]
AS
...
FROM [SIEOperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [SIEOperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [SIEOperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [SIEOperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'SIE' AS SiteFlag,
