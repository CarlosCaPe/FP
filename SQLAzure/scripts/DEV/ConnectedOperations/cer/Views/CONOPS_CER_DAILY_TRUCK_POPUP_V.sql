CREATE VIEW [cer].[CONOPS_CER_DAILY_TRUCK_POPUP_V] AS



-- SELECT * FROM [cer].[CONOPS_CER_DAILY_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'  
CREATE VIEW [cer].[CONOPS_CER_DAILY_TRUCK_POPUP_V]  
AS  
...
AE AS (
	...
	FROM [cer].[CONOPS_CER_DAILY_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V] WITH (NOLOCK)
),
...
SELECT DISTINCT [t].shiftflag
	...
	,[t].[Location]
FROM [cer].[CONOPS_CER_DAILY_TRUCK_DETAIL_V] [t] WITH (NOLOCK)
LEFT JOIN [cer].[CONOPS_CER_DAILY_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK) 
...
LEFT JOIN [cer].[CONOPS_CER_TP_DELTA_C_AVG_V] [dc] WITH (NOLOCK) 
