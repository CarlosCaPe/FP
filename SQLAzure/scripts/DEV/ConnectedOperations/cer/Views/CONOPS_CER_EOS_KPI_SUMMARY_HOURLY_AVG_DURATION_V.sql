CREATE VIEW [cer].[CONOPS_CER_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [cer].[CONOPS_CER_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]  
AS  
...
StatusEvent AS (
	...
	FROM (
		...
		FROM [cer].[CONOPS_CER_SHIFT_INFO_V] [si] WITH (NOLOCK)
