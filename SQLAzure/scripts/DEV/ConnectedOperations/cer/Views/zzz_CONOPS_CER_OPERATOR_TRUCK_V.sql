CREATE VIEW [cer].[zzz_CONOPS_CER_OPERATOR_TRUCK_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [cer].[zzz_CONOPS_CER_OPERATOR_TRUCK_V] 
AS
...
WITH CTE AS (
	...
	FROM [cer].[CONOPS_CER_TRUCK_DETAIL_V] [td]
	LEFT JOIN [cer].[CONOPS_CER_SHIFT_OVERVIEW_V] [so]
...
FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_LIST_V] [tl]
	LEFT JOIN [cer].[CONOPS_CER_SHIFT_INFO_V] [shift]
	ON [tl].[shiftflag] = [shift].[shiftflag]
LEFT JOIN [cer].[CONOPS_CER_TRUCK_POPUP_V] [pop]
