CREATE VIEW [cer].[ZZZ_CONOPS_CER_HOURLY_DRILL_ASSET_EFFICIENCY_V_OLD] AS


--select * from [cer].[CONOPS_CER_HOURLY_DRILL_ASSET_EFFICIENCY_V_OLD] WITH (NOLOCK)
CREATE VIEW [cer].[CONOPS_CER_HOURLY_DRILL_ASSET_EFFICIENCY_V_OLD]
AS
...
WITH TimesDetails AS (
	...
	FROM(
		...
		FROM [cer].[DRILL_UTILIZATION] [HOS] WITH (NOLOCK)
	...
	GROUP BY SHIFTINDEX, SITE_CODE, EQMT, HOS
)
