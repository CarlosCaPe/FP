CREATE VIEW [cer].[CONOPS_CER_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV'
CREATE VIEW [cer].[CONOPS_CER_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] 
AS
...
    FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_DELAY_V] WITH (NOLOCK)
	UNION ALL
	SELECT [Shiftflag]
		,[Siteflag]
		,[TruckId]
...
    FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_RAMP_EVENT_V] WITH (NOLOCK)
	UNION ALL
	SELECT [shiftflag]
		,[siteflag]
		,[eqmtid] AS [TruckId]
...
    FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_ALERTS_V] WITH (NOLOCK)
)
...
SELECT [op].shiftflag
	...
	,[oa].GeneratedDate
FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_V] [op] WITH (NOLOCK) 
