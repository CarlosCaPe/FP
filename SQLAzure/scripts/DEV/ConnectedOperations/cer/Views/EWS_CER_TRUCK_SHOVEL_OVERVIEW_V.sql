CREATE VIEW [cer].[EWS_CER_TRUCK_SHOVEL_OVERVIEW_V] AS


--select * from [cer].[EWS_CER_TRUCK_SHOVEL_OVERVIEW_V] WITH(NOLOCK)
CREATE VIEW [cer].[EWS_CER_TRUCK_SHOVEL_OVERVIEW_V] 
AS
...
FROM [cer].[CONOPS_CER_TP_TONS_HAUL_V]  WITH (NOLOCK)
...
FROM [cer].[CONOPS_CER_OVERALL_AVG_PAYLOAD_V] WITH (NOLOCK)),
...
FROM [cer].[CONOPS_CER_TP_EQMT_STATUS_V] WITH (NOLOCK)
...
[eqmtcurrstatus],
count(distinct([eqmt])) countEquip
FROM [cer].[CONOPS_CER_SP_EQMT_STATUS_V] WITH (NOLOCK)
group by [siteflag],[shiftflag],[eqmtcurrstatus]
) d
PIVOT  
(  
...
FROM [cer].[CONOPS_CER_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
...
,ROUND([availability],0) AS [availability]
,ROUND([use_of_availability],0) AS use_of_availability
FROM [cer].[CONOPS_CER_SHOVEL_ASSET_EFFICIENCY_V] WITH (NOLOCK))
