CREATE VIEW [cer].[CONOPS_CER_DRILL_OPERATOR_HAS_LATE_START_V] AS


-- SELECT * FROM [cer].[CONOPS_CER_DRILL_OPERATOR_HAS_LATE_START_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [cer].[CONOPS_CER_DRILL_OPERATOR_HAS_LATE_START_V]
AS
...
SELECT [shift].shiftflag,
	   ...
	   eqmtid,
	   unit_code,
	   RIGHT('0000000000' + [PersonnelId], 10) [OperatorId],
	   CASE WHEN [OperatorId] IS NULL OR [OperatorId] = -1 THEN NULL
	   ELSE concat('https://images.services.fmi.com/publishedimages/',RIGHT('0000000000' + [OperatorId], 10),'.jpg') END as OperatorImageURL,
	   OperatorName,
	   [FirstLoginDateTime],
...
LEFT JOIN (
	...
	FROM cte [LateStart]
	LEFT JOIN [cer].[operator_personnel_map] [opm]
	ON [LateStart].OPERID = [opm].[Operator_Id]
	WHERE rn = 1
) [OperatorLateStart]
