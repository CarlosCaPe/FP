CREATE VIEW [cer].[CONOPS_CER_TM_HAUL_STATUS_V] AS


-- SELECT * FROM [cer].[CONOPS_CER_TM_HAUL_STATUS_V] WITH (NOLOCK)
CREATE VIEW [cer].[CONOPS_CER_TM_HAUL_STATUS_V] 
AS
	...
	WITH HaulStatus AS (
		...
		SELECT delta.SHIFTINDEX AS SHIFTINDEX,
			   ...
			   CASE WHEN unit = 'Stock' THEN COALESCE(AVG([dumpdelta]), 0) END AS DumpingAtStockpile,
			   CASE WHEN unit = 'Chancador' THEN COALESCE(AVG([dumpdelta]), 0) END AS DumpingAtCrusher
	...
	LEFT JOIN [cer].[CONOPS_CER_DELTA_C_DETAIL_V] [dt]
