CREATE VIEW [cer].[CONOPS_CER_MATERIAL_DELIVERED_TO_CHRUSHER_V] AS


--select * from [cer].[CONOPS_CER_MATERIAL_DELIVERED_TO_CHRUSHER_V] WITH (NOLOCK) where shiftflag = 'curr'
...
LEFT JOIN cer.shift_loc s WITH (NOLOCK) ON shift_loc_id = dumps.FieldLoc
LEFT JOIN cer.shift_eqmt SSE WITH (NOLOCK) ON SSE.shift_eqmt_id = dumps.FieldTruck AND SSE.ShiftId=dumps.ShiftId
LEFT JOIN cer.Enum enums WITH (NOLOCK) on enums.enum_Id=dumps.FieldLoad AND enums.Idx NOT IN (26,27,28,29,30)
...
LEFT JOIN [cer].[CONOPS_CER_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] c on a.shiftid = c.shiftid AND b.CrusherLoc = c.[Location]
