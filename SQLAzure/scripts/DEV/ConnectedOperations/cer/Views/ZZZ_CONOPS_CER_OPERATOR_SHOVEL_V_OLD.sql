CREATE VIEW [cer].[ZZZ_CONOPS_CER_OPERATOR_SHOVEL_V_OLD] AS




-- SELECT * FROM [CER].[CONOPS_CER_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [cer].[CONOPS_CER_OPERATOR_SHOVEL_V_OLD]
AS
...
    ,[os].[OperatorStatus]
FROM OperStatus [os]
LEFT OUTER JOIN [CER].[CONOPS_CER_EQMT_SHOVEL_V] [es]
	ON [os].ShiftFlag = [es].shiftflag
	AND [os].ShovelID = [es].shovelid
LEFT OUTER JOIN [CER].[CONOPS_CER_SHIFT_INFO_V] [shift]
