CREATE VIEW [cer].[CONOPS_CER_EQMT_SHOVEL_HOURLY_TPRH_V] AS

--select * from [cer].[CONOPS_CER_EQMT_SHOVEL_HOURLY_TPRH_V] where shiftflag = 'curr'
CREATE VIEW [cer].[CONOPS_CER_EQMT_SHOVEL_HOURLY_TPRH_V]
AS
...
FROM(
	...
	FROM CER.EQUIPMENT_HOURLY_STATUS (NOLOCK) AS HOS
	WHERE HOS.UNIT = '2' 
) AS sub1
...
FROM CER.SHIFT_LOAD_DETAIL_V (NOLOCK)
