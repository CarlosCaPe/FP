CREATE VIEW [cer].[ZZZ_CONOPS_CER_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V_OLD] AS


--select * from [cer].[CONOPS_CER_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] WITH (NOLOCK)
CREATE VIEW [cer].[CONOPS_CER_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V_OLD]
AS
	...
	ON [C2].[ShiftId] = LEFT([shift].[ShiftID], 4) AND [C2].[siteflag] = [shift].[siteflag]
	WHERE [C2].[siteflag] = 'CER'
