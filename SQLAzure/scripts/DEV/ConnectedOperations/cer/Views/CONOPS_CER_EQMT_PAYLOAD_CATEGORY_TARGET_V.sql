CREATE VIEW [cer].[CONOPS_CER_EQMT_PAYLOAD_CATEGORY_TARGET_V] AS
  
 
  
-- SELECT * FROM [cer].[CONOPS_CER_EQMT_PAYLOAD_CATEGORY_TARGET_V] WHERE shiftindex = '38916'  
CREATE VIEW [cer].[CONOPS_CER_EQMT_PAYLOAD_CATEGORY_TARGET_V]  
AS  
...
left join [cer].enum enum WITH (NOLOCK)on enum.enum_id = t.FieldEqmttype),  
