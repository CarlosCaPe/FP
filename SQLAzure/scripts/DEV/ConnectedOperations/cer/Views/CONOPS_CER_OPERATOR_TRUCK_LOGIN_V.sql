CREATE VIEW [cer].[CONOPS_CER_OPERATOR_TRUCK_LOGIN_V] AS





-- SELECT * FROM [cer].[CONOPS_CER_OPERATOR_TRUCK_LOGIN_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [cer].[CONOPS_CER_OPERATOR_TRUCK_LOGIN_V] 
AS
...
    FROM [cer].[CONOPS_CER_TRUCK_DETAIL_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
