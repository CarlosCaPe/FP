CREATE VIEW [cer].[CONOPS_CER_DAILY_SHOVEL_INFO_V] AS
  
  
  
  
  
-- SELECT * FROM [cer].[CONOPS_CER_DAILY_SHOVEL_INFO_V] WITH (NOLOCK)  
CREATE VIEW [cer].[CONOPS_CER_DAILY_SHOVEL_INFO_V]  
AS  
...
LEFT JOIN (  
 SELECT [s].SHIFTINDEX,  
     ...
     [s].FieldReason  
 FROM [CER].[pit_excav_c] [s] WITH (NOLOCK)  
 LEFT JOIN [CER].[enum] [enumStats] WITH (NOLOCK)  
  ON [s].FieldStatus = [enumStats].Enum_Id  
