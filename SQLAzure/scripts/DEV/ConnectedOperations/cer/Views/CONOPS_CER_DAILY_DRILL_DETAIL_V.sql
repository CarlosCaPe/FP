CREATE VIEW [cer].[CONOPS_CER_DAILY_DRILL_DETAIL_V] AS
  
       
-- SELECT * FROM [cer].[CONOPS_CER_DAILY_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]        
CREATE VIEW [cer].[CONOPS_CER_DAILY_DRILL_DETAIL_V]        
AS        
...
SELECT DISTINCT ae.[shiftflag] ,        
 ...
 DRILLASSETEFFICIENCY AS AssetEfficiencyTarget,      
    ...
    [XY_Drill_Score],        
    [OVERALLSCORE]        
FROM [cer].[CONOPS_CER_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)        
LEFT JOIN [cer].[CONOPS_CER_DAILY_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)        
