CREATE VIEW [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS
...
-- SELECT * FROM [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]  
AS  
...
StatusEvent AS (
	...
	FROM (
		...
		FROM [sie].[CONOPS_SIE_SHIFT_INFO_V] [si] WITH (NOLOCK)
