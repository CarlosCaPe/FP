CREATE VIEW [sie].[CONOPS_SIE_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] AS
...
--select * from [sie].[CONOPS_SIE_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V]  
CREATE VIEW [sie].[CONOPS_SIE_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V]   
AS  
...
FROM (  
 select shiftid,eqmt  
  ...
  categoryidx,  
  duration,  
  case when eqmt like '%T%' THEN 'truck' ELSE 'shovel' END AS unittype  
  from [sie].[asset_efficiency] WITH (NOLOCK)) ae  
