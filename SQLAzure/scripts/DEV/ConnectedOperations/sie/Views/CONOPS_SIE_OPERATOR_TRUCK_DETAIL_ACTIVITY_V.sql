CREATE VIEW [sie].[CONOPS_SIE_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] AS
...
-- SELECT * FROM [sie].[CONOPS_SIE_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV'
CREATE VIEW [sie].[CONOPS_SIE_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] 
AS
...
    FROM [sie].[CONOPS_SIE_OPERATOR_TRUCK_ALERTS_V] WITH (NOLOCK)
)
...
SELECT [op].shiftflag
	...
	,[oa].GeneratedDate
FROM [sie].[CONOPS_SIE_OPERATOR_TRUCK_V] [op] WITH (NOLOCK) 
