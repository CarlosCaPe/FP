CREATE VIEW [sie].[CONOPS_SIE_EQMT_ALARM_SHOVEL_DOWN_V] AS


-- SELECT * FROM [sie].[CONOPS_SIE_EQMT_ALARM_SHOVEL_DOWN_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [sie].[CONOPS_SIE_EQMT_ALARM_SHOVEL_DOWN_V]
AS
	...
	LEFT JOIN [sie].[asset_efficiency] [ae] (NOLOCK)
	ON [a].SHIFTID = [ae].shiftid
	LEFT JOIN [sie].[PIT_EXCAV_C] [s] (NOLOCK)
	ON [s].FieldId = [ae].eqmt and [s].SHIFTID = [ae].shiftid
	LEFT JOIN [sie].[PIT_WORKER_C] [w] (NOLOCK)
