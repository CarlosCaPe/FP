CREATE VIEW [sie].[CONOPS_SIE_TRUCK_POPUP_V] AS

-- SELECT * FROM [sie].[CONOPS_SIE_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'  
CREATE VIEW [sie].[CONOPS_SIE_TRUCK_POPUP_V]   
AS  
...
AS (
	SELECT shiftid
		,truckid
		,LoadTarget
	FROM [sie].[CONOPS_SIE_TRUCK_DETAIL_V] truck WITH (NOLOCK)
	LEFT JOIN (
		SELECT
			--substring(replace(DateEffective,'-',''),3,4) as shiftdate,  
			substring(replace(cast(getdate() AS DATE), '-', ''), 3, 4) AS shiftdate
