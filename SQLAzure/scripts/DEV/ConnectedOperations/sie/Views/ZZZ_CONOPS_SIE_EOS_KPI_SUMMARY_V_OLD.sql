CREATE VIEW [sie].[ZZZ_CONOPS_SIE_EOS_KPI_SUMMARY_V_OLD] AS
...
-- SELECT * FROM [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_V]  
AS  
...
GetHosQuery AS (
	...
	WHERE site_code = 'SIE' and shiftindex = (SELECT ShiftIndex FROM CteShiftInfo WHERE ShiftFlag = 'CURR') 
	ORDER BY hos DESC
),
...
LEFT JOIN [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_HAULAGE_V] [ksh] WITH (NOLOCK)
