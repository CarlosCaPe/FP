CREATE VIEW [sie].[CONOPS_SIE_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] AS
...
--select * from [sie].[CONOPS_SIE_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
CREATE VIEW [sie].[CONOPS_SIE_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] 
AS
...
WITH TimesDetails AS (
	...
	FROM(
		...
		FROM [sie].[EQUIPMENT_HOURLY_STATUS] [HOS] WITH (NOLOCK)
