CREATE VIEW [sie].[CONOPS_SIE_CM_AVG_THROUGHPUT_V] AS
...
-- SELECT * FROM [sie].[CONOPS_SIE_CM_AVG_THROUGHPUT_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [sie].[CONOPS_SIE_CM_AVG_THROUGHPUT_V]    
AS    
...
WITH ShiftAvg AS (  
 SELECT ShiftIndex  
  ...
  --,Hr  
 FROM [sie].[CONOPS_SIE_CM_HOURLY_CRUSHER_THROUGHPUT_V] WITH (NOLOCK)
...
FROM (   
 ...
 FROM [sie].[CONOPS_SIE_CM_HOURLY_CRUSHER_THROUGHPUT_V] [hct] WITH (NOLOCK)  
