CREATE VIEW [sie].[ZZZ_CONOPS_SIE_OPERATOR_SHOVEL_V_OLD] AS




-- SELECT * FROM [SIE].[CONOPS_SIE_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [sie].[CONOPS_SIE_OPERATOR_SHOVEL_V_OLD]
AS
...
    ,[os].[OperatorStatus]
FROM OperStatus [os]
LEFT OUTER JOIN [SIE].[CONOPS_SIE_EQMT_SHOVEL_V] [es]
	ON [os].ShiftFlag = [es].shiftflag
	AND [os].ShovelID = [es].shovelid
LEFT OUTER JOIN [SIE].[CONOPS_SIE_SHIFT_INFO_V] [shift]
