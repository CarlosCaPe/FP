CREATE VIEW [sie].[zzz_CONOPS_SIE_OPERATOR_TRUCK_V] AS
...
CREATE VIEW [sie].[zzz_CONOPS_SIE_OPERATOR_TRUCK_V] 
AS
...
WITH CTE AS (
	...
	FROM [sie].[CONOPS_SIE_TRUCK_DETAIL_V] [td]
	LEFT JOIN [sie].[CONOPS_SIE_SHIFT_OVERVIEW_V] [so]
...
FROM [sie].[CONOPS_SIE_OPERATOR_TRUCK_LIST_V] [tl]
	LEFT JOIN [sie].[CONOPS_SIE_SHIFT_INFO_V] [shift]
	ON [tl].[shiftflag] = [shift].[shiftflag]
LEFT JOIN [sie].[CONOPS_SIE_TRUCK_POPUP_V] [pop]
