CREATE VIEW [sie].[CONOPS_SIE_DRILL_DETAIL_V] AS
  
    
-- SELECT * FROM [sie].[CONOPS_SIE_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]      
CREATE VIEW [sie].[CONOPS_SIE_DRILL_DETAIL_V]      
AS      
...
SELECT ae.[shiftflag] ,      
 ...
 DRILLASSETEFFICIENCY AS AssetEfficiencyTarget,    
    ...
    [XY_Drill_Score],      
    [OVERALLSCORE]      
FROM [sie].[CONOPS_SIE_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)      
LEFT JOIN [sie].[CONOPS_SIE_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)      
