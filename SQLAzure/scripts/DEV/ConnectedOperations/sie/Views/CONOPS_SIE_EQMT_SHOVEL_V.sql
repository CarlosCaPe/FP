CREATE VIEW [sie].[CONOPS_SIE_EQMT_SHOVEL_V] AS

  
 
-- SELECT * FROM [sie].[CONOPS_SIE_EQMT_SHOVEL_V] where shiftflag = 'prev'  
CREATE VIEW [sie].[CONOPS_SIE_EQMT_SHOVEL_V]  
AS  
...
SELECT  
 ...
 IdleTime,  
 IdleTimeTarget  
FROM [sie].[CONOPS_SIE_SHOVEL_POPUP] WITH (NOLOCK)),  
...
FROM [SIE].[CONOPS_SIE_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] ae  
CROSS JOIN [SIE].[CONOPS_SIE_EQMT_ASSET_EFFICIENCY_TARGET_V] t),  
...
-- OverLoaded,  
-- InvalidPayload  
--FROM [sie].[CONOPS_SIE_EQMT_PAYLOAD_CATEGORY_V]),  
...
--FROM [sie].[CONOPS_SIE_EQMT_PAYLOAD_CATEGORY_TARGET_V]),  
