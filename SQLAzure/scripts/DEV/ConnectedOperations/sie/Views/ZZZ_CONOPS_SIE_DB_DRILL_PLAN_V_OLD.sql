CREATE VIEW [sie].[ZZZ_CONOPS_SIE_DB_DRILL_PLAN_V_OLD] AS
...
--select * from [sie].[CONOPS_SIE_DB_DRILL_PLAN_V] where shiftflag = 'prev'
CREATE VIEW [sie].[CONOPS_SIE_DB_DRILL_PLAN_V]
AS
...
REPLACE (op.FIRST_LAST_NAME,'.',' ') AS OperatorName,
...
FROM [sie].[CONOPS_SIE_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] ),

TGT AS (
SELECT TOP 1
FEETDRILLEDTARGET AS FeetShiftTarget,
...
LEFT JOIN EqmtStatus d ON a.ShiftIndex = d.ShiftIndex AND b.Eqmt = d.eqmt AND a.siteflag = 'SIE' and d.num = 1
