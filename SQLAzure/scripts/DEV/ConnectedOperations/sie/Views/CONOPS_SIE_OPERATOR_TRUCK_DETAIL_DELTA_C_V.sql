CREATE VIEW [sie].[CONOPS_SIE_OPERATOR_TRUCK_DETAIL_DELTA_C_V] AS
...
-- SELECT * FROM [sie].[CONOPS_SIE_OPERATOR_TRUCK_DETAIL_DELTA_C_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'      
CREATE VIEW [sie].[CONOPS_SIE_OPERATOR_TRUCK_DETAIL_DELTA_C_V]       
AS      
...
LEFT JOIN [SIE].[CONOPS_SIE_TRUCK_SHIFT_OVERVIEW_V] c
...
LEFT JOIN [sie].[CONOPS_SIE_OPERATOR_TRUCK_LIST_V] [tl]    
ON [shift].shiftflag = [tl].shiftflag    
LEFT JOIN [sie].[CONOPS_SIE_TRUCK_POPUP] [pop] WITH (NOLOCK)
ON [pop].shiftflag = [tl].shiftflag    
AND [pop].TruckID = [tl].truckid    
LEFT JOIN [sie].[CONOPS_SIE_TRUCK_TPRH] [tp]    
