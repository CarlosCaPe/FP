CREATE VIEW [sie].[CONOPS_SIE_DAILY_DRILL_DETAIL_V] AS
  
    
      
-- SELECT * FROM [sie].[CONOPS_SIE_DAILY_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]        
CREATE VIEW [sie].[CONOPS_SIE_DAILY_DRILL_DETAIL_V]        
AS        
...
OperatorDetail AS (        
 SELECT [ds].SHIFTINDEX,        
        [ds].[SITE_CODE],        
     'DR' + RIGHT(REPLACE(DRILL_ID, ' ',''), 2)  AS DRILL_ID,        
     [ds].PATTERN_NO,        
     OPERATORID,        
     [w].CREW,        
 ...
 WHERE DRILL_ID IS NOT NULL AND [ds].[SITE_CODE] = 'SIE'        
)        
...
SELECT DISTINCT ae.[shiftflag] ,        
 ...
 DRILLASSETEFFICIENCY AS AssetEfficiencyTarget,      
    ...
    [XY_Drill_Score],        
    [OVERALLSCORE]        
FROM [sie].[CONOPS_SIE_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)        
LEFT JOIN [sie].[CONOPS_SIE_DAILY_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)        
ON [ae].shiftid = [ds].shiftid AND [ae].siteflag = [ds].siteflag        
AND [ds].DRILL_ID = [ae].Equipment        
LEFT JOIN [sie].[CONOPS_SIE_DB_DRILL_SCORE_TARGET_V] [t] WITH (NOLOCK)        
...
ON [o].DRILL_ID = [ae].Equipment and [o].num = 1        
   AND [o].SHIFTINDEX = [ae].ShiftIndex AND [ae].siteflag = [o].[