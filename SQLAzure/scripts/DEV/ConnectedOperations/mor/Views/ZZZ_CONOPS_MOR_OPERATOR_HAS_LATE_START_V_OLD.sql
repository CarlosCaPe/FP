CREATE VIEW [mor].[ZZZ_CONOPS_MOR_OPERATOR_HAS_LATE_START_V_OLD] AS





-- SELECT * FROM [mor].[CONOPS_MOR_OPERATOR_HAS_LATE_START_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [mor].[CONOPS_MOR_OPERATOR_HAS_LATE_START_V_OLD] 
AS
...
WITH cte AS (
  ...
  WHERE trim(oper.operid) not in ('mmsunk', '')
        ...
        AND oper.logintime > 1800 AND oper.logintime < 7200 --Only looks until 2nd hour in the Shift
        AND trim(eroot.site_code) = 'MOR'
