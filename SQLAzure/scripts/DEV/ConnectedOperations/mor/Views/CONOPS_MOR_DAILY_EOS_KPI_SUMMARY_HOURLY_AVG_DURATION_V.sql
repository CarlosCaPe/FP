CREATE VIEW [mor].[CONOPS_MOR_DAILY_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS
...
-- SELECT * FROM [mor].[CONOPS_MOR_DAILY_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'    
CREATE VIEW [mor].[CONOPS_MOR_DAILY_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]    
AS    
...
StatusEvent AS (  
 ...
 FROM (  
  ...
  FROM [mor].[CONOPS_MOR_EOS_SHIFT_INFO_V] [si] WITH (NOLOCK)  
