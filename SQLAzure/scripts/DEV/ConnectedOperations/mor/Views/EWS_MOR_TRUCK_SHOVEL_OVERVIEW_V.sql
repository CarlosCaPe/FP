CREATE VIEW [mor].[EWS_MOR_TRUCK_SHOVEL_OVERVIEW_V] AS


--select * from [mor].[EWS_TRUCK_SHOVEL_OVERVIEW_V] WITH(NOLOCK)
CREATE VIEW [mor].[EWS_MOR_TRUCK_SHOVEL_OVERVIEW_V] 
AS
...
FROM [mor].[CONOPS_MOR_TP_TONS_HAUL_V]  WITH (NOLOCK)
...
FROM [mor].[CONOPS_MOR_OVERALL_AVG_PAYLOAD_V] WITH (NOLOCK)),
...
FROM [MOR].[CONOPS_MOR_TP_EQMT_STATUS_V] WITH (NOLOCK)
...
[eqmtcurrstatus],
count(distinct([eqmt])) countEquip
FROM [mor].[CONOPS_MOR_SP_EQMT_STATUS_V] WITH (NOLOCK)
group by [siteflag],[shiftflag],[eqmtcurrstatus]
) d
PIVOT  
(  
...
FROM [mor].[CONOPS_MOR_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
...
,ROUND([availability],0) AS [availability]
,ROUND([use_of_availability],0) AS use_of_availability
FROM [mor].[CONOPS_MOR_SHOVEL_ASSET_EFFICIENCY_V] WITH (NOLOCK))
