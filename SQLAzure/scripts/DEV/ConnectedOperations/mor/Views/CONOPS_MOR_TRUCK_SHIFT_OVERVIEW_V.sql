CREATE VIEW [mor].[CONOPS_MOR_TRUCK_SHIFT_OVERVIEW_V] AS



CREATE VIEW [MOR].[CONOPS_MOR_TRUCK_SHIFT_OVERVIEW_V]
AS
...
AS ( SELECT   
   ...
   FROM     mor.shift_dump_v sd WITH (NOLOCK)
            ...
            LEFT JOIN mor.shift_eqmt s WITH (NOLOCK) ON s.Id = sd.FieldTruck
   ...
   GROUP BY sd.shiftid,
    ...
    TruckByRegion
      AS ( SELECT   pe.FieldId AS [TruckId]
   FROM     mor.pit_truck pe WITH (NOLOCK)
...
LEFT JOIN ShiftDumpPerTruck sdps ON sdps.Tr