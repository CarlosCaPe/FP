CREATE VIEW [mor].[CONOPS_MOR_EOS_DRILL_READY_V] AS
  
  
  
--SELECT * FROM [mor].[CONOPS_MOR_EOS_DRILL_READY_V] WHERE shiftflag = 'curr' order by datetime  
CREATE VIEW [mor].[CONOPS_MOR_EOS_DRILL_READY_V]  
AS  
...
drill_id as eqmt,  
StartDateTime  
FROM [mor].drill_asset_efficiency_v  
WHERE reasonidx = 200),  
