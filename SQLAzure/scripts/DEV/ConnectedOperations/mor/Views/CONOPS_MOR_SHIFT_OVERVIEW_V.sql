...
ShiftDumpPerShovel AS (
    ...
    FROM ShiftDump sd
    GROUP BY sd.shiftid, sd.ShovelId, sd.[Description], sd.[Location], sd.LoadLoc
),
...
SELECT  
    ...
    (ISNULL(SUM(sdps.TotalMillOreMined), 0) + ISNULL(SUM(sdps.TotalROMLeachMined), 0) + ISNULL(SUM(sdps.TotalCrushedLeachMined), 0) + ISNULL(SUM(sdps.TotalWasteMined), 0) + ISNULL(SUM(sdps.TotalTransferOre), 0)) AS [TotalMineralsMined], --TotalMaterialMoved
    (ISNULL(SUM(sdps.TotalMillOreMined), 0) + ISNULL(SUM(sdps.TotalROMLeachMined), 0) + ISNULL(SUM(sdps.TotalCrushedLeachMined), 0) + ISNULL(SUM(sdps.TotalWasteMined), 0)) AS [TotalMaterialMined], --TotalMaterialMined
