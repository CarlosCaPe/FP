CREATE VIEW [mor].[CONOPS_MOR_EQMT_ALARM_SHOVEL_DOWN_V] AS


-- SELECT * FROM [mor].[CONOPS_MOR_EQMT_ALARM_SHOVEL_DOWN_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [mor].[CONOPS_MOR_EQMT_ALARM_SHOVEL_DOWN_V]
AS
	...
	LEFT JOIN [mor].[asset_efficiency] [ae] (NOLOCK)
	ON [a].SHIFTID = [ae].shiftid
	LEFT JOIN [mor].[PIT_EXCAV_C] [s] (NOLOCK)
	ON [s].FieldId = [ae].eqmt and [s].SHIFTID = [ae].shiftid
	LEFT JOIN [mor].[PIT_WORKER_C] [w] (NOLOCK)
