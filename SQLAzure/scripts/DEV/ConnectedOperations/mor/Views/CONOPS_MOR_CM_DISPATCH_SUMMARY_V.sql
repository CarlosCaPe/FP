...
AS  
	...
	MaxArrive AS (
		...
		FROM (
			...
			WHERE [SITE_CODE] = 'MOR'
				  AND LOC IN ('C2MIL','C2MFL','849-MFL', 'C3MIL','C3MFL','859-MILL')
		) m
	...
	RawDump AS (
		...
		WHERE d.[SITE_CODE] = 'MOR'
			  AND d.LOC IN ('C2MIL','C2MFL','849-MFL', 'C3MIL','C3MFL','859-MILL')
	),
	...
	NrOfLoad AS (
		...
		FROM (
			SELECT  shiftindex,
					site_code,
					HOS,
					CASE WHEN DUMPASN IN ('C2MIL', 'C2MFL', '849-MFL') THEN 'Crusher 2'
						WHEN DUMPASN IN ('C3MIL', 'C3MIL', '859-MILL') THEN 'Crusher 3'
					END CrusherLoc,
					excav
			FROM dbo.lh_load WITH (nolock)
			WHERE site_code = 'MOR'
				  AND DUMPASN IN ('C2MIL','C2MFL','849-MFL', 'C3MIL','C3MFL','859-MILL')
		) m
