CREATE VIEW [mor].[CONOPS_MOR_SP_AVG_LOAD_TIME_V] AS

  
  
--SELECT * FROM [mor].[CONOPS_MOR_SP_AVG_LOAD_TIME_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [mor].[CONOPS_MOR_SP_AVG_LOAD_TIME_V]  
AS  
...
pop.PayloadTarget,  
nl.LoadTime,  
CASE WHEN pop.eqmttype = 'P&H 4100A' THEN 1.4
	WHEN pop.eqmttype = 'CAT 994' THEN NULL
	ELSE 1.1 END AS LoadTimeTarget, 
CASE WHEN pop.eqmttype = 'P&H 4100A' THEN 1.4
	WHEN pop.eqmttype = 'CAT 994' THEN NULL
	ELSE 1.1 END AS LoadTimeShiftTarget, 
pop.NumberOfLoads,  
