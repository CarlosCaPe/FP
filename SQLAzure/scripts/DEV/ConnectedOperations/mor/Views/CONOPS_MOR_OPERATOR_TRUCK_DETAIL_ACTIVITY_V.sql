CREATE VIEW [mor].[CONOPS_MOR_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] AS
...
-- SELECT * FROM [mor].[CONOPS_MOR_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV' OperatorId = '60030149'
CREATE VIEW [mor].[CONOPS_MOR_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] 
AS
...
    FROM [mor].[CONOPS_MOR_OPERATOR_TRUCK_RAMP_EVENT_V] WITH (NOLOCK)
	UNION ALL
	SELECT [shiftflag]
		,[siteflag]
		,[eqmtid] AS [TruckId]
...
    FROM [mor].[CONOPS_MOR_OPERATOR_TRUCK_ALERTS_V] WITH (NOLOCK)
)
...
SELECT [op].shiftflag
	...
	,[oa].GeneratedDate
FROM [mor].[CONOPS_MOR_OPERATOR_TRUCK_V] [op] WITH (NOLOCK) 
