CREATE VIEW [mor].[CONOPS_MOR_DAILY_TRUCK_POPUP_V] AS



-- SELECT * FROM [mor].[CONOPS_MOR_DAILY_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'  
CREATE VIEW [mor].[CONOPS_MOR_DAILY_TRUCK_POPUP_V]   
AS  
...
AE AS (
	...
	FROM [mor].[CONOPS_MOR_DAILY_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V] WITH (NOLOCK)
	),

DCTarget AS (
	SELECT TOP 1 substring(replace(DateEffective, '-', ''), 3, 4) AS shiftdate
...
SELECT DISTINCT [t].shiftflag
	...
	,[t].Location
FROM [mor].[CONOPS_MOR_DAILY_TRUCK_DETAIL_V] [t] WITH (NOLOCK)
LEFT JOIN [mor].[CONOPS_MOR_DAILY_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK) 
