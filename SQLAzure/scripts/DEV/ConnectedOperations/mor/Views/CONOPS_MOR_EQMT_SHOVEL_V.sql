CREATE VIEW [mor].[CONOPS_MOR_EQMT_SHOVEL_V] AS

  
  
-- SELECT * FROM [mor].[CONOPS_MOR_EQMT_SHOVEL_V] where shiftflag = 'prev'  
CREATE VIEW [mor].[CONOPS_MOR_EQMT_SHOVEL_V]  
AS  
...
FROM [MOR].[CONOPS_MOR_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] ae  
CROSS JOIN [MOR].[CONOPS_MOR_EQMT_ASSET_EFFICIENCY_TARGET_V] t),  
...
-- OverLoaded,  
-- InvalidPayload  
--FROM [mor].[CONOPS_MOR_EQMT_PAYLOAD_CATEGORY_V]),  
...
--FROM [mor].[CONOPS_MOR_EQMT_PAYLOAD_CATEGORY_TARGET_V]),  
