CREATE VIEW [mor].[CONOPS_MOR_OPERATOR_SHOVEL_RAMP_EVENT_V] AS




-- SELECT * FROM [MOR].[CONOPS_MOR_OPERATOR_SHOVEL_RAMP_EVENT_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [MOR].[CONOPS_MOR_OPERATOR_SHOVEL_RAMP_EVENT_V] 
AS
...
    	   ea.alarm_start_time,
    	   ea.alarm_end_time
	FROM [MOR].[CONOPS_MOR_OPERATOR_SHOVEL_LIST_V] [s] WITH (NOLOCK)
