CREATE VIEW [mor].[CONOPS_MOR_SHOVEL_TO_WATCH_V] AS
  
  
--select * from [mor].[CONOPS_MOR_SHOVEL_TO_WATCH_V] where shiftflag = 'curr'  
CREATE VIEW [mor].[CONOPS_MOR_SHOVEL_TO_WATCH_V]  
AS  
...
LEFT JOIN [mor].[CONOPS_MOR_SHOVEL_POPUP] a WITH (NOLOCK) ON a.shiftid = tn.shiftid and a.ShovelID = tn.ShovelId  
