CREATE VIEW [mor].[CONOPS_MOR_EQMT_TRUCK_V] AS


  
   
-- SELECT * FROM [mor].[CONOPS_MOR_EQMT_TRUCK_V] WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [mor].[CONOPS_MOR_EQMT_TRUCK_V]    
AS    
...
LEFT JOIN [mor].[CONOPS_MOR_TRUCK_SHIFT_OVERVIEW_V] b
...
[Availability],
AvailabilityTarget
FROM [mor].[CONOPS_MOR_TRUCK_POPUP] WITH (NOLOCK)),
...
truck,
TonsHaul
FROM [mor].[CONOPS_MOR_TRUCK_TPRH])
...
LEFT JOIN MOR.CONOPS_MOR_EQMT_DELTA_C_V dc
