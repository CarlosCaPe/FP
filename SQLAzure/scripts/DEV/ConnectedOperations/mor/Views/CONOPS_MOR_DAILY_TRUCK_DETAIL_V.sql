CREATE VIEW [mor].[CONOPS_MOR_DAILY_TRUCK_DETAIL_V] AS
  
  
  
  
-- SELECT * FROM [mor].[CONOPS_MOR_DAILY_TRUCK_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'  
CREATE VIEW [mor].[CONOPS_MOR_DAILY_TRUCK_DETAIL_V]   
AS  
...
LEFT JOIN (  
 ...
 LEFT JOIN [MOR].[pit_loc] [des] WITH (NOLOCK)  
