CREATE VIEW [bag].[zzz_CONOPS_BAG_OPERATOR_TRUCK_V] AS


-- SELECT * FROM [bag].[CONOPS_BAG_OPERATOR_TRUCK_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [bag].[zzz_CONOPS_BAG_OPERATOR_TRUCK_V] 
AS
...
WITH CTE AS (
	...
	FROM [bag].[CONOPS_BAG_TRUCK_DETAIL_V] [td]
	LEFT JOIN [bag].[CONOPS_BAG_SHIFT_OVERVIEW_V] [so]
...
FROM [bag].[CONOPS_BAG_OPERATOR_TRUCK_LIST_V] [tl]
