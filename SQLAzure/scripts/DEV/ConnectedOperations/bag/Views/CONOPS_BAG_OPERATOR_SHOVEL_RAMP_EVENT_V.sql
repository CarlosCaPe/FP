CREATE VIEW [bag].[CONOPS_BAG_OPERATOR_SHOVEL_RAMP_EVENT_V] AS




-- SELECT * FROM [bag].[CONOPS_BAG_OPERATOR_SHOVEL_RAMP_EVENT_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [bag].[CONOPS_BAG_OPERATOR_SHOVEL_RAMP_EVENT_V] 
AS
...
    	   ea.alarm_start_time,
    	   ea.alarm_end_time
	FROM [bag].[CONOPS_BAG_OPERATOR_SHOVEL_LIST_V] [s] WITH (NOLOCK)
