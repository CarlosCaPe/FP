CREATE VIEW [bag].[CONOPS_BAG_SHIFT_SNAPSHOT_SEQ_V] AS


--SELECT * FROM [bag].[CONOPS_BAG_SHIFT_SNAPSHOT_SEQ_V] where shiftflag = 'curr' order by shiftseq asc

CREATE VIEW [bag].[CONOPS_BAG_SHIFT_SNAPSHOT_SEQ_V]
AS
  ...
  FROM(
   SELECT
    ...
    FROM
      ...
      LEFT JOIN [bag].[CONOPS_BAG_SHIFT_SNAPSHOT_V] snap ON shiftinfo.shiftid = snap.shiftid --ON shiftinfo.shiftflag = timeseq.[shift]
