CREATE VIEW [bag].[CONOPS_BAG_DB_EQMT_STATUS_GANTTCHART_V] AS
  
  
--select * from [bag].[CONOPS_BAG_DB_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'prev'  
CREATE VIEW [bag].[CONOPS_BAG_DB_EQMT_STATUS_GANTTCHART_V]  
AS  
...
FROM [bag].[CONOPS_BAG_EQMT_SHIFT_INFO_V] WITH (NOLOCK)  
...
reason AS reasons,  
[status]  
FROM [bag].[drill_asset_efficiency_v] (NOLOCK)  
),  
...
WHERE SITE_CODE = 'BAG'  
GROUP BY SHIFTINDEX, SITE_CODE, DRILL_ID  ) h  
)  
