CREATE VIEW [bag].[CONOPS_BAG_DAILY_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS
  
  
-- SELECT * FROM [bag].[CONOPS_BAG_DAILY_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'    
CREATE VIEW [bag].[CONOPS_BAG_DAILY_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]    
AS    
...
StatusEvent AS (  
 ...
 FROM (  
  ...
  FROM [bag].[CONOPS_BAG_EOS_SHIFT_INFO_V] [si] WITH (NOLOCK)  
