CREATE VIEW [bag].[CONOPS_BAG_EOS_SHIFT_INFO_V] AS
...
--select * from [BAG].[CONOPS_BAG_EOS_SHIFT_INFO_V]    order by shiftflag  
CREATE VIEW [BAG].[CONOPS_BAG_EOS_SHIFT_INFO_V]   
AS  
    ...
    GROUP BY siteflag  
       ...
       THEN (CASE WHEN RIGHT(max(shiftid), 1) = 2  
            THEN right(concat(replace(cast(dateadd(DAY, 1, cast(LEFT(max(shiftid), 6) AS date)) AS varchar(10)), '-', ''), '001'), 9)  
                                       ELSE right(concat(replace(cast(dateadd(DAY, 1,