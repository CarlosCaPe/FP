...
FROM cteFilter ct
INNER JOIN BAG2.CYCLE C WITH(NOLOCK)
    ON ct.CYCLE_OID = C.CYCLE_OID
LEFT JOIN BAG2.SHIFT S WITH(NOLOCK)
    ON C.ENDTIME_UTC > S.STARTTIME_UTC
    AND C.ENDTIME_UTC <= S.ENDTIME_UTC
LEFT JOIN BAG2.CYCLEACTIVITYCOMPONENT CAC WITH(NOLOCK)
...
SELECT 
    ...
    SUM(CASE WHEN CYCLENAME = 'Truck' AND cc.name = 'Loading' THEN duration ELSE 0 END) AS T_Loading,
    SUM(CASE WHEN CYCLENAME = 