CREATE VIEW [bag].[EWS_BAG_TRUCK_SHOVEL_OVERVIEW_V] AS


--select * from [bag].[EWS_TRUCK_SHOVEL_OVERVIEW_V] WITH(NOLOCK)
CREATE VIEW [bag].[EWS_BAG_TRUCK_SHOVEL_OVERVIEW_V] 
AS
...
FROM [bag].[CONOPS_BAG_TP_TONS_HAUL_V]  WITH (NOLOCK)
...
FROM [bag].[CONOPS_BAG_TP_EQMT_STATUS_V] WITH (NOLOCK)
...
[eqmtcurrstatus],
count(distinct([eqmt])) countEquip
FROM [bag].[CONOPS_BAG_SP_EQMT_STATUS_V] WITH (NOLOCK)
group by [siteflag],[shiftflag],[eqmtcurrstatus]
) d
PIVOT  
(  
...
FROM [bag].[CONOPS_BAG_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
...
,ROUND([availability],0) AS [availability]
,ROUND([use_of_availability],0) AS use_of_availability
FROM [bag].[CONOPS_BAG_SHOVEL_ASSET_EFFICIENCY_V] WITH (NOLOCK))
