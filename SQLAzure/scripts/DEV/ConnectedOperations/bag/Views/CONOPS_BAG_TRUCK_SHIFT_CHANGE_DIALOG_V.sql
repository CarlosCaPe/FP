...
FROM (
	SELECT [TruckDetail].shiftflag,
		   ...
		   dateadd(second, duration, si.SHIFTSTARTDATETIME) [ChangeDuration],
		   [se].duration / 60 [DurationMinute],
		   [TruckDetail].Region
	FROM [dbo].[status_event] [se] WITH (NOLOCK)
	LEFT JOIN [BAG].[CONOPS_BAG_TRUCK_DETAIL_V] [TruckDetail] WITH (NOLOCK)
	ON [se].eqmt = [TruckDetail].TruckID
	   AND [se].shiftindex = [TruckDetail].SHIFTINDEX
	LEFT JOIN [bag].[CONOPS_BAG_SHIFT_INFO_V] [si] WITH (NOLOCK)
	ON [si].SITEFLAG = [se].site_code AND [si].SHIFTINDEX = [se].shiftindex
	LEFT JOIN [BAG].[pit_worker] [w] WITH (NOLOCK)
	ON [w].FieldId = [se].operid
	WHERE [se].reason = 439 AND
		  [se].site_code = 'BAG' AND
		  [se].unit = 1 AND
		  [w].FieldName IS NOT NULL
) [ChangeDuration]
WHERE shiftflag IS NOT NULL 
AND [ChangeDuration].[DurationMinute] > 1
