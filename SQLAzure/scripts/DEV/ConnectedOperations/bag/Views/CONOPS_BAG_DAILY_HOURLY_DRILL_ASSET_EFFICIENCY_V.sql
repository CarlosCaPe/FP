CREATE VIEW [bag].[CONOPS_BAG_DAILY_HOURLY_DRILL_ASSET_EFFICIENCY_V] AS
  
  
  
  
--select * from [bag].[CONOPS_BAG_DAILY_HOURLY_DRILL_ASSET_EFFICIENCY_V] WITH (NOLOCK)  
CREATE VIEW [bag].[CONOPS_BAG_DAILY_HOURLY_DRILL_ASSET_EFFICIENCY_V]  
AS  
...
WITH TimesDetails AS (  
 ...
 FROM(  
  ...
  FROM [bag].[DRILL_UTILIZATION] [HOS] WITH (NOLOCK)  
