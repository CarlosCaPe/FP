CREATE VIEW [bag].[CONOPS_BAG_EQMT_SHOVEL_V] AS

  
  
-- SELECT * FROM [bag].[CONOPS_BAG_EQMT_SHOVEL_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [bag].[CONOPS_BAG_EQMT_SHOVEL_V]  
AS  
...
FROM [bag].[CONOPS_BAG_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] ae  
CROSS JOIN [BAG].[CONOPS_BAG_EQMT_ASSET_EFFICIENCY_TARGET_V] t),  
...
-- OverLoaded,  
-- InvalidPayload  
--FROM [bag].[CONOPS_BAG_EQMT_PAYLOAD_CATEGORY_V]),  
...
--FROM [bag].[CONOPS_BAG_EQMT_PAYLOAD_CATEGORY_TARGET_V]),  
