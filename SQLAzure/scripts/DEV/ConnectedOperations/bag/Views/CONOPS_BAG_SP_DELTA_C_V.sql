CREATE VIEW [bag].[CONOPS_BAG_SP_DELTA_C_V] AS
  
 
--select * from [bag].[CONOPS_BAG_SP_DELTA_C_V]  
CREATE VIEW [bag].[CONOPS_BAG_SP_DELTA_C_V]  
AS  
...
EmptyTravel,  
LoadedTravel  
FROM [BAG].[CONOPS_BAG_SP_DELTA_C_AVG_V]   
),  
...
stat.eqmtcurrstatus  
FROM DELTAC dc  
LEFT JOIN [BAG].[CONOPS_BAG_SHOVEL_POPUP] [pop] WITH (NOLOCK)
