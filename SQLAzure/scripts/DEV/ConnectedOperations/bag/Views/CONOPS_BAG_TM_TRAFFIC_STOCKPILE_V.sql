CREATE VIEW [bag].[CONOPS_BAG_TM_TRAFFIC_STOCKPILE_V] AS
...
--SELECT * FROM [bag].[CONOPS_BAG_TM_TRAFFIC_STOCKPILE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'      
CREATE VIEW [bag].[CONOPS_BAG_TM_TRAFFIC_STOCKPILE_V]      
AS      
 ...
 WITH STOCKPILE AS (    
  ...
  LEFT JOIN [bag].[PIT_LOC_C] loc WITH (NOLOCK)    
  ...
  LEFT JOIN [bag].ENUM e WITH (NOLOCK)    
  ON rp.STATUS = e.IDX AND e.ENUMTYPEID = 1    
  LEFT JOIN [bag].ENUM enumUnit  WITH (NOLOCK)    
