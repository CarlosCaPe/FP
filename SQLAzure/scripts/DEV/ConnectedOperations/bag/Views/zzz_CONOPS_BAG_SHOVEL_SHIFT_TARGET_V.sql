CREATE VIEW [bag].[zzz_CONOPS_BAG_SHOVEL_SHIFT_TARGET_V] AS




--select * from [bag].[CONOPS_BAG_SHOVEL_SHIFT_TARGET_V] where shiftflag = 'curr'
CREATE VIEW [bag].[zzz_CONOPS_BAG_SHOVEL_SHIFT_TARGET_V]
AS
...
LEFT JOIN SINFO si on a.shiftid = si.shiftid 
LEFT JOIN TGT tg on a.shiftid = tg.formatshiftid )
