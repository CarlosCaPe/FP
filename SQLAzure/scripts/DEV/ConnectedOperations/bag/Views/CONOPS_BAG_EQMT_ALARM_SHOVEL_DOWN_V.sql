CREATE VIEW [bag].[CONOPS_BAG_EQMT_ALARM_SHOVEL_DOWN_V] AS


-- SELECT * FROM [bag].[CONOPS_BAG_EQMT_ALARM_SHOVEL_DOWN_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [bag].[CONOPS_BAG_EQMT_ALARM_SHOVEL_DOWN_V]
AS
	...
	LEFT JOIN [bag].[asset_efficiency] [ae] (NOLOCK)
	ON [a].SHIFTID = [ae].shiftid
	LEFT JOIN [bag].[PIT_EXCAV_C] [s] (NOLOCK)
	ON [s].FieldId = [ae].eqmt and [s].SHIFTID = [ae].shiftid
	LEFT JOIN [bag].[PIT_WORKER_C] [w] (NOLOCK)
