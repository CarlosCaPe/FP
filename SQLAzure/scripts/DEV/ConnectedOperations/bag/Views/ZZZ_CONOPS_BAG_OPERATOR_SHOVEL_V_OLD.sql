CREATE VIEW [bag].[ZZZ_CONOPS_BAG_OPERATOR_SHOVEL_V_OLD] AS




-- SELECT * FROM [BAG].[CONOPS_BAG_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [bag].[CONOPS_BAG_OPERATOR_SHOVEL_V_OLD]
AS
...
    ,[os].[OperatorStatus]
FROM OperStatus [os]
LEFT OUTER JOIN [BAG].[CONOPS_BAG_EQMT_SHOVEL_V] [es]
	ON [os].ShiftFlag = [es].shiftflag
	AND [os].ShovelID = [es].shovelid
LEFT OUTER JOIN [BAG].[CONOPS_BAG_SHIFT_INFO_V] [shift]
