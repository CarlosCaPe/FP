CREATE VIEW [bag].[CONOPS_BAG_ALARM_DRILL_OPERATOR_BREAK_V] AS



-- SELECT * FROM [bag].[CONOPS_BAG_ALARM_DRILL_OPERATOR_BREAK_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [bag].[CONOPS_BAG_ALARM_DRILL_OPERATOR_BREAK_V]
AS
...
          	   STATUS_DURATION,
          	   OPERATORID
   		FROM [bag].[DRILL_UTILIZATION] [D] WITH (NOLOCK)
...
 			SELECT siteflag, [SHIFTINDEX], [MACHINESTATUSCODE], EQUIPMENTNUMBER, END_TIME_TS,
	...
	SELECT SHIFTINDEX,
		   ...
		   CASE Priority
				...
				WHEN 2 THEN 'Operator break exceeds 20 minu