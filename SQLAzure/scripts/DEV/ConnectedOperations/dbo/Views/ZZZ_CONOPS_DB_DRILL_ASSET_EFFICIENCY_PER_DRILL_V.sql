CREATE VIEW [dbo].[ZZZ_CONOPS_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] AS




--select * from [dbo].[CONOPS_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] WITH (NOLOCK)
CREATE VIEW [dbo].[CONOPS_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V]
AS
...
WHERE [siteflag] = 'MOR'
GROUP BY [shiftflag], [siteflag], Equipment

UNION ALL

SELECT [shiftflag],
	   a.[siteflag],
	   REPLACE(Equipment, ' ', '') AS Equipment,
	   AVG(AE) [AE],
	   AVG([Avail]) [Avail],
	   AVG([UofA]) [UofA],
...
GROUP BY [shiftflag], a.[siteflag], Equipment,
