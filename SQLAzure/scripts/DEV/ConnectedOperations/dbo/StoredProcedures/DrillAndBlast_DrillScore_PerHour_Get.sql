...
* SAMPLE	: 
	1. EXEC dbo.DrillAndBlast_DrillScore_PerHour_Get 'PREV', 'MOR', NULL, NULL, NULL
	...
	BEGIN
		WITH DrillTime AS (
				SELECT shiftflag,
					   ...
					   LEAD([START_HOLE_TS]) OVER ( PARTITION BY shiftflag, HOS ORDER BY [START_HOLE_TS] ASC ) AS [NextStartDateTime],
					   ROW_NUMBER() OVER ( PARTITION BY shiftflag, HOS ORDER BY [START_HOLE_TS] ASC ) AS [FirstHoleIndex],
					   ROW_NUMBER() OVER ( PARTITION BY shiftflag, HOS ORDER BY [START_HOLE_TS] DESC ) AS [LastHoleIndex]
				FROM [BAG].[CONOPS_BAG_DB_DRILL_SCORE_CARD_PER_HOUR_V] WITH (NOLOCK)
			...
			FROM [BAG].[CONOPS_BAG_DB_DRILL_SCORE_CARD_PER_HOUR_V] [ds] WITH (NOLOCK)
