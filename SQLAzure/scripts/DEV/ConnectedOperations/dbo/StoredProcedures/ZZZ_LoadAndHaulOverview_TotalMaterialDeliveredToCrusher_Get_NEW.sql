...
* SAMPLE	: 
	1. EXEC dbo.LoadAndHaulOverview_TotalMaterialDeliveredToCrusher_Get_NEW 'PREV', 'MOR'
...
*******************************************************************/ 
CREATE PROCEDURE [dbo].[LoadAndHaulOverview_TotalMaterialDeliveredToCrusher_Get_NEW] 
(	
	@SHIFT VARCHAR(4),
	@SITE VARCHAR(4)
)
...
+' FROM '+@SITE+'.[CONOPS_'+@SITE+'_MATERIAL_DELIVERED_TO_CHRUSHER_V] [ca] WITH (NOLOCK)'
+' LEFT JOIN '+@SITE+'.[CONOPS_'+@SITE+'_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] [ct] WITH (NOLOCK)'
+' ON [ct].shiftid = [ca].shiftid AND'
...
+' FROM '+@SITE+'.[CONOPS_'+@SITE+'_TRUCK_TO_WATCH_V] [tw] WITH (NOLOCK)'
+' LEFT JOIN '+@SITE+'.[CONOPS_'+@SITE+'_TRUCK_POPUP_V] [dialog] WITH (NOLOCK)'
