...
*******************************************************************/ 
CREATE PROCEDURE [dbo].[DrillAndBlast_DrillStatusEquipment_Get_OLD] 
(	
	@SHIFT VARCHAR(4),
	@SITE VARCHAR(4),
	@STATUS NVARCHAR(MAX),
...
BEGIN          
	...
	SELECT 
		...
		eventstart AS StartDateTime,
		eventend AS EndDateTime,
	...
	INTO
		#TempTable
	FROM [dbo].[CONOPS_DB_EQMT_STATUS_V] (NOLOCK)
	WHERE 
		shiftflag = @SHIFT
		AND siteflag = @SITE
