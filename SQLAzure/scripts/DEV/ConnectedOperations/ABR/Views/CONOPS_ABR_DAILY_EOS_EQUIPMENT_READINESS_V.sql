CREATE VIEW [ABR].[CONOPS_ABR_DAILY_EOS_EQUIPMENT_READINESS_V] AS


  
--select * from [abr].[CONOPS_ABR_DAILY_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'    
CREATE VIEW [abr].[CONOPS_ABR_DAILY_EOS_EQUIPMENT_READINESS_V]    
AS    
    
/*WITH EqReadinessTarget AS(    
 SELECT TOP 1    
  SITEFLAG,     
  AvgCountReadyTrucks AS TruckReadinessTarget,    
  ElecShovelAvgReadyNumber AS ShovelReadinessTarget,    
  ReadyDrills AS DrillReadinessTarget    
 FROM [abr].PLAN_VALUES_PROD_SUM WITH (NOLOCK)    
...
FROM(    
 ...
 FROM(    
  ...
  (    
   ...
   WHERE    
    site_code = 'ELA'
	AND unit IN (1,2,12) --trucks, shovels, drills    
    AND status <> 0    
    AND 1 = CASE WHEN unit = 2 AND eqmt like 'L%' THEN 0 ELSE 1 END --exclude loaders    
   GROUP BY site_code, shiftindex, eqmt, unit    
  ) as r    
  LEFT JOIN [abr].[CONOPS_ABR_EOS_SHIFT_INFO_V] s    
   ON r.shiftindex = s.ShiftIndex    
