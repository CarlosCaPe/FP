CREATE VIEW [ABR].[CONOPS_ABR_DRILL_DETAIL_V] AS


  
    
    
-- SELECT * FROM [abr].[CONOPS_ABR_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]    
CREATE VIEW [ABR].[CONOPS_ABR_DRILL_DETAIL_V]    
AS    
...
SELECT ae.[shiftflag] ,    
 ...
 0 AS AssetEfficiencyTarget,  
    ...
    [XY_Drill_Score],    
    [OVERALLSCORE]    
FROM [abr].[CONOPS_ABR_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)    
LEFT JOIN [abr].[CONOPS_ABR_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)    
...
CROSS JOIN [dbo].[LOOKUPS] img
WHERE img.TableCode = 'IMGURL'
