CREATE VIEW [ABR].[CONOPS_ABR_DAILY_TRUCK_POPUP_V] AS



-- SELECT * FROM [abr].[CONOPS_ABR_DAILY_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_DAILY_TRUCK_POPUP_V] 
AS
...
AE AS (
	...
	FROM [abr].[CONOPS_ABR_DAILY_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V] WITH (NOLOCK)
),
...
TruckLoad AS (
	...
	FROM [abr].[CONOPS_ABR_EQMT_TRUCK_LOAD_DELTAC_V]
	GROUP BY shiftindex
		,truck
)
...
SELECT DISTINCT [t].shiftflag
	...
	,[t].Location
FROM [abr].[CONOPS_ABR_DAILY_TRUCK_DETAIL_V] [t] WITH (NOLOCK)
LEFT JOIN [abr].[CONOPS_ABR_DAILY_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK) 
