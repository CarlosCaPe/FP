CREATE VIEW [cli].[CONOPS_CLI_DAILY_TRUCK_DETAIL_V] AS
  
  
  
  
-- SELECT * FROM [cli].[CONOPS_CLI_DAILY_TRUCK_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'  
CREATE VIEW [cli].[CONOPS_CLI_DAILY_TRUCK_DETAIL_V]   
AS  
...
LEFT JOIN (  
 ...
 LEFT JOIN [CLI].[pit_loc] [des] WITH (NOLOCK)  
