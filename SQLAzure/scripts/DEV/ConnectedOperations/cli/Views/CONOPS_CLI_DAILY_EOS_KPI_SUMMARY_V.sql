CREATE VIEW [cli].[CONOPS_CLI_DAILY_EOS_KPI_SUMMARY_V] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_DAILY_EOS_KPI_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'      
CREATE VIEW [cli].[CONOPS_CLI_DAILY_EOS_KPI_SUMMARY_V]      
AS      
...
FROM [cli].[CONOPS_CLI_DAILY_DELTA_J_V]  
WHERE DeltaJ <> 0  
