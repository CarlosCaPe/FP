CREATE VIEW [cli].[ZZZ_CONOPS_CLI_OPERATOR_SHOVEL_V_OLD] AS




-- SELECT * FROM [CLI].[CONOPS_CLI_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_SHOVEL_V_OLD]
AS
...
    ,[os].[OperatorStatus]
FROM OperStatus [os]
LEFT OUTER JOIN [CLI].[CONOPS_CLI_EQMT_SHOVEL_V] [es]
	ON [os].ShiftFlag = [es].shiftflag
	AND [os].ShovelID = [es].shovelid
LEFT OUTER JOIN [CLI].[CONOPS_CLI_SHIFT_INFO_V] [shift]
