CREATE VIEW [cli].[ZZZ_CONOPS_CLI_EOS_KPI_SUMMARY_V_OLD] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_V]  
AS  
...
GetHosQuery AS (
	...
	WHERE site_code = 'CLI' and shiftindex = (SELECT ShiftIndex FROM CteShiftInfo WHERE ShiftFlag = 'CURR') 
	ORDER BY hos DESC
),
...
LEFT JOIN [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HAULAGE_V] [ksh] WITH (NOLOCK)
