CREATE VIEW [cli].[CONOPS_CLI_EQMT_ALARM_TRUCK_SHOVEL_OPERATOR_BREAK_V] AS


-- SELECT * FROM [cli].[CONOPS_CLI_EQMT_ALARM_TRUCK_SHOVEL_OPERATOR_BREAK_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [cli].[CONOPS_CLI_EQMT_ALARM_TRUCK_SHOVEL_OPERATOR_BREAK_V]
AS
...
   		SELECT [eq].[SHIFTINDEX],
			   ...
			   [DURATION],
			   [OPERID]
   		FROM [cli].[EQUIPMENT_HOURLY_STATUS] [eq] WITH (NOLOCK)
   		WHERE [REASON] = 400
		      AND [UNIT] IN (1,2)
	),
...
 		SELECT [SHIFTINDEX], [SITE_CODE], [REASON], EQUIPMENTNUMBER, END_TIME_TS,
	...
	ON a.SHIFTINDEX = o.SHIFTINDEX
	AND a.SITEFLAG = 'CMX' AND o.SITE_CODE = 'CLI'
