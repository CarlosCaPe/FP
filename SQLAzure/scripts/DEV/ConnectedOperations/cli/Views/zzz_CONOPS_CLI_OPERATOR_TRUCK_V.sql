CREATE VIEW [cli].[zzz_CONOPS_CLI_OPERATOR_TRUCK_V] AS
...
CREATE VIEW [cli].[zzz_CONOPS_CLI_OPERATOR_TRUCK_V] 
AS
...
WITH CTE AS (
	...
	FROM [cli].[CONOPS_CLI_TRUCK_DETAIL_V] [td]
	LEFT JOIN [cli].[CONOPS_CLI_SHIFT_OVERVIEW_V] [so]
...
FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_LIST_V] [tl]
	LEFT JOIN [cli].[CONOPS_CLI_SHIFT_INFO_V] [shift]
	ON [tl].[shiftflag] = [shift].[shiftflag]
LEFT JOIN [cli].[CONOPS_CLI_TRUCK_POPUP_V] [pop]
