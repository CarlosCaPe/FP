CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_SHOVEL_V] AS
...
-- SELECT * FROM [CLI].[CONOPS_CLI_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [CLI].[CONOPS_CLI_OPERATOR_SHOVEL_V]  
AS   
...
SELECT   
 ...
 ,[es].Hangtime  
 ,[es].HangtimeTarget
FROM [CLI].[CONOPS_CLI_OPERATOR_SHOVEL_LIST_V] [os]  
LEFT JOIN [CLI].[CONOPS_CLI_SHIFT_INFO_V] [shift]   
 ON [os].shiftid = [shift].shiftid  
LEFT JOIN [CLI].[CONOPS_CLI_SHOVEL_POPUP] [es] WITH (NOLOCK)
 ON [os].ShiftFlag = [es].shiftflag  
 AND [os].ShovelID = [es].shovelid  
LEFT JOIN [CLI].[CONOPS_CLI_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] [ae]  
 ON [os].shiftid = [ae].shiftid  
 AND [os].ShovelID = [ae].eqmt  
CROSS JOIN [CLI].[CONOPS_CLI_EQMT_ASSET_EFFICIENCY_TARGET_V] [dct]  
