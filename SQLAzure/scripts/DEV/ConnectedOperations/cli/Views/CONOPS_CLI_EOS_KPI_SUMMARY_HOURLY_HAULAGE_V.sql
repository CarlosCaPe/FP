CREATE VIEW [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] AS




-- SELECT * FROM [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V]  
AS  
	...
	TonnageRaw AS (
		...
		FROM (
			SELECT Siteflag,
				   ...
				   ShiftStartDateTime,
				   ShiftEndDateTime
			FROM [cli].[CONOPS_CLI_SHIFT_LINE_GRAPH_V] (NOLOCK)
		) m
	),
	...
	EFHTarget AS (
		...
		FROM [cli].[CONOPS_CLI_SHIFT_TARGET_V] WITH (NOLOCK)
	),
