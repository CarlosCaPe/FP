CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_DRILL_LIST_V] AS



-- SELECT * FROM [cli].[CONOPS_CLI_OPERATOR_DRILL_LIST_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_DRILL_LIST_V] 
AS
	...
	WITH OperatorDetail AS (
		...
		FROM [cli].[CONOPS_CLI_DRILL_DETAIL_V] d WITH (NOLOCK)
