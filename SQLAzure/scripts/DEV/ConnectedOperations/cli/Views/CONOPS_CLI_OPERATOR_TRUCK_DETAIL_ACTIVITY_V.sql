CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV'
CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] 
AS
...
    FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_RAMP_EVENT_V] WITH (NOLOCK)
	UNION ALL
	SELECT [shiftflag]
		,[siteflag]
		,[eqmtid] AS [TruckId]
...
    FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_ALERTS_V] WITH (NOLOCK)
)
...
SELECT [op].shiftflag
	...
	,[oa].GeneratedDate
FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_V] [op] WITH (NOLOCK) 
