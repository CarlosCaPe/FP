CREATE VIEW [cli].[CONOPS_CLI_SHOVEL_TO_WATCH_V] AS
  
  
--select * from [cli].[CONOPS_CLI_SHOVEL_TO_WATCH_V] where shiftflag = 'prev'  
CREATE VIEW [cli].[CONOPS_CLI_SHOVEL_TO_WATCH_V]  
AS  
...
LEFT JOIN [cli].[CONOPS_CLI_SHOVEL_POPUP] a WITH (NOLOCK) ON tn.shiftid = a.shiftid and tn.ShovelId = a.ShovelID  
