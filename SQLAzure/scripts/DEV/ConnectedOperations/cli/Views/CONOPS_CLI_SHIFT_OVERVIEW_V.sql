CREATE VIEW [cli].[CONOPS_CLI_SHIFT_OVERVIEW_V] AS

--SELECT * FROM [cli].[CONOPS_CLI_SHIFT_OVERVIEW_V]
CREATE VIEW [cli].[CONOPS_CLI_SHIFT_OVERVIEW_V]
AS
...
ShiftDumpPerShovel AS (
    ...
    FROM     
        ShiftDump sd
    GROUP BY 
        sd.shiftid, sd.ShovelId, sd.[Location], sd.[Description], sd.[Load], sd.[Blast]
),
...
SELECT  
    ...
    ISNULL(SUM(sdps.ExpitOreToCrusher), 0) AS [ExpitOreToCrushe