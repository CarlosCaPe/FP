CREATE VIEW [cli].[EWS_CLI_TRUCK_SHOVEL_OVERVIEW_V] AS


--select * from [cli].[EWS_CLI_TRUCK_SHOVEL_OVERVIEW_V] WITH(NOLOCK)
CREATE VIEW [cli].[EWS_CLI_TRUCK_SHOVEL_OVERVIEW_V] 
AS
...
FROM [cli].[CONOPS_CLI_TP_TONS_HAUL_V]  WITH (NOLOCK)
...
FROM [cli].[CONOPS_CLI_OVERALL_AVG_PAYLOAD_V] WITH (NOLOCK)),
...
FROM [cli].[CONOPS_CLI_TP_EQMT_STATUS_V] WITH (NOLOCK)
...
[eqmtcurrstatus],
count(distinct([eqmt])) countEquip
FROM [cli].[CONOPS_CLI_SP_EQMT_STATUS_V] WITH (NOLOCK)
group by [siteflag],[shiftflag],[eqmtcurrstatus]
) d
PIVOT  
(  
...
FROM [cli].[CONOPS_CLI_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
...
,ROUND([availability],0) AS [availability]
,ROUND([use_of_availability],0) AS use_of_availability
FROM [cli].[CONOPS_CLI_SHOVEL_ASSET_EFFICIENCY_V] WITH (NOLOCK))
