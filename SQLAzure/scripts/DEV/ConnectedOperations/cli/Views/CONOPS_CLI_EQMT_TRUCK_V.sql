CREATE VIEW [cli].[CONOPS_CLI_EQMT_TRUCK_V] AS


  
    
    
-- SELECT * FROM [cli].[CONOPS_CLI_EQMT_TRUCK_V] WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [cli].[CONOPS_CLI_EQMT_TRUCK_V]    
AS    
...
LEFT JOIN [cli].[CONOPS_CLI_TRUCK_SHIFT_OVERVIEW_V] b
...
[Availability],
AvailabilityTarget
FROM [cli].[CONOPS_CLI_TRUCK_POPUP] WITH (NOLOCK)),
...
truck,
TonsHaul
FROM [cli].[CONOPS_CLI_TRUCK_TPRH])
...
LEFT JOIN CLI.CONOPS_CLI_EQMT_DELTA_C_V dc
