CREATE VIEW [cli].[CONOPS_CLI_EQMT_SHOVEL_V] AS

  
  
  
-- SELECT * FROM [cli].[CONOPS_CLI_EQMT_SHOVEL_V] where shiftflag = 'prev'  
CREATE VIEW [cli].[CONOPS_CLI_EQMT_SHOVEL_V]  
AS  
...
SELECT  
 ...
 operatorimageURL  
FROM [cli].[CONOPS_CLI_SHOVEL_INFO_V]),  
...
SELECT  
 ...
 IdleTime,  
 IdleTimeTarget  
FROM [cli].[CONOPS_CLI_SHOVEL_POPUP] WITH (NOLOCK)),  
...
FROM [CLI].[CONOPS_CLI_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] ae  
CROSS JOIN [CLI].[CONOPS_CLI_EQMT_ASSET_EFFICIENCY_TARGET_V] t),  
...
-- OverLoaded,  
-- InvalidPayload  
--FROM [cli].[CONOPS_CLI_EQMT_PAYLOAD_CATEGORY_V]),  
...
--FROM [cli].[CONOPS_CLI_EQMT_PAYLOAD_CATEGORY_TARGET_V]),  
