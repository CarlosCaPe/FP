CREATE VIEW [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HAULAGE_V] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'  
CREATE VIEW [cli].[CONOPS_CLI_EOS_KPI_SUMMARY_HAULAGE_V]  
AS  
...
FORECAST AS (
	...
	LEFT JOIN [cli].[CONOPS_CLI_SHIFT_TARGET_V] [st] WITH (NOLOCK)
...
EFHCTE AS (
	...
	FROM (
		SELECT ShiftFlag
			...
			,EFH
		FROM [cli].[CONOPS_CLI_EFH_V] WITH (NOLOCK)
	) AS efhc
	WHERE rn = 1
),
