CREATE VIEW [cli].[CONOPS_CLI_TONS_LINE_GRAPH_V] AS

--select * from [CLI].[CONOPS_CLI_TONS_LINE_GRAPH_V] where shiftflag = 'curr'
CREATE VIEW [CLI].[CONOPS_CLI_TONS_LINE_GRAPH_V]
AS
...
SELECT  
    ...
    ISNULL(SUM(sdps.TotalInpit), 0) AS [TotalInpitMined],
    ISNULL(