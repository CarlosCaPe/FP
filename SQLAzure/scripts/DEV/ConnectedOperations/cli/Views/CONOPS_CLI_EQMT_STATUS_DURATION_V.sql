CREATE VIEW [cli].[CONOPS_CLI_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [CLI].[CONOPS_CLI_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [CLI].[CONOPS_CLI_EQMT_STATUS_DURATION_V] 
AS
...
FROM [CLI].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [CLI].[CONOPS_CLI_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [CLI].[CONOPS_CLI_AUTONOMOUS_TRUCK_V] aut
