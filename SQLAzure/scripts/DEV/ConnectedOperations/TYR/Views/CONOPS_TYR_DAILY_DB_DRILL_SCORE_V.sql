CREATE VIEW [TYR].[CONOPS_TYR_DAILY_DB_DRILL_SCORE_V] AS

  
--select * from [tyr].[CONOPS_TYR_DAILY_DB_DRILL_SCORE_V] WITH (NOLOCK) where shiftflag = 'prev'  
CREATE VIEW [tyr].[CONOPS_TYR_DAILY_DB_DRILL_SCORE_V]  
AS  
...
     WHERE dt.NextStartDateTime IS NOT NULL  
 ...
 SELECT a.shiftflag,  
    ...
    ELSE [Average_Last_Drill]  
     ...
     ISNULL([Average_HoleTime], 0) AS [Average_Ho