CREATE VIEW [TYR].[CONOPS_TYR_EOS_KPI_SUMMARY_HAULAGE_V] AS


-- SELECT * FROM [tyr].[CONOPS_TYR_EOS_KPI_SUMMARY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'  
CREATE VIEW [tyr].[CONOPS_TYR_EOS_KPI_SUMMARY_HAULAGE_V]  
AS  
...
FORECAST AS (
	...
	--LEFT JOIN [tyr].[CONOPS_TYR_SHIFT_TARGET_V] [st] WITH (NOLOCK)
...
OVERVIEW AS (
	...
	FROM [tyr].[CONOPS_TYR_OVERVIEW_V] WITH (NOLOCK)
...
EFHCTE AS (
	...
	FROM (
		SELECT ShiftFlag
			...
			,EFH
		FROM [tyr].[CONOPS_TYR_EFH_V] WITH (NOLOCK)
	) AS efhc
	WHERE rn = 1
),
