CREATE VIEW [TYR].[CONOPS_TYR_EQMT_ALARM_DRILL_OPERATOR_BREAK_V] AS


-- SELECT * FROM [tyr].[CONOPS_TYR_EQMT_ALARM_DRILL_OPERATOR_BREAK_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [TYR].[CONOPS_TYR_EQMT_ALARM_DRILL_OPERATOR_BREAK_V]
AS
...
          	   OPERATORID,
          	   OperatorName
   		FROM [tyr].[DRILL_UTILIZATION] [D] WITH (NOLOCK)
...
 		SELECT shiftflag, siteflag, [SHIFTINDEX], [MACHINESTATUSCODE], EQUIPMENTNUMBER, END_TIME_TS,
		...
		WHERE cn > 2
		UNION ALL
		SELECT shiftflag, siteflag, [SHIFTINDEX], [MACHINESTATUSCODE], EQUIPMENTNUMBER, END_