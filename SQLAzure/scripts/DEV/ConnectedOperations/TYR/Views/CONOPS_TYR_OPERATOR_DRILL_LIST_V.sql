CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_DRILL_LIST_V] AS





-- SELECT * FROM [tyr].[CONOPS_TYR_OPERATOR_DRILL_LIST_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [tyr].[CONOPS_TYR_OPERATOR_DRILL_LIST_V] 
AS
	...
	OperatorDetail AS (
		...
		FROM OperatorList [o]
		LEFT JOIN [tyr].[CONOPS_TYR_DRILL_DETAIL_V] d WITH (NOLOCK)
