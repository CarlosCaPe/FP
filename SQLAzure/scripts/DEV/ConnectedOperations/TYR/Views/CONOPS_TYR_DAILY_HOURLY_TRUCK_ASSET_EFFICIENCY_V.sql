CREATE VIEW [TYR].[CONOPS_TYR_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] AS


  
--select * from [tyr].[CONOPS_TYR_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)  
CREATE VIEW [tyr].[CONOPS_TYR_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V]   
AS  
...
WITH TimesDetails AS (
	...
	FROM(
		...
		FROM [TYR].[EQUIPMENT_HOURLY_STATUS] [HOS] WITH (NOLOCK)
