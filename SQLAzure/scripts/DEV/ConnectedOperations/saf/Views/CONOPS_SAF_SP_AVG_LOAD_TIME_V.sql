CREATE VIEW [saf].[CONOPS_SAF_SP_AVG_LOAD_TIME_V] AS
  
  
--SELECT * FROM [saf].[CONOPS_SAF_SP_AVG_LOAD_TIME_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [saf].[CONOPS_SAF_SP_AVG_LOAD_TIME_V]  
AS  
...
SELECT s.shiftflag,  
    ...
    s.HangTimeTarget,  
       ...
       st.eqmtcurrstatus  
FROM NL nl  
LEFT JOIN [saf].[CONOPS_SAF_SHOVEL_POPUP] [s] WITH (NOLOCK)  
ON nl.shiftindex = [s].shiftindex AND nl.excav = [s].ShovelID  
LEFT JOIN STAT st ON s.shiftid = st.shiftid AND nl.EXCAV = st.eqmt AND st.num = 1  
