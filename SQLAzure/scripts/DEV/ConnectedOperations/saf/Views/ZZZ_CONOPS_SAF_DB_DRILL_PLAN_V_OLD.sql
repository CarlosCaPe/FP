CREATE VIEW [saf].[ZZZ_CONOPS_SAF_DB_DRILL_PLAN_V_OLD] AS


--select * from [saf].[CONOPS_SAF_DB_DRILL_PLAN_V] where shiftflag = 'prev'
CREATE VIEW [saf].[CONOPS_SAF_DB_DRILL_PLAN_V]
AS
...
fdr.SHIFTINDEX,
fdr.SITE_CODE,
'D' + RIGHT('000' + SUBSTRING(DRILL_ID,CHARINDEX('-',DRILL_ID)+1,LEN(DRILL_ID)), 3) AS Eqmt,
...
LEFT JOIN EqmtStatus d ON a.ShiftIndex = d.ShiftIndex AND b.Eqmt = d.eqmt AND a.siteflag = 'SAF' and d.num = 1
