CREATE VIEW [saf].[ZZZ_CONOPS_SAF_EOS_KPI_SUMMARY_V_OLD] AS





-- SELECT * FROM [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_V]  
AS  
...
GetHosQuery AS (
	...
	WHERE site_code = 'SAF' and shiftindex = (SELECT ShiftIndex FROM CteShiftInfo WHERE ShiftFlag = 'CURR') 
	ORDER BY hos DESC
),
...
LEFT JOIN [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HAULAGE_V] [ksh] WITH (NOLOCK)
