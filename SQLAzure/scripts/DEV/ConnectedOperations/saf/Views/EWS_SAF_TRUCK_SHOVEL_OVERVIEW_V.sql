CREATE VIEW [saf].[EWS_SAF_TRUCK_SHOVEL_OVERVIEW_V] AS


--select * from [saf].[EWS_SAF_TRUCK_SHOVEL_OVERVIEW_V] WITH(NOLOCK)
CREATE VIEW [saf].[EWS_SAF_TRUCK_SHOVEL_OVERVIEW_V] 
AS
...
FROM [saf].[CONOPS_SAF_TP_TONS_HAUL_V]  WITH (NOLOCK)
...
FROM [saf].[CONOPS_SAF_OVERALL_AVG_PAYLOAD_V] WITH (NOLOCK)),
...
FROM [saf].[CONOPS_SAF_TP_EQMT_STATUS_V] WITH (NOLOCK)
...
[eqmtcurrstatus],
count(distinct([eqmt])) countEquip
FROM [saf].[CONOPS_SAF_SP_EQMT_STATUS_V] WITH (NOLOCK)
group by [siteflag],[shiftflag],[eqmtcurrstatus]
) d
PIVOT  
(  
...
FROM [saf].[CONOPS_SAF_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
...
,ROUND([availability],0) AS [availability]
,ROUND([use_of_availability],0) AS use_of_availability
FROM [saf].[CONOPS_SAF_SHOVEL_ASSET_EFFICIENCY_V] WITH (NOLOCK))
