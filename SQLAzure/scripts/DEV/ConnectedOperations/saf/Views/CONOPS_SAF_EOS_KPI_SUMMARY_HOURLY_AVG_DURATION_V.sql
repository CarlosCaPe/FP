CREATE VIEW [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] AS
...
-- SELECT * FROM [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HOURLY_AVG_DURATION_V]  
AS  
...
StatusEvent AS (
	...
	FROM (
		...
		FROM [saf].[CONOPS_SAF_SHIFT_INFO_V] [si] WITH (NOLOCK)
