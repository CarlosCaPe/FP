CREATE VIEW [saf].[CONOPS_SAF_TP_AVG_PAYLOAD_V] AS

--select * from [saf].[CONOPS_SAF_TP_AVG_PAYLOAD_V] WITH (NOLOCK) ORDER BY shiftflag, siteflag, TRUCK
CREATE VIEW [saf].[CONOPS_SAF_TP_AVG_PAYLOAD_V] 
AS
...
RIGHT JOIN [SAF].[CONOPS_SAF_TRUCK_DETAIL_V] [truck] WITH (NOLOCK)
