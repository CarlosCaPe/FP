CREATE VIEW [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] AS




-- SELECT * FROM [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [saf].[CONOPS_SAF_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V]  
AS  
	...
	TonnageRaw AS (
		...
		FROM (
			SELECT Siteflag,
				   ...
				   ShiftStartDateTime,
				   ShiftEndDateTime
			FROM [saf].[CONOPS_SAF_SHIFT_LINE_GRAPH_V] (NOLOCK)
		) m
	),
	...
	TotalTarget AS (
		...
		FROM  [saf].[CONOPS_SAF_SHIFT_TARGET_V] WITH (NOLOCK)
	),
