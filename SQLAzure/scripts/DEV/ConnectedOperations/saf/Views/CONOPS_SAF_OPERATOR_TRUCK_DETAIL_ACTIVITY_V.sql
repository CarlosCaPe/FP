CREATE VIEW [saf].[CONOPS_SAF_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] AS
...
-- SELECT * FROM [SAF].[CONOPS_SAF_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV'
CREATE VIEW [saf].[CONOPS_SAF_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] 
AS
...
    FROM [SAF].[CONOPS_SAF_OPERATOR_TRUCK_RAMP_EVENT_V] WITH (NOLOCK)
	UNION ALL
	SELECT [shiftflag]
		,[siteflag]
		,[eqmtid] AS [TruckId]
...
    FROM [SAF].[CONOPS_SAF_OPERATOR_TRUCK_ALERTS_V] WITH (NOLOCK)
)
...
SELECT [op].shiftflag
	...
	,[oa].GeneratedDate
FROM [SAF].[CONOPS_SAF_OPERATOR_TRUCK_V] [op] WITH (NOLOCK) 
