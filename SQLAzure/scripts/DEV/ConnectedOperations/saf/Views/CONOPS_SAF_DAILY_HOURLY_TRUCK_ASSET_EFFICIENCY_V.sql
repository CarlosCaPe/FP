CREATE VIEW [saf].[CONOPS_SAF_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] AS





--select * from [saf].[CONOPS_SAF_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
CREATE VIEW [saf].[CONOPS_SAF_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] 
AS
...
WITH TimesDetails AS (
	...
	FROM(
		...
		FROM [saf].[EQUIPMENT_HOURLY_STATUS] [HOS] WITH (NOLOCK)
