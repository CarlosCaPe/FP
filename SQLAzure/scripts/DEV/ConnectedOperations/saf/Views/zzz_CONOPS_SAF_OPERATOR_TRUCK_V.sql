CREATE VIEW [saf].[zzz_CONOPS_SAF_OPERATOR_TRUCK_V] AS
...
CREATE VIEW [saf].[zzz_CONOPS_SAF_OPERATOR_TRUCK_V] 
AS
...
WITH CTE AS (
	...
	FROM [saf].[CONOPS_SAF_TRUCK_DETAIL_V] [td]
	LEFT JOIN [saf].[CONOPS_SAF_SHIFT_OVERVIEW_V] [so]
...
FROM [saf].[CONOPS_SAF_OPERATOR_TRUCK_LIST_V] [tl]
	LEFT JOIN [saf].[CONOPS_SAF_SHIFT_INFO_V] [shift]
	ON [tl].[shiftflag] = [shift].[shiftflag]
LEFT JOIN [saf].[CONOPS_SAF_TRUCK_POPUP_V] [pop]
