CREATE VIEW [saf].[CONOPS_SAF_EQMT_PAYLOAD_CATEGORY_V] AS
  
 
-- SELECT * FROM [saf].[CONOPS_SAF_EQMT_PAYLOAD_CATEGORY_V] WHERE shiftindex = '38820'  
CREATE VIEW [saf].[CONOPS_SAF_EQMT_PAYLOAD_CATEGORY_V]  
AS  
...
LEFT JOIN [saf].[pit_excav_c] [s] WITH (NOLOCK)  
ON [t].fieldexcav = [s].Id AND [t].SHIFTINDEX = [s].SHIFTINDEX  
LEFT JOIN dbo.LH_LOAD ld WITH (NOLOCK) ON t.SHIFTINDEX = ld.SHIFTINDEX AND s.FieldId = ld.excav AND ld.SITE_CODE = 'SAF'  
left join saf.enum enum WITH (NOLOCK)on enum.id = t.FieldEqmttype),  
...
FROM CatTotal a  
LEFT JOIN TOtalCat b ON a.SHIFTINDEX = b.SHIFTINDEX AND a.ShovelId