CREATE VIEW [saf].[CONOPS_SAF_EQMT_SHIFT_INFO_V] AS




--select * from [saf].[CONOPS_SAF_EQMT_SHIFT_INFO_V] order by shiftflag
CREATE VIEW [saf].[CONOPS_SAF_EQMT_SHIFT_INFO_V] 
AS
...
FROM (
     ...
     LEFT JOIN
         (SELECT shiftid,
                 ...
                 ShiftDuration
          FROM [saf].[shift_info] WITH (nolock)) b ON a.shiftid = b.shiftid) sh
