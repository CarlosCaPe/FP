CREATE VIEW [saf].[ZZZ_CONOPS_SAF_OPERATOR_SHOVEL_V_OLD] AS




-- SELECT * FROM [SAF].[CONOPS_SAF_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [saf].[CONOPS_SAF_OPERATOR_SHOVEL_V_OLD]
AS
...
    ,[os].[OperatorStatus]
FROM OperStatus [os]
LEFT OUTER JOIN [SAF].[CONOPS_SAF_EQMT_SHOVEL_V] [es]
	ON [os].ShiftFlag = [es].shiftflag
	AND [os].ShovelID = [es].shovelid
LEFT OUTER JOIN [SAF].[CONOPS_SAF_SHIFT_INFO_V] [shift]
