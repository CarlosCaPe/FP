CREATE VIEW [saf].[CONOPS_SAF_SHIFT_SNAPSHOT_SEQ_V] AS


--SELECT * FROM [saf].[CONOPS_SAF_SHIFT_SNAPSHOT_SEQ_V] where shiftflag = 'curr' order by shiftseq asc

CREATE VIEW [saf].[CONOPS_SAF_SHIFT_SNAPSHOT_SEQ_V]
AS
  ...
  FROM(
   SELECT
    ...
    FROM
      ...
      LEFT JOIN [saf].[CONOPS_SAF_SHIFT_SNAPSHOT_V] snap ON shiftinfo.shiftid = snap.shiftid --ON shiftinfo.shiftflag = timeseq.[shift]
