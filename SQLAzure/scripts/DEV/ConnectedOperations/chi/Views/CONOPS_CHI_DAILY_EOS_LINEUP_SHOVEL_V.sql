CREATE VIEW [chi].[CONOPS_CHI_DAILY_EOS_LINEUP_SHOVEL_V] AS



-- SELECT * FROM [chi].[CONOPS_CHI_DAILY_EOS_LINEUP_SHOVEL_V]  WHERE [shiftflag] = 'CURR'
CREATE VIEW [chi].[CONOPS_CHI_DAILY_EOS_LINEUP_SHOVEL_V] 
AS
...
FROM (
	SELECT [shift].shiftflag,
		   ...
		   [s].FieldReason
	FROM [CHI].[pit_excav_c] [s] WITH (NOLOCK)
	LEFT JOIN [CHI].[CONOPS_CHI_EOS_SHIFT_INFO_V] [shift] WITH (NOLOCK)
