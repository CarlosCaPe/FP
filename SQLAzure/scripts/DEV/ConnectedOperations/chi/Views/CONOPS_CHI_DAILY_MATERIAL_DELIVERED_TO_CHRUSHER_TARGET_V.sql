CREATE VIEW [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] AS

  
  
  
  
--select * from [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] WITH (NOLOCK)  
CREATE VIEW [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V]   
AS  

 WITH crusherTarget AS (  
  SELECT Right([Year], 2) + FORMAT(CAST([Month] AS numeric), '00') [ShiftId],  
      [siteflag],  
      [TotalMaterialtoCrusher] as [Target]  
  FROM (  
   SELECT DATEEFFECTIVE,  
       ...
       siteflag,  
       [TotalMaterialtoCrusher] / 2 AS [TotalMaterialtoCrusher]
