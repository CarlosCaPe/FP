CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_SHOVEL_V] AS
...
-- SELECT * FROM [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_V]  
AS  
...
SELECT   
 ...
 ,[es].Hangtime  
 ,[es].HangtimeTarget 
FROM [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_LIST_V] [os]  
LEFT JOIN [CHI].[CONOPS_CHI_SHIFT_INFO_V] [shift]   
 ON [os].shiftid = [shift].shiftid  
LEFT JOIN [CHI].[CONOPS_CHI_SHOVEL_POPUP] [es] WITH (NOLOCK)
 ON [os].ShiftFlag = [es].shiftflag  
 AND [os].ShovelID = [es].shovelid  
LEFT JOIN [CHI].[CONOPS_CHI_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V] [ae]  
 ON [os].shiftid = [ae].shiftid  
 AND [os].ShovelID = [ae].eqmt   
CROSS JOIN [CHI].[CONOPS_CHI_EQMT_ASSET_EFFICIENCY_TARGET_V] [dct]  
