CREATE VIEW [chi].[CONOPS_CHI_JOB_STATUS_V] AS

--SELECT * FROM CHI.CONOPS_CHI_JOB_STATUS_V
CREATE VIEW CHI.CONOPS_CHI_JOB_STATUS_V
AS
...
--WHERE JOB_QUEUE IN ('JC_DSP_CONOPS_CHI')
...
FROM dbo.DI_JOB_CONTROL_ENTRY_TS_BASE WITH(NOLOCK)
WHERE JOB_NAME IN ('job_conops_equipment_hourly_status_CHI', 'job_conops_asset_efficiency_CHI', 
'job_conops_drill_utilization_CHI', 'job_conops_delta_c', 'job_conops_ios_stockpile_levels', 
...
DataCheckTarget AS(
SELECT 
	'CHI_SP_To_SQLMI_PlanValues' AS JOB_NAME,
...
SELECT
	JOB_NAME,
	REPLACE(REPLACE(JOB_NAME, 'job_conops_', ''), '_CHI', '') AS TABLE_NAME,
	JOB_TYPE,
