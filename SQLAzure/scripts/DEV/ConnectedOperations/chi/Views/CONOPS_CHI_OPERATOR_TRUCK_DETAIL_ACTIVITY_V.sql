CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] AS
...
-- SELECT * FROM [chi].[CONOPS_CHI_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV'
CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_TRUCK_DETAIL_ACTIVITY_V] 
AS
...
    FROM [chi].[CONOPS_CHI_OPERATOR_TRUCK_DELAY_V] WITH (NOLOCK)
	UNION ALL
	SELECT [Shiftflag]
		,[Siteflag]
		,[TruckId]
...
    FROM [chi].[CONOPS_CHI_OPERATOR_TRUCK_ALERTS_V] WITH (NOLOCK)
)
...
SELECT [op].shiftflag
	...
	,[oa].GeneratedDate
FROM [chi].[CONOPS_CHI_OPERATOR_TRUCK_V] [op] WITH (NOLOCK) 
