CREATE VIEW [chi].[CONOPS_CHI_EQMT_ALARM_TRUCK_SHOVEL_OPERATOR_BREAK_V] AS


-- SELECT * FROM [chi].[CONOPS_CHI_EQMT_ALARM_TRUCK_SHOVEL_OPERATOR_BREAK_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [chi].[CONOPS_CHI_EQMT_ALARM_TRUCK_SHOVEL_OPERATOR_BREAK_V]
AS
...
   		SELECT [eq].[SHIFTINDEX],
			   ...
			   [DURATION],
			   [OPERID]
   		FROM [chi].[EQUIPMENT_HOURLY_STATUS] [eq] WITH (NOLOCK)
   		WHERE [REASON] = 400
		      AND [UNIT] IN (1,2)
	),
