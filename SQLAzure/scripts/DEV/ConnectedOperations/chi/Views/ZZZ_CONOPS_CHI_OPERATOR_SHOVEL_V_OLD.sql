CREATE VIEW [chi].[ZZZ_CONOPS_CHI_OPERATOR_SHOVEL_V_OLD] AS




-- SELECT * FROM [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_SHOVEL_V_OLD]
AS
...
    ,[os].[OperatorStatus]
FROM OperStatus [os]
LEFT OUTER JOIN [CHI].[CONOPS_CHI_EQMT_SHOVEL_V] [es]
	ON [os].ShiftFlag = [es].shiftflag
	AND [os].ShovelID = [es].shovelid
LEFT OUTER JOIN [CHI].[CONOPS_CHI_SHIFT_INFO_V] [shift]
