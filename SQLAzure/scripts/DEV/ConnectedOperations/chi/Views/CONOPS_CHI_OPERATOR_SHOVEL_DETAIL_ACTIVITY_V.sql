CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_SHOVEL_DETAIL_ACTIVITY_V] AS




-- SELECT * FROM [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_DETAIL_ACTIVITY_V] WITH (NOLOCK) WHERE ShiftFlag = 'PREV' OperatorId = '60030149'
CREATE VIEW [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_DETAIL_ACTIVITY_V] 
AS
...
    FROM [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_DELAY_V] WITH (NOLOCK)
	UNION ALL
	SELECT [Shiftflag]
		,[Siteflag]
		,[ShovelId]
...
SELECT [op].shiftflag
	...
	,[oa].Reasons
	,[oa].Generated
FROM [CHI].[CONOPS_CHI_OPERATOR_SHOVEL_LIST_V] [op] WITH (NOLOCK) 
