CREATE VIEW [chi].[zzz_CONOPS_CHI_OPERATOR_TRUCK_V] AS
...
CREATE VIEW [chi].[zzz_CONOPS_CHI_OPERATOR_TRUCK_V] 
AS
...
WITH CTE AS (
	SELECT [td].ShiftFlag
		,[td].Siteflag
		,[td].TruckId
		,SUM(NrDumps) AS NrofDumps
	FROM [chi].[CONOPS_CHI_TRUCK_DETAIL_V] [td]
	LEFT JOIN [chi].[CONOPS_CHI_SHIFT_OVERVIEW_V] [so]
...
FROM [chi].[CONOPS_CHI_OPERATOR_TRUCK_LIST_V] [tl]
	LEFT JOIN [chi].[CONOPS_CHI_SHIFT_INFO_V] [shift]
	ON [tl].[shiftflag] = [shift].[shiftflag]
LEFT JOIN [chi].[CONOPS_CHI_TRUCK_POPUP_V] [pop]
