CREATE VIEW [chi].[ZZZ_CONOPS_CHI_DB_DRILL_PLAN_V_OLD] AS
...
--select * from [chi].[CONOPS_CHI_DB_DRILL_PLAN_V] where shiftflag = 'curr'
CREATE VIEW [chi].[CONOPS_CHI_DB_DRILL_PLAN_V]
AS
...
AND RIGHT('0000000000' + fdr.OperatorId, 10) = RIGHT('0000000000' + op.Operator_Id, 10)
WHERE fdr.SITE_CODE = 'CHI'
...
FROM [chi].[CONOPS_CHI_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] ),

TGT AS (
SELECT TOP 1
CAST(FeetDrilledDay/2 AS INT) AS FeetShiftTarget,
CAST(HolesDrilledDay/2 AS INT) AS HoleShiftTarget
...
WHEN a.ShiftDuration = '43200' THEN tg.HoleShiftTarget ELSE 
CAST(tg.HoleShiftTarget/(a.ShiftDuration/3600.0) AS INT) END AS HoleTarget,
tg.FeetShiftTarget,
CASE WHEN a.ShiftDuration = 0 THEN 0 
WHEN a.ShiftDuration = '43200' THEn tg.FeetShiftTarget ELSE 
CAST(tg.FeetShiftTarget/(a.ShiftDuration/3600.0) AS INT) END AS FeetTarget,
...
LEFT JOIN EqmtStatus d ON a.ShiftIndex = d.ShiftIndex AND b.Eqmt = d.eqmt AND a.siteflag = 'CHI' and d.num = 1
