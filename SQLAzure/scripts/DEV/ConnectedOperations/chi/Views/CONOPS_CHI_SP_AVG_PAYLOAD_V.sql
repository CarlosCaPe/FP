CREATE VIEW [chi].[CONOPS_CHI_SP_AVG_PAYLOAD_V] AS

--select * from [chi].[CONOPS_CHI_SP_AVG_PAYLOAD_V] where shiftflag = 'curr'
CREATE VIEW [chi].[CONOPS_CHI_SP_AVG_PAYLOAD_V] 
AS
...
SELECT 
    ...
    [s].[OperatorImageURL]
FROM CTE [sap]
RIGHT JOIN [CHI].[CONOPS_CHI_SHOVEL_INFO_V] [s]
