CREATE VIEW [chi].[CONOPS_CHI_OEE_V] AS




--select * from [chi].[CONOPS_CHI_OEE_V]

CREATE VIEW [chi].[CONOPS_CHI_OEE_V]
AS
...
SELECT shift.shiftflag,
       ...
       shift.shiftid,
       shiftoee.OEE
FROM [chi].[CONOPS_CHI_SHIFT_INFO_V] shift (NOLOCK)
LEFT JOIN (
	 SELECT a.shiftid,
            a.SITE_CODE,
     ...
     FROM (
          ...
          GROUP BY oee.SHIFTID, oee.SITE_CODE
     )
a) shiftoee ON shift.shiftid = shiftoee.shiftid AND shift.siteflag = shiftoee.SITE_CODE
