CREATE VIEW [MOR].[CONOPS_MOR_EQMT_TRUCK_HOURLY_NRDUMP_V] AS

--SELECT * FROM [mor].[CONOPS_MOR_EQMT_TRUCK_HOURLY_NRDUMP_V] WHERE shiftflag = 'prev' and equipment = 'C101'
CREATE VIEW [mor].[CONOPS_MOR_EQMT_TRUCK_HOURLY_NRDUMP_V]
AS

WITH CTE AS (
SELECT
	shiftindex,
	truck AS Equipment,
	count(truck) AS NumberofDumps,
	DUMPTIME_HOS AS HOS
FROM MOR.SHIFT_DUMP_DETAIL_V WITH (NOLOCK)
GROUP BY shiftindex, truck, DUMPTIME_HOS
)

SELECT
	siteflag,
	shiftflag,
	Equipment,
	SUM(NumberofDumps) NumberofDumps,
	HOS,
	dateadd(hour,HOS,ShiftStartDateTime) as TimeinHour
FROM [mor].[CONOPS_MOR_SHIFT_INFO_V] a
LEFT JOIN CTE b on a.SHIFTINDEX = b.SHIFTINDEX
GROUP BY siteflag, shiftflag, Equipment, NumberofDumps, HOS, ShiftStartDateTime

