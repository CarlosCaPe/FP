CREATE VIEW [MOR].[CONOPS_MOR_EQMT_OTHER_V] AS
  
  
  
-- SELECT * FROM [mor].[CONOPS_MOR_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'  ORDER BY shiftflag, siteflag, SupportEquipment  
CREATE VIEW [mor].[CONOPS_MOR_EQMT_OTHER_V]  
AS  
...
WITH SUPEQMT AS (  
 SELECT   
  pax.[ShiftIndex],  
  pax.[FieldId],  
  ( SELECT TOP 1 [DESCRIPTION] FROM [MOR].[Enum] WITH (NOLOCK) WHERE [Id] = pax.[FieldEqmttype] ) AS [EquipmentType],  
  ( SELECT TOP 1 [DESCRIPTION] FROM [MOR].[Enum] WITH (NOLOCK) WHERE [Id] = pax.[FieldUnit] ) AS [EquipmentGroup],  
  pax.[FieldUnit],  
  pax.[FieldStatus],  
  pax.[FieldLoc],  
  pax.[FieldCuroper],  
...
LEFT JOIN (  
 ...
 LEFT JOIN [MOR].[enum] [crew] WITH (NOLOCK)  
