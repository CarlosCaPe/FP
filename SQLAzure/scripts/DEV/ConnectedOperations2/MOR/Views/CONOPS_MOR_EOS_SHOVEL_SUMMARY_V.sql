CREATE VIEW [MOR].[CONOPS_MOR_EOS_SHOVEL_SUMMARY_V] AS





--SELECT * FROM [mor].[CONOPS_MOR_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [mor].[CONOPS_MOR_EOS_SHOVEL_SUMMARY_V]  
AS
...
SELECT 
	...
	CASE WHEN MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'MOR') 
		AND MEASURETON <= (SELECT PayloadFilterUpper FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'MOR')
		THEN MEASURETON ELSE NULL END AS Payload
...
FROM ShovelLoad sl
RIGHT JOIN [MOR].[CONOPS_MOR_SHIFT_INFO_V] s
...
LEFT JOIN mor.CONOPS_MOR_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
