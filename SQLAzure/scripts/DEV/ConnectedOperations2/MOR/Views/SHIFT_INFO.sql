CREATE VIEW [MOR].[SHIFT_INFO] AS

--SELECT * FROM MOR.SHIFT_INFO
CREATE VIEW [MOR].[SHIFT_INFO]
AS
...
FROM [MOROperational].[dbo].SHIFTRootShiftdate AS s WITH (NOLOCK) 
INNER JOIN [MOROperational].[dbo].Enum AS m WITH (NOLOCK) 
	ON s.FieldMonth = m.Id 
INNER JOIN [MOROperational].[dbo].Enum AS st WITH (NOLOCK) 
	ON s.FieldShift = st.Id 
INNER JOIN [MOROperational].[dbo].Enum AS c WITH (NOLOCK) 
	ON s.FieldCrew = c.Id 
),

Last4Shift AS(
SELECT
	TOP 4 'MOR' AS SiteFlag,
...
SELECT
	...
	,ShiftStartDate
	,ShiftStartDateTime
	,CASE WHEN ShiftEndDateTime IS NULL
		THEN (CASE WHEN RIGHT(SHIFTID, 1) = 1
				THEN CONCAT(CAST(LEFT(SHIFTID, 6) AS DATE), ' 19:15:00.000')
				ELSE CONCAT(DATEADD(DAY, 1, CAST(LEFT(SHIFTID, 6) AS DATE)), ' 07:15:00.000') END)
		ELSE ShiftEndDateTime
	...
	,ShiftDuration
	,ShiftDate
	,((DATEDIFF(DAY, '1970-01-01', shiftdate)) * 2 + ((shiftid - 1) % 2)) AS ShiftIndex
	,TimeZoneName
	,is_currently_dst
	,current_utc_offset
