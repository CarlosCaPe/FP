CREATE VIEW [MOR].[CONOPS_MOR_EOS_LINEUP_TRUCK_V] AS
...
-- SELECT * FROM [mor].[CONOPS_MOR_EOS_LINEUP_TRUCK_V]  WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [mor].[CONOPS_MOR_EOS_LINEUP_TRUCK_V] 
AS
...
FROM (
	...
	LEFT JOIN [MOR].[CONOPS_MOR_SHIFT_INFO_V] [shift] WITH (NOLOCK)
