CREATE VIEW [MOR].[CONOPS_MOR_CM_AVG_THROUGHPUT_V] AS
  
  
  
  
  
-- SELECT * FROM [mor].[CONOPS_MOR_CM_AVG_THROUGHPUT_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [mor].[CONOPS_MOR_CM_AVG_THROUGHPUT_V]    
AS    
...
WITH ShiftAvg AS (  
 SELECT ShiftIndex  
  ...
  --,Hr  
 FROM [mor].[CONOPS_MOR_CM_HOURLY_CRUSHER_THROUGHPUT_V] WITH (NOLOCK)  
...
FROM (   
 ...
 FROM [mor].[CONOPS_MOR_CM_HOURLY_CRUSHER_THROUGHPUT_V] [hct] WITH (NOLOCK)  
