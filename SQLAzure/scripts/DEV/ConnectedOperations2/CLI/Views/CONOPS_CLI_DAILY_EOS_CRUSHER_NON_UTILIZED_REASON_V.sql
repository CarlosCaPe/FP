CREATE VIEW [CLI].[CONOPS_CLI_DAILY_EOS_CRUSHER_NON_UTILIZED_REASON_V] AS
  
  
  
--SELECT * FROM [CLI].[CONOPS_CLI_DAILY_EOS_CRUSHER_NON_UTILIZED_REASON_V] WHERE SHIFTFLAG = 'CURR'    
CREATE VIEW [CLI].[CONOPS_CLI_DAILY_EOS_CRUSHER_NON_UTILIZED_REASON_V]    
AS    
 ...
 AND a.SITEFLAG = 'CMX' AND se.site_code = 'CLI'  
 ...
 WHERE se.site_code = 'CLI'  
 AND se.unit = 16 AND se.status IN (1, 3, 4)  
