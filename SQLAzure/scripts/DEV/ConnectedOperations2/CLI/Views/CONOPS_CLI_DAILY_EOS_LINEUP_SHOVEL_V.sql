CREATE VIEW [CLI].[CONOPS_CLI_DAILY_EOS_LINEUP_SHOVEL_V] AS



-- SELECT * FROM [cli].[CONOPS_CLI_DAILY_EOS_LINEUP_SHOVEL_V]  WHERE [shiftflag] = 'CURR'
CREATE VIEW [cli].[CONOPS_CLI_DAILY_EOS_LINEUP_SHOVEL_V] 
AS
...
FROM (
	SELECT [shift].shiftflag,
		   ...
		   [s].FieldReason
	FROM [CLI].[pit_excav_c] [s] WITH (NOLOCK)
	LEFT JOIN [CLI].[CONOPS_CLI_EOS_SHIFT_INFO_V] [shift] WITH (NOLOCK)
