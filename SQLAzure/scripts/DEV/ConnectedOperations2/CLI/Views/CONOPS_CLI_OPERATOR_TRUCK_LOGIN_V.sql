CREATE VIEW [CLI].[CONOPS_CLI_OPERATOR_TRUCK_LOGIN_V] AS
...
-- SELECT * FROM [cli].[CONOPS_CLI_OPERATOR_TRUCK_LOGIN_V] WITH (NOLOCK) WHcliERE [shiftflag] = 'CURR'
CREATE VIEW [cli].[CONOPS_CLI_OPERATOR_TRUCK_LOGIN_V] 
AS
...
WITH OperatorDetail AS (
    ...
    FROM [cli].[CONOPS_CLI_TRUCK_DETAIL_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
...
ON a.SHIFTINDEX = [od].SHIFTINDEX 
	AND a.SITEFLAG = [od].[siteflag]
