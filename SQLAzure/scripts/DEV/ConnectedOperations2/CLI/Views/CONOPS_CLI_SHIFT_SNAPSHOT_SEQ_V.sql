CREATE VIEW [CLI].[CONOPS_CLI_SHIFT_SNAPSHOT_SEQ_V] AS




--SELECT * FROM [cli].[CONOPS_CLI_SHIFT_SNAPSHOT_SEQ_V] where shiftflag = 'curr' order by shiftseq asc

CREATE VIEW [cli].[CONOPS_CLI_SHIFT_SNAPSHOT_SEQ_V]
AS
  ...
  FROM(
   SELECT
    ...
    FROM
      ...
      LEFT JOIN [cli].[CONOPS_CLI_SHIFT_SNAPSHOT_V] snap ON shiftinfo.shiftid = snap.shiftid --ON shiftinfo.shiftflag = timeseq.[shift]
      --WHERE shiftinfo.shiftflag = 'PREV'
	  WHERE shiftinfo.siteflag = 'CMX'
	  ) a
