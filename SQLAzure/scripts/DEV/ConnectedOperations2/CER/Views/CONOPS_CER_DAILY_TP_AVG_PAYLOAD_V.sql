...
--select * from [cer].[CONOPS_CER_DAILY_TP_AVG_PAYLOAD_V] WITH (NOLOCK) where shiftflag = 'curr'  ORDER BY shiftflag, siteflag, TRUCK  
...
FROM (  
 ...
 GROUP BY [payload].SHIFTINDEX,[payload]. SITE_CODE, [t].FieldId, [w].FieldName, [opm].[Personnel_Id]  
 )main  
)   
...
SELECT truck.shiftflag,  
    ...
    OperatorImageURL,  
    [AVG_Payload],  
    --CASE WHEN RIGHT(TRUCK,3) >= 300 THEN '300' ELSE '240' END AS [Target],  
...
LEFT JOIN [CER].[CONOPS_CER_DAILY_TRUCK_DETAIL_V] [truck] WITH (NOLOCK)  
