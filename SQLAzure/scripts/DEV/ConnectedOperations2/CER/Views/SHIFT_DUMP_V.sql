CREATE VIEW [CER].[SHIFT_DUMP_V] AS



CREATE VIEW [CER].[SHIFT_DUMP_V] 
AS
...
SELECT 
	...
	DATEDIFF(HOUR, si.ShiftStartDateTime, DATEADD(SECOND, FieldTimedump, orisi.ShiftStartDateTime)) AS DUMPTIME_HOS
FROM CTE a
LEFT JOIN CER.shift_info orisi
	ON a.OrigShiftid = orisi.shiftid
LEFT JOIN CER.shift_info si
