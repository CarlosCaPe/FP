CREATE VIEW [CER].[CONOPS_CER_CM_DISPATCH_SUMMARY_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_CM_DISPATCH_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [cer].[CONOPS_CER_CM_DISPATCH_SUMMARY_V]  
AS  
	...
	NrOfLoad AS (
		...
		WHERE site_code = 'CER' AND DUMPASN IN (SELECT CrusherLoc FROM CrLoc)
