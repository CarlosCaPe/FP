CREATE VIEW [CER].[CONOPS_CER_DAILY_EOS_SHOVEL_SUMMARY_V] AS





--SELECT * FROM [CER].[CONOPS_CER_DAILY_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [CER].[CONOPS_CER_DAILY_EOS_SHOVEL_SUMMARY_V]  
AS  
...
SELECT 
	...
	CASE WHEN MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'CER') THEN MEASURETON ELSE NULL END AS Payload
...
FROM ShovelLoad sl
RIGHT JOIN [CER].[CONOPS_CER_EOS_SHIFT_INFO_V] s
...
LEFT JOIN CER.CONOPS_CER_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
