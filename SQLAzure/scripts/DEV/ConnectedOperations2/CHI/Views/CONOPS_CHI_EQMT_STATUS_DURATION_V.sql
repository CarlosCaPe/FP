CREATE VIEW [CHI].[CONOPS_CHI_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [CHI].[CONOPS_CHI_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [CHI].[CONOPS_CHI_EQMT_STATUS_DURATION_V] 
AS
...
FROM [CHI].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [CHI].[CONOPS_CHI_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [CHI].[CONOPS_CHI_AUTONOMOUS_TRUCK_V] aut
