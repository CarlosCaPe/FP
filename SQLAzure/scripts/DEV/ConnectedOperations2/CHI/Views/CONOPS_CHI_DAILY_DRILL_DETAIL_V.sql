CREATE VIEW [CHI].[CONOPS_CHI_DAILY_DRILL_DETAIL_V] AS
  
    
      
      
-- SELECT * FROM [chi].[CONOPS_CHI_DAILY_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]        
CREATE VIEW [chi].[CONOPS_CHI_DAILY_DRILL_DETAIL_V]        
AS        
...
SELECT DISTINCT ae.[shiftflag] ,        
 ...
 DRILLASSETEFFICIENCY AS AssetEfficiencyTarget,      
    ...
    [XY_Drill_Score],        
    [OVERALLSCORE]        
FROM [chi].[CONOPS_CHI_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)        
LEFT JOIN [chi].[CONOPS_CHI_DAILY_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)        
...
ON [o].DRILL_ID = [ae].Equipment and [o].num = 1        
   AND [o].SHIFTINDEX = [ae].ShiftIndex AND [ae].siteflag