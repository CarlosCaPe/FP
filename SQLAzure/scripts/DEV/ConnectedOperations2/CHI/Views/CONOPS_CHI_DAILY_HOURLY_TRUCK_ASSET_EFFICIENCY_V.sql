CREATE VIEW [CHI].[CONOPS_CHI_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] AS




--select * from [chi].[CONOPS_CHI_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)
CREATE VIEW [chi].[CONOPS_CHI_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] 
AS
...
WITH TimesDetails AS (
	...
	FROM(
		...
		FROM [chi].[EQUIPMENT_HOURLY_STATUS] [HOS] WITH (NOLOCK)
