CREATE VIEW [ABR].[CONOPS_ABR_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [ABR].[CONOPS_ABR_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_EQMT_STATUS_DURATION_V] 
AS
...
FROM [ABR].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [ABR].[CONOPS_ABR_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [ABR].[CONOPS_ABR_AUTONOMOUS_TRUCK_V] aut
