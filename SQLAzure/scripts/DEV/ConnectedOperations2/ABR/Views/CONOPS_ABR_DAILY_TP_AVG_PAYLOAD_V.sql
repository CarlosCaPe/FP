CREATE VIEW [ABR].[CONOPS_ABR_DAILY_TP_AVG_PAYLOAD_V] AS
...
--select * from [abr].[CONOPS_ABR_DAILY_TP_AVG_PAYLOAD_V] WITH (NOLOCK) where shiftflag = 'curr'  ORDER BY shiftflag, siteflag, TRUCK      
CREATE VIEW [ABR].[CONOPS_ABR_DAILY_TP_AVG_PAYLOAD_V]       
AS      
...
FROM (      
 ...
 WHERE [payload].SITE_CODE = 'ELA'      
    AND [payload].MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'ABR')   
...
LEFT JOIN [abr].[CONOPS_ABR_DAILY_TRUCK_DETAIL_V] [truck] WITH (NOLOCK)      
