CREATE VIEW [ABR].[CONOPS_ABR_DAILY_EOS_SHOVEL_SUMMARY_V] AS





--SELECT * FROM [ABR].[CONOPS_ABR_DAILY_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [ABR].[CONOPS_ABR_DAILY_EOS_SHOVEL_SUMMARY_V]  
AS  
...
SELECT 
	...
	CASE WHEN MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'ABR') THEN MEASURETON ELSE NULL END AS Payload
...
FROM ABR.CONOPS_ABR_DAILY_SHOVEL_INFO_V
)
...
LEFT JOIN ABR.CONOPS_ABR_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
