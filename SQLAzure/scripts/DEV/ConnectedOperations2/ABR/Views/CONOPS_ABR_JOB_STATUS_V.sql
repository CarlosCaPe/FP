CREATE VIEW [ABR].[CONOPS_ABR_JOB_STATUS_V] AS

--SELECT * FROM ABR.CONOPS_ABR_JOB_STATUS_V
CREATE VIEW ABR.CONOPS_ABR_JOB_STATUS_V
AS

WITH TimezoneDiff AS(
SELECT
	current_utc_offset
FROM ABR.CONOPS_ABR_SHIFT_INFO_V
...
--WHERE JOB_QUEUE IN ('JC_DSP_CONOPS_ABR')
...
FROM dbo.DI_JOB_CONTROL_ENTRY_TS_BASE WITH(NOLOCK)
WHERE JOB_NAME IN ('job_conops_equipment_hourly_status_ABR', 'job_conops_asset_efficiency_ABR', 
'job_conops_drill_utilization_ABR', 'job_conops_delta_c', 'job_conops_ios_stockpile_levels', 
...
DataCheckTarget AS(
SELECT
	'ABR_SP_To_SQLMI_PlanValues' AS JOB_NAME,
	'plan_values' AS TABLE_NAME,
	'Shift Target' AS JOB_TYPE,
	CASE 
        WHEN RIGHT(MAX(formatshiftid), 3) = '001' THEN 
            DATEADD(HOUR, 12, CONVERT(DATETIME, '20' + LEFT(MAX(formatshiftid), 6) + ' 08:00', 120))
        ELSE 
            DATEADD(HOUR, 12, CONVERT(DATETIME, '20' + LEFT(MAX(formatshiftid), 6) + ' 20:00', 120))
    END AS MAX_DATA_LOCAL_TS,
	MAX(UTC_CREATED_DATE) AS MAX_DATA_LOAD_UTC,
	30 AS JOB_SCHEDULE_MINS,
	120 AS JOB_ALERT_MINS
FROM ABR.plan_values pv WITH(NOLOCK)
),

Final AS(
SELECT
	JOB_NAME,
	REPLACE(REPLACE(JOB_NAME, 'job_conops_', ''), '_ABR', '') AS TABLE_NAME,
	JOB_TYPE,
