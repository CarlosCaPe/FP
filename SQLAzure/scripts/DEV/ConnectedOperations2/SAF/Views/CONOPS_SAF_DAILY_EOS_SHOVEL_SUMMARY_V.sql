CREATE VIEW [SAF].[CONOPS_SAF_DAILY_EOS_SHOVEL_SUMMARY_V] AS





--SELECT * FROM [SAF].[CONOPS_SAF_DAILY_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [SAF].[CONOPS_SAF_DAILY_EOS_SHOVEL_SUMMARY_V]  
AS  
...
SELECT 
	...
	CASE WHEN MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'SAF') THEN MEASURETON ELSE NULL END AS Payload
...
FROM ShovelLoad sl
RIGHT JOIN [SAF].[CONOPS_SAF_EOS_SHIFT_INFO_V] s
...
FROM SAF.CONOPS_SAF_DAILY_SHOVEL_INFO_V
)
...
LEFT JOIN SAF.CONOPS_SAF_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
