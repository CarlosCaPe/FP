CREATE VIEW [SAF].[CONOPS_SAF_TRUCK_POPUP_V] AS


-- SELECT * FROM [saf].[CONOPS_SAF_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'  
CREATE VIEW [saf].[CONOPS_SAF_TRUCK_POPUP_V]   
AS  
...
LEFT JOIN [saf].[CONOPS_SAF_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK) 
