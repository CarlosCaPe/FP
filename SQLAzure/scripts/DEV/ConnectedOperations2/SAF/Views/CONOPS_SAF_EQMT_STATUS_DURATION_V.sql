CREATE VIEW [SAF].[CONOPS_SAF_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [SAF].[CONOPS_SAF_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [SAF].[CONOPS_SAF_EQMT_STATUS_DURATION_V] 
AS
...
FROM [SAF].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [SAF].[CONOPS_SAF_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [SAF].[CONOPS_SAF_AUTONOMOUS_TRUCK_V] aut
