CREATE VIEW [SAF].[CONOPS_SAF_TRUCK_SHIFT_OVERVIEW_V] AS

-- SELECT * FROM [saf].[CONOPS_SAF_TRUCK_SHIFT_OVERVIEW_V] WITH (NOLOCK)
CREATE VIEW [saf].[CONOPS_SAF_TRUCK_SHIFT_OVERVIEW_V]
AS
...
ShiftDumpPerTruck AS (
   ...
   GROUP BY sd.shiftid, sd.Region, sd.TruckId, sd.[Location], sd.[enumId], sd.[Dump]
),
...
TruckByRegion AS (
   SELECT pe.SHIFTID,
          ...
          CASE WHEN (pplr.FieldId = 'SAN JUAN' AND pe.FieldId = 'S003')
               ...
               ELSE 1
          END AS [IsValidTruckRegion]
   FROM saf.pit_truck_c pe WITH (NOLOCK)
   ...
   SELECT DISTINCT sd.shiftid,
				   sd.Region,
				   sd.TruckId,
				   CASE WHEN (sd.Region = 'SAN JUAN' AND sd.TruckId = 'S003')
							  OR (sd.Region = 'SAN JUAN' AND sd.TruckId = 'S005') THEN 0
				   ELSE 1
				   END AS [IsValidTruckRegion]
   FROM ShiftDump sd
)
...
FROM TruckByRegion sbr
LEFT JOIN ShiftDumpPerTruck sdps
ON sdps.Region = sbr.Region AND sdps.TruckId = sbr.TruckId
   AND sdps.shiftid = sbr.shiftid
WHERE sbr.IsValidTruckRegion = 1 AND sbr.SHIFTID IS NOT NULL
GROUP BY sbr.SHIFTID, sbr.TruckId, sbr.IsValidTruckRegion;
