CREATE VIEW [TYR].[CONOPS_TYR_EQMT_OTHER_V] AS



-- SELECT * FROM [TYR].[CONOPS_TYR_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'  ORDER BY shiftflag, siteflag, SupportEquipment
CREATE VIEW [TYR].[CONOPS_TYR_EQMT_OTHER_V]
AS
...
WITH SUPEQMT AS (
	SELECT 
		pax.[ShiftIndex],
		pax.[FieldId],
		( SELECT TOP 1 [DESCRIPTION] FROM [TYR].[Enum] WITH (NOLOCK) WHERE [Id] = pax.[FieldEqmttype] ) AS [EquipmentType],
		( SELECT TOP 1 [DESCRIPTION] FROM [TYR].[Enum] WITH (NOLOCK) WHERE [Id] = pax.[FieldUnit] ) AS [EquipmentGroup],
		pax.[FieldUnit],
		pax.[FieldStatus],
		pax.[FieldLoc],
		pax.[FieldCuroper],
	...
	WHERE pax.FieldUnit IN (251, 246, 241, 253, 243)
),
...
LEFT JOIN (
	SELECT [pax].SHIFTINDEX,
		   ...
		   CASE 
			WHEN [pax].[FieldUnit] = 251 THEN 'Front End Loader'
			WHEN [pax].[FieldUnit] = 246 THEN 'Water Truck'
			WHEN [pax].[FieldUnit] = 241 THEN 'Dozer'
			WHEN [pax].[FieldUnit] = 253 THEN 'Wrecker'
			WHEN [pax].[FieldUnit] = 243 THEN 'Rubber Tire Dozer'
