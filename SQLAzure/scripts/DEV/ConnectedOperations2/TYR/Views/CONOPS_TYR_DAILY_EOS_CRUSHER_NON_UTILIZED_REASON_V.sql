CREATE VIEW [TYR].[CONOPS_TYR_DAILY_EOS_CRUSHER_NON_UTILIZED_REASON_V] AS



--SELECT * FROM [tyr].[CONOPS_TYR_DAILY_EOS_CRUSHER_NON_UTILIZED_REASON_V] WHERE SHIFTFLAG = 'CURR'    
CREATE VIEW [TYR].[CONOPS_TYR_DAILY_EOS_CRUSHER_NON_UTILIZED_REASON_V]    
AS    
 ...
 ON se.shiftindex = rt.shiftindex AND rt.SITE_CODE = 'TYR'
    AND se.status = rt.status AND se.reason = rt.reason  
 WHERE se.site_code = 'TYR'  
