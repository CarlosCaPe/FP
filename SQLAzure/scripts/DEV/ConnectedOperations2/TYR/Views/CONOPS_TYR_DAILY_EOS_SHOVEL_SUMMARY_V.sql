CREATE VIEW [TYR].[CONOPS_TYR_DAILY_EOS_SHOVEL_SUMMARY_V] AS





--SELECT * FROM [TYR].[CONOPS_TYR_DAILY_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [TYR].[CONOPS_TYR_DAILY_EOS_SHOVEL_SUMMARY_V]  
AS  
...
SELECT 
	...
	CASE WHEN MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'TYR') THEN MEASURETON ELSE NULL END AS Payload
...
FROM ShovelLoad sl
RIGHT JOIN [TYR].[CONOPS_TYR_EOS_SHIFT_INFO_V] s
...
FROM TYR.CONOPS_TYR_DAILY_SHOVEL_INFO_V
)
...
LEFT JOIN TYR.CONOPS_TYR_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
