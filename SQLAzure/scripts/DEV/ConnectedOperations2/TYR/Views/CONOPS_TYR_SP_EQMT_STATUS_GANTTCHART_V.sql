CREATE VIEW [TYR].[CONOPS_TYR_SP_EQMT_STATUS_GANTTCHART_V] AS


  
--select * from [tyr].[CONOPS_TYR_SP_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'curr'order by eqmt  
  
CREATE VIEW [TYR].[CONOPS_TYR_SP_EQMT_STATUS_GANTTCHART_V]  
AS  
...
b.ShiftEndDateTime  
FROM [tyr].[CONOPS_TYR_EQMT_SHIFT_INFO_V] a  
LEFT JOIN (  
SELECT   
siteflag,  
shiftflag,  
...
FROM [tyr].[CONOPS_TYR_EQMT_SHIFT_INFO_V]  
...
eqmt,  
tprh   
FROM [tyr].[CONOPS_TYR_SHOVEL_TPRH_V] WITH (NOLOCK)  
WHERE site_code = 'TYR')  
