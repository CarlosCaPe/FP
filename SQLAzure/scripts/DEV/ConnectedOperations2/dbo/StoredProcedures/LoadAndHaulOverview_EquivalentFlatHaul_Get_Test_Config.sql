...
CREATE PROCEDURE [dbo].[LoadAndHaulOverview_EquivalentFlatHaul_Get_Test_Config] 
(	
	@SHIFT VARCHAR(4),
	@SITE VARCHAR(4)
)
...
BEGIN TRY
	...
	IF @SITE = 'MOR' AND (SELECT STATUS FROM dbo.CONOPS_SITE_STATUS WHERE SITE_CODE = 'MOR') = 1
	BEGIN
	...
	ELSE IF @SITE = 'SAM' AND (SELECT STATUS FROM dbo.CONOPS_SITE_STATUS WHERE SITE_CODE = 'SAM') = 1
	BEGIN
	...
	ELSE IF @SITE = 'SIE' AND (SELECT STATUS FROM dbo.CONOPS_SITE_STATUS WHERE SITE_CODE = 'SIE') = 1
	BEGIN
