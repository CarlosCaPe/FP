CREATE VIEW [SIE].[CONOPS_SIE_DAILY_TP_AVG_PAYLOAD_V] AS
...
--select * from [sie].[CONOPS_SIE_DAILY_TP_AVG_PAYLOAD_V] WITH (NOLOCK) where shiftflag = 'curr'  ORDER BY shiftflag, siteflag, TRUCK    
CREATE VIEW [sie].[CONOPS_SIE_DAILY_TP_AVG_PAYLOAD_V]     
AS    
...
LEFT JOIN [SIE].[CONOPS_SIE_DAILY_TRUCK_DETAIL_V] [truck] WITH (NOLOCK)    
ON [pl].TRUCK = [truck].TruckID AND [pl].SHIFTINDEX = [truck].SHIFTINDEX     
LEFT JOIN [dbo].[PAYLOAD_TARGET] [pt] ON [truck].siteflag = [pt].siteflag    
