CREATE VIEW [SIE].[CONOPS_SIE_DAILY_EOS_SHOVEL_SUMMARY_V] AS





--SELECT * FROM [SIE].[CONOPS_SIE_DAILY_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [SIE].[CONOPS_SIE_DAILY_EOS_SHOVEL_SUMMARY_V]  
AS  
...
SELECT 
	...
	CASE WHEN MEASURETON >= (SELECT PayloadFilterLower FROM dbo.PAYLOAD_FILTER WHERE SITEFLAG = 'SIE') THEN MEASURETON ELSE NULL END AS Payload
...
FROM ShovelLoad sl
RIGHT JOIN [SIE].[CONOPS_SIE_EOS_SHIFT_INFO_V] s
...
LEFT JOIN SIE.CONOPS_SIE_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
