CREATE VIEW [SIE].[CONOPS_SIE_DAILY_EOS_CRUSHER_STATS_V] AS



--SELECT * FROM [sie].[CONOPS_SIE_DAILY_EOS_CRUSHER_STATS_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'
CREATE VIEW [sie].[CONOPS_SIE_DAILY_EOS_CRUSHER_STATS_V]
AS
...
WHERE cs.CRUSHERLOC IN ('A-SIDE','B-SIDE')
...
SELECT s.SHIFTFLAG  
   ...
   ,DATEADD(HOUR,s.current_utc_offset,cs.UTC_CREATED_DATE) AS DateTime
   ,SUM(cs.NOOFTRUCKWAITING)
FROM [SIE].CONOPS_SIE_EOS_SHIFT_INFO_V s WITH (NOLOCK)
LEFT JOIN RecursiveCTE cs WITH (NOLOCK)
ON s.SHIFTINDEX = cs.SHIFTINDEX AND s.SITEFLAG = cs.SITEFLAG
WHERE cs.CRUSHERLOC IN ('A-SIDE','B-SIDE')
GROUP BY s.SHIFTFLAG  
,s.SITEFLAG  
,s.SHIFTSTARTDATETIME  
,s.SHIFTENDDATETIME  
