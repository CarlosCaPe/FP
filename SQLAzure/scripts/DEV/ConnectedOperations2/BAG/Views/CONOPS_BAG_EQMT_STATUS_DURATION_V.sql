CREATE VIEW [BAG].[CONOPS_BAG_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [bag].[CONOPS_BAG_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [BAG].[CONOPS_BAG_EQMT_STATUS_DURATION_V] 
AS
...
FROM [BAG].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [BAG].[CONOPS_BAG_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [BAG].[CONOPS_BAG_AUTONOMOUS_TRUCK_V] aut
