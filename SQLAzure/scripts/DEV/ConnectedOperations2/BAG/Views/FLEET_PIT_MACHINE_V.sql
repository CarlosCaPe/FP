...
SELECT
	...
	DATEADD(HH, -7, p.STATUS_LAST_UPDATED_UTC) AS StatusStart,
...
FROM ConnectedOperations.bag2.MACHINE_IN_PIT p
LEFT JOIN ConnectedOperations.bag2.MACHINE m
	ON p.MACHINE_OID = m.MACHINE_OID
LEFT JOIN ConnectedOperations.bag2.MACHINECLASS mc
	ON m.CLASS = mc.MACHINECLASS_OID
LEFT JOIN ConnectedOperations.bag2.MACHINECATEGORY mca
	ON mc.CATEGORY = mca.MACHINECATEGORY_OID
LEFT JOIN ConnectedOperations.bag2.LOCATION l
	ON l.LOCATION_OID = p.LOCATION_OID
LEFT JOIN ConnectedOperations.bag2.PERSON op
	ON op.PERSON_OID = p.CURRENT_OPERATOR_OID
LEFT JOIN ConnectedOperations.bag2.MACHINE s
