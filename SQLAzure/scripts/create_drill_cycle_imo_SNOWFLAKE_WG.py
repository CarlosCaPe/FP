"""
Create DRILL_CYCLE_IMO table type in DEV SQL Azure - SNOWFLAKE_WG database
Ticket: Requested by Vikas Uttam
CORRECTED: Should be in SNOWFLAKE_WG, not ConnectedOperations
"""
...
# DEV server - SNOWFLAKE_WG database (CORRECT!)
server = 'azwd22midbx02.eb8a77f2eea6.database.windows.net'
database = 'SNOWFLAKE_WG'  # <-- CORRECTED DATABASE
...
if existing:
    print(f'Type DRILL_CYCLE_IMO already exists in SNOWFLAKE_WG!')
else:
    print("Creating [dbo].[DRILL_CYCLE_IMO] table type in SNOWFLAKE_WG...")
...
'''
    ...
    conn.commit()
    print('SUCCESS: Created [dbo].[DRILL_CYCLE_IMO] table type in SNOWFLAKE_WG!')
...
ORDER BY t.name
""")
print('\nAll Table Types in SNOWFLAKE_WG:')
