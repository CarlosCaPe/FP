...
WITH recent_holes AS (
    SELECT
        ...
        dc.drill_id,
        DENSE_RANK() OVER (
            PARTITION BY dc.pushback
            ORDER BY CAST(LEFT(dc.start_hole_ts_local,10) AS DATE) DESC
        ) AS start_date_rank,
        ROW_NUMBER() OVER (
            PARTITION BY dc.ACTUAL_DRILL_HOLE_START_METERS_X, dc.ACTUAL_DRILL_HOLE_START_METERS_Y
            ORDER BY dc.PLAN_CREATION_TS_LOCAL DESC
        ) AS rn
    FROM dbo.drill_cycle AS dc
    LEFT JOIN dbo.drill_plan AS dp
        ON dc.drill_plan_sk = dp.drill_plan_sk
    WHERE
        dc.site_code LIKE '%mor%'                -- ILIKE → LIKE
        AND dc.drill_hole_status IN (2, 6)
        AND CAST(LEFT(dc.start_hole_ts_local,23) AS DATETIME) >= CAST(DATEADD(DAY, -1, SYSDATETIME()) AS DATE)
...
per_pit_per_day AS (
    SELECT
        pushback,
        start_hole_date,
        COUNT(DISTINCT drill_hole_name) AS hole_count,
        COUNT(DISTINCT drill_id)       AS drill_count
    FROM recent_holes
    WHERE rn = 1  and start_date_rank <= 7                     -- QUALIFY rn = 1 → WHERE rn = 1
    GROUP BY
        pushback,
