CREATE VIEW [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V] AS
...
--select * from [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V] WITH (NOLOCK)  
CREATE VIEW [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V]   
AS  
...
FROM CrLocShift cl  
LEFT JOIN CTE  
ON cl.SITEFLAG = cte.siteflag AND cl.SHIFTID = cte.SHIFTID --AND cl.shiftflag = cte.shiftflag
   AND cl.CrusherLoc = cte.CrusherLoc  
LEFT JOIN [chi].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] [ct] WITH (NOLOCK)  
ON [ct].shiftid = cl.shiftid AND ct.shiftflag = cl.shiftflag AND
   cl.CrusherLoc = [ct].[Location]  
GROUP BY cl.siteflag, cl.SHIFTFLAG, cl.CrusherLoc, [ct].[Target], cl.ShiftDuration, NRDumps ,cl.shiftid  
