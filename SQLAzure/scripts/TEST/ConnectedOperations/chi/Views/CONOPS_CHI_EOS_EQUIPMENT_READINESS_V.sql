CREATE VIEW [chi].[CONOPS_CHI_EOS_EQUIPMENT_READINESS_V] AS

--select * from [CHI].[CONOPS_CHI_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [chi].[CONOPS_CHI_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			FROM CHI.EQUIPMENT_HOURLY_STATUS WITH(NOLOCK)
			WHERE
		...
		INNER JOIN CHI.CONOPS_CHI_SHIFT_INFO_V s
