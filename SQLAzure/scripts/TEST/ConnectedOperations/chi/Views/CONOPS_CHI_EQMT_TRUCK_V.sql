CREATE VIEW [chi].[CONOPS_CHI_EQMT_TRUCK_V] AS
...
-- SELECT * FROM [chi].[CONOPS_CHI_EQMT_TRUCK_V] WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [chi].[CONOPS_CHI_EQMT_TRUCK_V]    
AS    
...
SUM(NRDumps) AS NumberOfDumps
FROM [chi].[CONOPS_CHI_TRUCK_DETAIL_V] a
LEFT JOIN [chi].[CONOPS_CHI_TRUCK_SHIFT_OVERVIEW_V] b
ON a.shiftid = b.shiftid AND a.truckid = b.truckid
LEFT JOIN [chi].[CONOPS_CHI_SHOVEL_TARGET_V] c
ON LEFT(a.shiftid, 4) >= c.shiftid AND a.AssignedShovel = c.shovel
GROUP BY
shiftflag,
siteflag,
shiftindex,
...
[Availability],
AvailabilityTarget
FROM [chi].[CONOPS_CHI_TRUCK_POPUP] WITH (NOLOCK)),
...
truck,
TonsHaul
FROM [chi].[CONOPS_CHI_TRUCK_TPRH])
...
LEFT JOIN CHI.CONOPS_CHI_EQMT_DELTA_C_V dc
