CREATE VIEW [chi].[CONOPS_CHI_DAILY_EOS_SHOVEL_SUMMARY_V] AS

--SELECT * FROM [CHI].[CONOPS_CHI_DAILY_EOS_SHOVEL_SUMMARY_V] WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [CHI].[CONOPS_CHI_DAILY_EOS_SHOVEL_SUMMARY_V]  
AS  
...
SELECT 
	...
	sl.SiteFlag,
	EXCAV as ShovelId,
	CASE WHEN LEFT(LOC,4) BETWEEN '0000' AND '9999' THEN LEFT(grade,5)
		ELSE LOC END AS PushBack,
	FieldLSizeTons AS Loadtons,
	CASE WHEN PayloadFilter = 1
		THEN FieldTons ELSE NULL
	END AS Payload
FROM CHI.SHIFT_LOAD_DETAIL_V sl WITH (NOLOCK)
RIGHT JOIN [CHI].[CONOPS_CHI_EOS_SHIFT_INFO_V] s
...
FROM CHI.CONOPS_CHI_DAILY_SHOVEL_INFO_V
)
...
LEFT JOIN CHI.CONOPS_CHI_DAILY_SHOVEL_ASSET_EFFICIENCY_PER_SHOVEL_V c
