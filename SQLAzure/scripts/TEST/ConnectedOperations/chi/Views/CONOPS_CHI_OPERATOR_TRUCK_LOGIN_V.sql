CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_TRUCK_LOGIN_V] AS
...
-- SELECT * FROM [chi].[CONOPS_CHI_OPERATOR_TRUCK_LOGIN_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_TRUCK_LOGIN_V] 
AS
...
WITH OperatorDetail AS (
    ...
    FROM [chi].[CONOPS_CHI_TRUCK_DETAIL_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
