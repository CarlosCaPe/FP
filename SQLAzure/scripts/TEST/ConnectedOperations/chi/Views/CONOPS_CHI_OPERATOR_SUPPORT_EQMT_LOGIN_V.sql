CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_SUPPORT_EQMT_LOGIN_V] AS




-- SELECT * FROM [chi].[CONOPS_CHI_OPERATOR_SUPPORT_EQMT_LOGIN_V] WHERE SHIFTFLAG = 'CURR'
CREATE VIEW [chi].[CONOPS_CHI_OPERATOR_SUPPORT_EQMT_LOGIN_V]
AS
...
WITH OperatorDetail AS (
    SELECT [shiftflag]
        ...
        ,[OperatorImageURL]
    FROM [chi].[CONOPS_CHI_EQMT_OTHER_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
