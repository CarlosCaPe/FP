CREATE VIEW [chi].[SHIFT_LOAD_DETAIL_V] AS

--SELECT * FROM CHI.SHIFT_LOAD_DETAIL_V
CREATE VIEW [CHI].[SHIFT_LOAD_DETAIL_V]
AS
...
LEFT JOIN chi.shift_eqmt eqe WITH(NOLOCK)
	ON sl.FieldExcav = eqe.Id
LEFT JOIN chi.shift_eqmt eqt WITH(NOLOCK)
	ON sl.FieldTruck = eqt.Id
--LEFT JOIN chi.SHIFT_OPER oe WITH(NOLOCK)
--	ON sl.FieldEoper = oe.Id
--LEFT JOIN chi.SHIFT_OPER ot WITH(NOLOCK)
--	ON sl.FieldToper = ot.Id
LEFT JOIN chi.enum exeqt WITH(NOLOCK)
	ON eqe.FieldEqmttype = exeqt.Id
LEFT JOIN chi.enum teqt
	ON eqt.FieldEqmttype = teqt.Id
--LEFT JOIN chi.shift_grade sg WITH(NOLOCK)
--	ON sl.FieldGrade = sg.Id
LEFT JOIN chi.SHIFT_LOC slo WITH(NOLOCK)
