CREATE VIEW [chi].[CONOPS_CHI_TP_TRUCK_ASSET_EFFICIENCY_V] AS
  
  
  
  
--select * from [dbo].[CONOPS_LH_SP_DELAY_V] where shiftflag = 'prev'  
CREATE VIEW [chi].[CONOPS_CHI_TP_TRUCK_ASSET_EFFICIENCY_V]   
AS  
...
LEFT JOIN [chi].CONOPS_CHI_TRUCK_POPUP b WITH (NOLOCK) 
