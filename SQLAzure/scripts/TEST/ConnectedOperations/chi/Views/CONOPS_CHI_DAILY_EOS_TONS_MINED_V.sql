CREATE VIEW [chi].[CONOPS_CHI_DAILY_EOS_TONS_MINED_V] AS

--SELECT * FROM [CHI].[CONOPS_CHI_DAILY_EOS_TONS_MINED_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [chi].[CONOPS_CHI_DAILY_EOS_TONS_MINED_V]
AS
...
FROM(
	...
	FROM [CHI].[CONOPS_CHI_DAILY_EOS_TOTAL_MATERIAL_V] ov
	LEFT JOIN [CHI].[CONOPS_CHI_SHIFT_BASE_TARGET_V] t
...
SELECT
	...
	CASE
		...
		ELSE 'Exceeds Plan'
	END AS Status
FROM [CHI].[CONOPS_CHI_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V]
CROSS APPLY
(
VALUES
