CREATE VIEW [ABR].[CONOPS_ABR_EQMT_ALARM_DRILL_OPERATOR_BREAK_V] AS


-- SELECT * FROM [abr].[CONOPS_ABR_EQMT_ALARM_DRILL_OPERATOR_BREAK_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_EQMT_ALARM_DRILL_OPERATOR_BREAK_V]
AS
	...
	OperatorBreak AS (
   		SELECT shiftflag,
			   ...
			   END_TIME_TS,
          	   ...
          	   OPERATORID,
          	   OperatorName
   		FROM [abr].[DRILL_UTILIZATION] [D] WITH (NOLOCK)
	...
	FROM (
		...
		WHERE cn > 2
		UNION ALL
		SELECT shiftflag, siteflag, [SHIFTINDEX], [MACHINESTATUSCODE], EQUIPMENTNUMBER, END