CREATE VIEW [ABR].[CONOPS_ABR_DAILY_DRILL_DETAIL_V] AS


 
-- SELECT * FROM [abr].[CONOPS_ABR_DAILY_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]      
CREATE VIEW [ABR].[CONOPS_ABR_DAILY_DRILL_DETAIL_V]      
AS      
...
SELECT DISTINCT ae.[shiftflag] ,      
 ...
 0 AS AssetEfficiencyTarget,    
    ...
    [XY_Drill_Score],      
    [OVERALLSCORE]      
FROM [abr].[CONOPS_ABR_DAILY_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)      
LEFT JOIN [abr].[CONOPS_ABR_DAILY_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)      
ON [ae].shiftid = [ds].shiftid --AND [ae].siteflag = [ds].siteflag      
AND [ds].DRILL_ID = [ae].Equipment      
--LEFT JOIN [abr].[CONOPS_ABR_DB_DRILL_SCORE_TARGET_V] [t] WITH (NOLOCK)      
--ON [ae].shiftid = [t].ShiftId      
