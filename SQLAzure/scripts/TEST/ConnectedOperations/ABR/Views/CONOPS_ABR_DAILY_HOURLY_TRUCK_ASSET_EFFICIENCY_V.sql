CREATE VIEW [ABR].[CONOPS_ABR_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] AS




  
--select * from [abr].[CONOPS_ABR_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V] WITH (NOLOCK)  
CREATE VIEW [ABR].[CONOPS_ABR_DAILY_HOURLY_TRUCK_ASSET_EFFICIENCY_V]   
AS  
...
WITH TimesDetails AS (
	...
	FROM(
		...
		FROM [ABR].[EQUIPMENT_HOURLY_STATUS] [HOS] WITH (NOLOCK)
