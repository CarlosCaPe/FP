CREATE VIEW [ABR].[CONOPS_ABR_CM_AVG_THROUGHPUT_V] AS


  
-- SELECT * FROM [abr].[CONOPS_ABR_CM_AVG_THROUGHPUT_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'    
CREATE VIEW [ABR].[CONOPS_ABR_CM_AVG_THROUGHPUT_V]    
AS    
...
WITH ShiftAvg AS (  
 SELECT ShiftIndex  
  ...
  --,Hr  
 FROM [abr].[CONOPS_ABR_CM_HOURLY_CRUSHER_THROUGHPUT_V] WITH (NOLOCK)  
...
FROM (   
 ...
 FROM [abr].[CONOPS_ABR_CM_HOURLY_CRUSHER_THROUGHPUT_V] [hct] WITH (NOLOCK)  
