CREATE VIEW [ABR].[CONOPS_ABR_DAILY_EOS_KPI_SUMMARY_V] AS
...
-- SELECT * FROM [abr].[CONOPS_ABR_DAILY_EOS_KPI_SUMMARY_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'      
CREATE VIEW [ABR].[CONOPS_ABR_DAILY_EOS_KPI_SUMMARY_V]      
AS      
...
FROM [abr].[CONOPS_ABR_DAILY_DELTA_J_V]  
WHERE DeltaJ <> 0  
