CREATE VIEW [ABR].[CONOPS_ABR_DB_DRILL_SCORE_CARD_PER_HOUR_V] AS


--select * from [abr].[CONOPS_ABR_DB_DRILL_SCORE_CARD_PER_HOUR_V] WITH (NOLOCK) where shiftflag = 'prev'
CREATE VIEW [ABR].[CONOPS_ABR_DB_DRILL_SCORE_CARD_PER_HOUR_V]
AS
	...
	WITH DrillKPI AS (
		...
		FROM (
			...
			FROM [abr].[CONOPS_ABR_SHIFT_INFO_V] a (NOLOCK)
       		...
       		ON [a].ShiftIndex = [ds].SHIFTINDEX AND [ds].SITE_CODE = 'ELA'
