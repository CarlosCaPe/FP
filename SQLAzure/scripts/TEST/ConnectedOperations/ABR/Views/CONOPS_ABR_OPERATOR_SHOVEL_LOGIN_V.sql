CREATE VIEW [ABR].[CONOPS_ABR_OPERATOR_SHOVEL_LOGIN_V] AS




-- SELECT * FROM [abr].[CONOPS_ABR_OPERATOR_SHOVEL_LOGIN_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [ABR].[CONOPS_ABR_OPERATOR_SHOVEL_LOGIN_V] 
AS
...
WITH OperatorDetail AS (
    SELECT [shiftflag]
        ...
        ,[OperatorImageURL]
    FROM [abr].[CONOPS_ABR_SHOVEL_INFO_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
