CREATE VIEW [ABR].[CONOPS_ABR_OPERATOR_TRUCK_LOGIN_V] AS


-- SELECT * FROM [abr].[CONOPS_ABR_OPERATOR_TRUCK_LOGIN_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [ABR].[CONOPS_ABR_OPERATOR_TRUCK_LOGIN_V] 
AS
...
WITH OperatorDetail AS (
    ...
    FROM [abr].[CONOPS_ABR_TRUCK_DETAIL_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
