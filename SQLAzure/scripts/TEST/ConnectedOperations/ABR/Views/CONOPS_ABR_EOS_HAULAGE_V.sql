CREATE VIEW [ABR].[CONOPS_ABR_EOS_HAULAGE_V] AS



--SELECT * FROM [abr].[CONOPS_ABR_EOS_HAULAGE_V] WHERE SHIFTFLAG = 'PREV'
CREATE VIEW [ABR].[CONOPS_ABR_EOS_HAULAGE_V]
AS
...
WITH NrOfLoad AS(
	...
	FROM [abr].[CONOPS_ABR_SP_NROFLOAD_V]
	GROUP BY shiftflag
),
...
FROM(
	...
	FROM [abr].[CONOPS_ABR_DELTA_C_DETAIL_V] [dc]
