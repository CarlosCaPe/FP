CREATE VIEW [SAF].[CONOPS_SAF_EOS_EQUIPMENT_READINESS_V] AS

--select * from [SAF].[CONOPS_SAF_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [saf].[CONOPS_SAF_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		INNER JOIN SAF.CONOPS_SAF_SHIFT_INFO_V s
