CREATE VIEW [SAF].[CONOPS_SAF_SHIFT_SNAPSHOT_V] AS




CREATE VIEW [saf].[CONOPS_SAF_SHIFT_SNAPSHOT_V]
AS
...
SELECT sbr.SHIFTID,
       ...
       ISNULL(SUM(sdps.TotalCrushedLeach), 0) + ISNULL(SUM(sdps.TotalStockpiledOre), 0) + ISNULL(SUM(sdps.TotalWaste), 0) AS [TotalMaterialMined]
