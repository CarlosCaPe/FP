CREATE VIEW [SAF].[CONOPS_SAF_DB_DRILL_SCORE_V] AS




--select * from [saf].[CONOPS_SAF_DB_DRILL_SCORE_V] WITH (NOLOCK) where shiftflag = 'prev'
CREATE VIEW [saf].[CONOPS_SAF_DB_DRILL_SCORE_V]
AS
	...
	DrillTime AS (
		SELECT [SHIFTINDEX],
			   'D' + RIGHT('000' + SUBSTRING(DRILL_ID,CHARINDEX('-',DRILL_ID)+1,LEN(DRILL_ID)), 3) DRILL_ID,
