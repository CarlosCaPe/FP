CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_TRUCK_LOGIN_V] AS


-- SELECT * FROM [tyr].[CONOPS_TYR_OPERATOR_TRUCK_LOGIN_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [TYR].[CONOPS_TYR_OPERATOR_TRUCK_LOGIN_V] 
AS
...
WITH OperatorDetail AS (
    ...
    FROM [tyr].[CONOPS_TYR_TRUCK_DETAIL_V] WITH (NOLOCK)
    WHERE [Operator] != 'NONE'
),
