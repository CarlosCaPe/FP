CREATE VIEW [TYR].[SHIFT_LOAD_DETAIL_V] AS

--SELECT * FROM TYR.SHIFT_LOAD_DETAIL_V
CREATE VIEW [TYR].[SHIFT_LOAD_DETAIL_V]
AS
...
LEFT JOIN tyr.shift_eqmt eqe WITH(NOLOCK)
	ON sl.FieldExcav = eqe.Id
LEFT JOIN tyr.shift_eqmt eqt WITH(NOLOCK)
	ON sl.FieldTruck = eqt.Id
--LEFT JOIN tyr.SHIFT_OPER oe WITH(NOLOCK)
--	ON sl.FieldEoper = oe.Id
--LEFT JOIN tyr.SHIFT_OPER ot WITH(NOLOCK)
--	ON sl.FieldToper = ot.Id
LEFT JOIN tyr.enum exeqt WITH(NOLOCK)
	ON eqe.FieldEqmttype = exeqt.Id
LEFT JOIN tyr.enum teqt
	ON eqt.FieldEqmttype = teqt.Id
LEFT JOIN tyr.shift_grade sg WITH(NOLOCK)
	ON sl.FieldGrade = sg.Id
LEFT JOIN tyr.SHIFT_LOC slo WITH(NOLOCK)
