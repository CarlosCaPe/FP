CREATE VIEW [TYR].[CONOPS_TYR_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [TYR].[CONOPS_TYR_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [TYR].[CONOPS_TYR_EQMT_STATUS_DURATION_V] 
AS
...
FROM [TYR].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [TYR].[CONOPS_TYR_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [TYR].[CONOPS_TYR_AUTONOMOUS_TRUCK_V] aut
