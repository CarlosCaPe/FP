CREATE VIEW [TYR].[CONOPS_TYR_DRILL_DETAIL_V] AS



  
-- SELECT * FROM [tyr].[CONOPS_TYR_DRILL_DETAIL_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR' ORDER BY [DRILL_ID]    
CREATE VIEW [TYR].[CONOPS_TYR_DRILL_DETAIL_V]    
AS    
...
SELECT ae.[shiftflag] ,    
 ...
 0 AS AssetEfficiencyTarget,  
    ...
    [XY_Drill_Score],    
    [OVERALLSCORE]    
FROM [tyr].[CONOPS_TYR_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] [ae] WITH (NOLOCK)    
LEFT JOIN [tyr].[CONOPS_TYR_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK)    
