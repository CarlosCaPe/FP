CREATE VIEW [cer].[CONOPS_CER_SP_EQMT_STATUS_GANTTCHART_V] AS





--select * from [cer].[CONOPS_CER_SP_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'prev'
CREATE VIEW [cer].[CONOPS_CER_SP_EQMT_STATUS_GANTTCHART_V]
AS
...
WITH SHIFTINFO AS (
     ...
     LEFT JOIN
         (SELECT siteflag,
          ...
          GROUP BY siteflag,
                   shiftflag) b ON a.shiftflag = b.shiftflag
),
...
eqmt,
tprh 
FROM [cer].[CONOPS_CER_SHOVEL_TPRH_V] WITH (NOLOCK)
WHERE site_code = 'CER')
