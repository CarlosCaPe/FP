CREATE VIEW [cer].[CONOPS_CER_SP_EQMT_STATUS_V] AS




--select * from [cer].[CONOPS_CER_SP_EQMT_STATUS_V] where shiftflag = 'curr'

CREATE VIEW [cer].[CONOPS_CER_SP_EQMT_STATUS_V]
AS
...
LEFT JOIN (
	 SELECT shiftindex,
            ...
            eqmt,
            tprh
     FROM [cer].[CONOPS_CER_SHOVEL_TPRH_V] (NOLOCK)
     WHERE site_code = 'CER'
