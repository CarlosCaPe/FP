CREATE VIEW [cer].[CONOPS_CER_CM_AVG_THROUGHPUT_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_CM_AVG_THROUGHPUT_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'PREV'  
CREATE VIEW [cer].[CONOPS_CER_CM_AVG_THROUGHPUT_V]  
AS  
...
FROM ( 
	...
	FROM [cer].[CONOPS_CER_CM_HOURLY_CRUSHER_THROUGHPUT_V] [hct] WITH (NOLOCK)
