CREATE VIEW [cer].[CONOPS_CER_EQMT_TRUCK_HOURLY_TOTALMATERIALDELIVERED_V] AS
...
--select * from [cer].[CONOPS_CER_EQMT_TRUCK_HOURLY_TOTALMATERIALDELIVERED_V] order by shiftseq  
CREATE VIEW [CER].[CONOPS_CER_EQMT_TRUCK_HOURLY_TOTALMATERIALDELIVERED_V]  
AS  
...
CAST(CONCAT(CAST(LoadTime AS DATE),' ',LEFT(CAST(LoadTime AS TIME),5),':00.000') AS DATETIME)   
END AS LoadTime,  
truckid,  
