CREATE VIEW [cer].[CONOPS_CER_OPERATOR_DRILL_LIST_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_OPERATOR_DRILL_LIST_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [cer].[CONOPS_CER_OPERATOR_DRILL_LIST_V] 
AS
	...
	WITH OperatorDetail AS (
		...
		FROM [cer].[CONOPS_CER_DRILL_DETAIL_V] d WITH (NOLOCK)
