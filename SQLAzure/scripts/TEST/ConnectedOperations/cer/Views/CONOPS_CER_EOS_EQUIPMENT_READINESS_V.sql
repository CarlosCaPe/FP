CREATE VIEW [cer].[CONOPS_CER_EOS_EQUIPMENT_READINESS_V] AS

--select * from [CER].[CONOPS_CER_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [cer].[CONOPS_CER_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		FROM (
			...
			FROM CER.EQUIPMENT_HOURLY_STATUS WITH(NOLOCK)
			WHERE
		...
		INNER JOIN CER.CONOPS_CER_SHIFT_INFO_V s
