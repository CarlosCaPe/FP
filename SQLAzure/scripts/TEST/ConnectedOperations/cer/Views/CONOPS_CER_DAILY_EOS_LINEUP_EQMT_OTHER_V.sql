CREATE VIEW [cer].[CONOPS_CER_DAILY_EOS_LINEUP_EQMT_OTHER_V] AS
...
-- SELECT * FROM [cer].[CONOPS_CER_DAILY_EOS_LINEUP_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'  ORDER BY shiftflag, siteflag, SupportEquipment
CREATE VIEW [cer].[CONOPS_CER_DAILY_EOS_LINEUP_EQMT_OTHER_V]
AS
...
FROM( 
	...
	LEFT JOIN [CER].[CONOPS_CER_EOS_SHIFT_INFO_V] [shift]
