CREATE VIEW [cer].[CONOPS_CER_EQMT_STATUS_DURATION_V] AS



-- SELECT * FROM [CER].[CONOPS_CER_EQMT_STATUS_DURATION_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [CER].[CONOPS_CER_EQMT_STATUS_DURATION_V] 
AS
...
FROM [CER].[asset_efficiency] [ae] (NOLOCK)
RIGHT JOIN [CER].[CONOPS_CER_SHIFT_INFO_V] [shift] WITH (NOLOCK)
	ON [ae].SHIFTID = [shift].SHIFTID
LEFT JOIN [CER].[CONOPS_CER_AUTONOMOUS_TRUCK_V] aut
