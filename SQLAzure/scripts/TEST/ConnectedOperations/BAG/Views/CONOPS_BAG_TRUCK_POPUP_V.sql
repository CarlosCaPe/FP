CREATE VIEW [BAG].[CONOPS_BAG_TRUCK_POPUP_V] AS

-- SELECT * FROM [bag].[CONOPS_BAG_TRUCK_POPUP_V] WITH (NOLOCK) WHERE [shiftflag] = 'CURR'
CREATE VIEW [bag].[CONOPS_BAG_TRUCK_POPUP_V]
AS
...
DCTarget AS (
	SELECT
		...
		b.TRUCKUSEOFAVAILABILITY AS useOfAvailabilityTarget,
		b.TRUCKAVAILABILITY AS AvailabilityTarget,
		b.EFH as EFHtarget
...
LEFT JOIN [bag].[CONOPS_BAG_TP_AVG_PAYLOAD_V] [payload] WITH (NOLOCK) 
...
LEFT JOIN DCTarget dct
	ON dct.FORMATSHIFTID = t.shiftid
