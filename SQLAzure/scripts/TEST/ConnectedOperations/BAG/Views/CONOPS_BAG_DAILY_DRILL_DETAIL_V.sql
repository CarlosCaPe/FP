...
OperatorDetail AS (
	...
	LEFT JOIN [dbo].[operator_personnel_map] [w] WITH (NOLOCK) ON CAST([w].[OPERATOR_ID] AS NUMERIC) = CAST([ds].OPERATORID AS NUMERIC)
		AND [w].SHIFTINDEX = [ds].SHIFTINDEX
...
LEFT JOIN [bag].[CONOPS_BAG_DAILY_DB_DRILL_SCORE_V] [ds] WITH (NOLOCK) ON [ae].shiftid = [ds].shiftid
	AND [ae].siteflag = [ds].siteflag
	AND [ds].DRILL_ID = [ae].Equipment
LEFT JOIN [bag].[CONOPS_BAG_DB_DRILL_SCORE_TARGET_V] [t] WITH (NOLOCK) ON LEFT([ae].shiftid, 4) = [t].ShiftId
	AND [ae].siteflag = [t].siteflag
LEFT JOIN EqmtStatus [es] ON [es].eqmt = [ae].Equipment
	AND [es].num = 1
	AND [es].SHIFTINDEX = [ae].ShiftIndex
	AND [ae].siteflag = [es].site_code
LEFT JOIN OperatorDetail [o] ON [o].DRILL_ID = [ae].Equipment
	AND [o].num = 1
	AND [o].SHIFTINDEX = [ae].ShiftIndex
	AND [ae].siteflag = [o].[SITE_CODE]
LEFT JOIN dbo.LOOKUPS [img] WITH (NOLOCK) ON [img].TableType = 'CONF'
