CREATE VIEW [BAG].[CONOPS_BAG_EQMT_OTHER_V] AS
...
--SELECT * FROM [bag].[CONOPS_BAG_EQMT_OTHER_V] WITH (NOLOCK) WHERE shiftflag = 'CURR'  ORDER BY shiftflag, siteflag, SupportEquipment  
CREATE VIEW [bag].[CONOPS_BAG_EQMT_OTHER_V]  
AS  
...
s.shiftid,  
s.shiftindex,  
p.EquipmentID AS SupportEquipmentId,  
p.EquipmentCategory AS SupportEquipment,  
e.StatusCode,  
e.StatusName,  
e.ReasonId,  
e.ReasonName AS ReasonDesc,  
e.StatusStart,  
DATEDIFF(MINUTE, e.StatusStart, GETDATE()) AS Duration,  
CONCAT('Crew', s.CrewID) AS Crew,  
p.Location,  
NULL AS Region,  
p.Operator,  
p.OperatorId,  
...
WHERE p.EquipmentCategory NOT IN ('Truck Classes', 'Shovel Classes', 'Loader Classes', 'Track Drill')  
