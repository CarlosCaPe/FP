CREATE VIEW [BAG].[CONOPS_BAG_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V] AS

--select * from [bag].[CONOPS_BAG_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V]  
CREATE VIEW [bag].[CONOPS_BAG_DB_DRILL_ASSET_EFFICIENCY_PER_DRILL_V]  
AS  
 ...
 SELECT  TimesDetails.shiftindex ,  
   ...
   CASE WHEN TimesDetails.TotalTime = 0 THEN 0   
     ELSE CAST(( TimesDetails.ReadyTime + TimesDetails.DelayTime + TimesDetails.SpareTime + TimesDetails.ShiftChangeTime ) AS FLOAT)  
       / CAST(TimesDetails.TotalTime AS FLOAT) * 100.00  
   END AS Avail,  
   CASE WHEN TimesDetails.TotalTime = 0 THEN 0  
     ELSE CAST(TimesDetails.ReadyTime AS FLOAT) / CAST(TimesDetails.TotalTime AS FLOAT) * 100.00  
   END AS AE ,  
   CASE WHEN TimesDetails.ReadyTime + TimesDetails.DelayTime + TimesDetails.SpareTime + TimesDetails.ShiftChangeTime = 0  
     ...
     ELSE CAST(TimesDetails.ReadyTime AS FLOAT) / CAST(TimesDetails.ReadyTime + TimesDetails.DelayTime + TimesDetails.SpareTime  
       + TimesDetails.ShiftChangeTime AS FLOAT) * 100.00  
   END AS UofA  
 FROM    TimesDetails  
) [drillAE]  
