CREATE VIEW [Arch].[CONOPS_ARCH_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V] AS




CREATE     VIEW [Arch].[CONOPS_ARCH_TRUCK_ASSET_EFFICIENCY_PER_TRUCK_V]
AS
...
LEFT JOIN (
	...
	from
	(
		select shiftid,unittype, eqmt,[SITECODE] AS [siteflag],
		...
		group by SITECODE, shiftid,unittype, eqmt
	) calc
	where unittype = 'truck'
) [ta]
ON [ta].shiftid = [shift].shiftid
   AND [ta].[siteflag] = [shift].[siteflag]
