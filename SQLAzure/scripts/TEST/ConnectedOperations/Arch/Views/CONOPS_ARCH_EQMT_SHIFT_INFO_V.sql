CREATE VIEW [Arch].[CONOPS_ARCH_EQMT_SHIFT_INFO_V] AS


CREATE VIEW [Arch].[CONOPS_ARCH_EQMT_SHIFT_INFO_V]
AS
...
FROM [Arch].[shift_info] WITH (NOLOCK)
...
END AS shiftid
FROM  [Arch].[shift_info] WITH (NOLOCK) ) a
...
ShiftDuration 
FROM [Arch].[shift_info] WITH (nolock)) b
