CREATE VIEW [Arch].[CONOPS_ARCH_SHOVEL_TO_WATCH_V] AS



CREATE VIEW [Arch].[CONOPS_ARCH_SHOVEL_TO_WATCH_V]
AS
...
LEFT JOIN [Arch].[CONOPS_ARCH_SHOVEL_POPUP_V] a ON tn.shiftid = a.shiftid and tn.ShovelId = a.ShovelID
LEFT JOIN STAT st ON a.shiftid = st.shiftid AND st.eqmt = a.ShovelID AND a.siteflag = '<SITECODE>'
