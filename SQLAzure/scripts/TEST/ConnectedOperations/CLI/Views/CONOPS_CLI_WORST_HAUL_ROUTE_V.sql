CREATE VIEW [CLI].[CONOPS_CLI_WORST_HAUL_ROUTE_V] AS


-- SELECT * FROM [cli].[CONOPS_CLI_WORST_HAUL_ROUTE_V] WITH (NOLOCK)
CREATE VIEW [cli].[CONOPS_CLI_WORST_HAUL_ROUTE_V] 
AS
...
FROM (
	...
	ON cte.TRUCK = [truck].TruckID
	   AND cte.SHIFTINDEX = [truck].shiftindex AND [truck].[siteflag] = 'CMX'
	WHERE cte.rn = 1
) [worstHaul]
