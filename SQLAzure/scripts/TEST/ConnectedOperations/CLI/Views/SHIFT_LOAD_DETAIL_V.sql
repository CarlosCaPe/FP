CREATE VIEW [CLI].[SHIFT_LOAD_DETAIL_V] AS

--SELECT * FROM CLI.SHIFT_LOAD_DETAIL_V
CREATE VIEW [CLI].[SHIFT_LOAD_DETAIL_V]
AS
...
LEFT JOIN cli.shift_eqmt eqe WITH(NOLOCK)
	ON sl.FieldExcav = eqe.Id
LEFT JOIN cli.shift_eqmt eqt WITH(NOLOCK)
	ON sl.FieldTruck = eqt.Id
--LEFT JOIN cli.SHIFT_OPER oe WITH(NOLOCK)
--	ON sl.FieldEoper = oe.Id
--LEFT JOIN cli.SHIFT_OPER ot WITH(NOLOCK)
--	ON sl.FieldToper = ot.Id
LEFT JOIN cli.enum exeqt WITH(NOLOCK)
	ON eqe.FieldEqmttype = exeqt.Id
LEFT JOIN cli.enum teqt
	ON eqt.FieldEqmttype = teqt.Id
LEFT JOIN cli.shift_grade sg WITH(NOLOCK)
	ON sl.FieldGrade = sg.Id
LEFT JOIN cli.SHIFT_LOC slo WITH(NOLOCK)
