CREATE VIEW [CLI].[CONOPS_CLI_DB_EQMT_STATUS_GANTTCHART_V] AS
  
 
  
--select * from [cli].[CONOPS_CLI_DB_EQMT_STATUS_GANTTCHART_V] where shiftflag = 'prev'  
CREATE VIEW [cli].[CONOPS_CLI_DB_EQMT_STATUS_GANTTCHART_V]  
AS  
...
EVNTS AS (  
   SELECT SHIFTINDEX,  
    DRILL_ID AS eqmt,  
             ...
             reason AS reasons,  
             [status]  
      FROM [cli].[drill_asset_efficiency_v] (NOLOCK)  
),  
...
NrOfHoles AS (  
  ...
  FROM [dbo].[FR_DRILLING_SCORES] [ds]  
  WHERE SITE_CODE = 'CLI'  
