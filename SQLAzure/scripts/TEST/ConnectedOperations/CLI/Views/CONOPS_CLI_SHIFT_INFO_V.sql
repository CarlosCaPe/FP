CREATE VIEW [CLI].[CONOPS_CLI_SHIFT_INFO_V] AS




--SELECT * FROM [CLI].[CONOPS_CLI_SHIFT_INFO_V]    
    
CREATE VIEW [cli].[CONOPS_CLI_SHIFT_INFO_V]     
AS    
...
FROM (    
 ...
 FROM    
   (SELECT SITEFLAG,    
    ...
    FROM [CLI].[SHIFT_INFO] WITH (NOLOCK)    
    GROUP BY SITEFLAG    
    UNION    
    SELECT SITEFLAG,    
     'CURR' AS SHIFTFLAG,    
