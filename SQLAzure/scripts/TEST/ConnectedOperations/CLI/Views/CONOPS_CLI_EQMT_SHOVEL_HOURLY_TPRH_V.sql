CREATE VIEW [CLI].[CONOPS_CLI_EQMT_SHOVEL_HOURLY_TPRH_V] AS

--select * from [cli].[CONOPS_CLI_EQMT_SHOVEL_HOURLY_TPRH_V] where shiftflag = 'curr'
CREATE VIEW [cli].[CONOPS_CLI_EQMT_SHOVEL_HOURLY_TPRH_V]
AS
...
FROM(
	...
	FROM CLI.EQUIPMENT_HOURLY_STATUS (NOLOCK) AS HOS
	WHERE HOS.UNIT = '2' 
) AS sub1
...
FROM CLI.SHIFT_LOAD_DETAIL_V (NOLOCK)
