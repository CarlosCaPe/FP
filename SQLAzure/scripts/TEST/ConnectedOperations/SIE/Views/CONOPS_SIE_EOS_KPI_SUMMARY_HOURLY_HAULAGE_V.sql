CREATE VIEW [SIE].[CONOPS_SIE_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] AS




-- SELECT * FROM [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V] WITH (NOLOCK) WHERE SHIFTFLAG = 'CURR'  
CREATE VIEW [sie].[CONOPS_SIE_EOS_KPI_SUMMARY_HOURLY_HAULAGE_V]  
AS  
	...
	TonnageRaw AS (
		...
		FROM (
			SELECT Siteflag,
				   ...
				   ShiftStartDateTime,
				   ShiftEndDateTime
			FROM [sie].[CONOPS_SIE_SHIFT_LINE_GRAPH_V] (NOLOCK)
		) m
	),

	EFHTarget AS (
		SELECT substring(replace(cast(getdate() as date),'-',''),3,4) as shiftdate,
	...
	--	FROM  [sie].[CONOPS_SIE_SHIFT_TARGET_V] WITH (NOLOCK)
	--),
