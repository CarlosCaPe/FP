CREATE VIEW [SIE].[CONOPS_SIE_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V] AS
...
--select * from [sie].[CONOPS_SIE_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V]  
CREATE VIEW [sie].[CONOPS_SIE_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_V]   
AS  
...
FROM [sie].[CONOPS_SIE_EOS_SHIFT_INFO_V] a WITH (NOLOCK)  
)  
...
LEFT JOIN [sie].[CONOPS_SIE_DAILY_MATERIAL_DELIVERED_TO_CHRUSHER_TARGET_V] [ct] WITH (NOLOCK)  
ON [ct].shiftid = cl.shiftid AND cl.CrusherLoc = [ct].[Location]  
GROUP BY cl.siteflag, cl.SHIFTFLAG, cl.CrusherLoc, [ct].MillOreTarget, MillOreShiftTarget, TotalTons,cl.shiftid  
