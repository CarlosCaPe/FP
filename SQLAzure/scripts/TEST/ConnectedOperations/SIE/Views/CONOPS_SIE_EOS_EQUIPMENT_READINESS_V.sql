CREATE VIEW [SIE].[CONOPS_SIE_EOS_EQUIPMENT_READINESS_V] AS

--select * from [SIE].[CONOPS_SIE_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [sie].[CONOPS_SIE_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		INNER JOIN SIE.CONOPS_SIE_SHIFT_INFO_V s
