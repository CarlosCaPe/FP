CREATE VIEW [SIE].[CONOPS_SIE_DB_DRILL_SCORE_V] AS




--select * from [sie].[CONOPS_SIE_DB_DRILL_SCORE_V] WITH (NOLOCK) where shiftflag = 'prev'
CREATE VIEW [sie].[CONOPS_SIE_DB_DRILL_SCORE_V]
AS
	...
	SELECT a.shiftflag,
		   ...
		   ISNULL([Holes_Drilled], 0) AS [Holes_Drilled],
		   ISNULL([Feet_Drilled], 0) AS [Feet_Drilled],
		   ISNULL([Average_HoleTime], 0) AS [Average_HoleTime],
		   ISNULL([Total_Depth], 0) AS [Total_Depth],
		   ISNULL([OVERALLSCORE], 0) AS [OVERALLSCORE]
	FROM [sie].[CONOPS_SIE_SHIFT_INFO