CREATE VIEW [MOR].[CONOPS_MOR_DB_DRILL_SCORE_TARGET_V] AS

--select * from [mor].[CONOPS_MOR_DB_DRILL_SCORE_TARGET_V]where shiftflag = 'curr'
CREATE VIEW [mor].[CONOPS_MOR_DB_DRILL_SCORE_TARGET_V]
AS
...
FROM (
    ...
    FROM [mor].[plan_values_monthly_drilling] (NOLOCK)
) [target]
