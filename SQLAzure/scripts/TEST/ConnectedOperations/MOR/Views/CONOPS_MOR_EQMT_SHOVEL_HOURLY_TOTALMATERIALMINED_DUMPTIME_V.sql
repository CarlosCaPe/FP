CREATE VIEW [MOR].[CONOPS_MOR_EQMT_SHOVEL_HOURLY_TOTALMATERIALMINED_DUMPTIME_V] AS
  
  
--select * from [mor].[CONOPS_MOR_EQMT_SHOVEL_HOURLY_TOTALMATERIALMINED_DUMPTIME_V]  
CREATE VIEW [mor].[CONOPS_MOR_EQMT_SHOVEL_HOURLY_TOTALMATERIALMINED_DUMPTIME_V]  
AS  
...
AS ( SELECT     
   ...
   from [mor].[shift_info] WITH (NOLOCK)) sinfo ON sd.shiftid = sinfo.shiftid  
...
( ISNULL(SUM(sdps.TotalMillOreMined), 0)  
  ...
  + ISNULL(SUM(sdps.TotalTransferOre), 0) ) AS TotalMaterialMoved ,   
( ISNULL(SUM(sdps.Total