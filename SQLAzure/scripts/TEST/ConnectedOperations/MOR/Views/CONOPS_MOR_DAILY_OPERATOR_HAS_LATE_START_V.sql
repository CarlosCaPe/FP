CREATE VIEW [MOR].[CONOPS_MOR_DAILY_OPERATOR_HAS_LATE_START_V] AS
  
  
-- SELECT * FROM [mor].[CONOPS_MOR_DAILY_OPERATOR_HAS_LATE_START_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'    
CREATE VIEW [mor].[CONOPS_MOR_DAILY_OPERATOR_HAS_LATE_START_V]     
AS    
...
LEFT JOIN (    
 SELECT [LateStart].shiftindex,    
    ...
    WHEN 2 THEN LEFT(CAST(DATEADD(MINUTE, -15, [fls].TIMELOAD_