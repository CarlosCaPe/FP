CREATE VIEW [MOR].[CONOPS_MOR_EOS_EQUIPMENT_READINESS_V] AS

--select * from [MOR].[CONOPS_MOR_EOS_EQUIPMENT_READINESS_V] where shiftflag = 'prev'
CREATE VIEW [mor].[CONOPS_MOR_EOS_EQUIPMENT_READINESS_V]
AS
...
FROM (
	...
	FROM (
		...
		INNER JOIN MOR.CONOPS_MOR_SHIFT_INFO_V s
