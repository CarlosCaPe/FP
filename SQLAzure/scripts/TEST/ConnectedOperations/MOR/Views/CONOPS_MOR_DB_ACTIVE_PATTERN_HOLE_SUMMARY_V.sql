CREATE VIEW [MOR].[CONOPS_MOR_DB_ACTIVE_PATTERN_HOLE_SUMMARY_V] AS

--SELECT * FROM MOR.CONOPS_MOR_DB_ACTIVE_PATTERN_HOLE_SUMMARY_V
CREATE VIEW MOR.CONOPS_MOR_DB_ACTIVE_PATTERN_HOLE_SUMMARY_V
AS
...
SELECT
	*,
	...
	--	WHEN PLANNED_BLAST_DATE_TARGET < GETDATE() THEN '' --Already Late
	--	WHEN ESTIMATED_FINISH_DATE <= PLANNED_BLAST_DATE_TARGET AND ESTIMATED_FINISH_DATE IS NOT NULL THEN '' --OnTime
	--	WHEN EST_FINISH_1Extra <= PLANNED_BLAST_DATE_TARGET THEN CONCAT('Add 1 drills to finish ', CAST(EST_FINISH_1Extra AS VARCHAR(33)))
	--	WHEN EST_FINISH_2Extra <= P