CREATE VIEW [MOR].[CONOPS_MOR_OPERATOR_DRILL_LIST_V] AS
...
-- SELECT * FROM [mor].[CONOPS_MOR_OPERATOR_DRILL_LIST_V] WITH (NOLOCK) WHERE Shiftflag = 'PREV'
CREATE VIEW [mor].[CONOPS_MOR_OPERATOR_DRILL_LIST_V] 
AS
	...
	OperatorDetail AS (
		...
		FROM OperatorList [o]
		LEFT JOIN [mor].[CONOPS_MOR_DRILL_DETAIL_V] d WITH (NOLOCK)
